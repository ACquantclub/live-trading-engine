...
// TODO Windows - need to figure out why adding DLL_PUBLIC to this class leads to weird compilation
// errors on CspEnumMeta's unordered_map...

// This is an extension of csp::CspEnumMeta for python dialect, we need it in order to
// keep a reference to the python enum type from conversion to/from csp::CspEnumMeta <-> PyObject
// properly
class CSPTYPESIMPL_EXPORT DialectCspEnumMeta : public CspEnumMeta {
  public:
    ...
    const PyTypeObjectPtr& pyType() const {
        return m_pyType;
    }

    const PyCspEnumMeta* pyMeta() const {
        return (const PyCspEnumMeta*)m_pyType.get();
    }
...
struct CSPTYPESIMPL_EXPORT PyCspEnum : public PyObject {
    ...
    PyCspEnumMeta* pyMeta() {
        return (PyCspEnumMeta*)ob_type;
    };
    const DialectCspEnumMeta* meta() {
        return static_cast<const DialectCspEnumMeta*>(pyMeta()->enumMeta.get());
    }
