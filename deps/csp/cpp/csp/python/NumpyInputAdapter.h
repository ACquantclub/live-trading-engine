...
class NumpyCurveAccessor {
    // Accesses values by indexing only on the outermost dimension of an n-dimensional Numpy array
    // and then creating a view to the subarray Thus, you can get the nested Numpy array values in
    // csp.curve with zero-copy operations
  ...
  public:
    ...
    NumpyCurveAccessor(PyArrayObject* arr) {
        ...
        if (m_nd < 2)
            CSP_THROW(csp::TypeError,
                      "NumpyCurveAccessor is inefficient for a 1-D Numpy array: use "
                      "PyArray_GETPTR1 to access indexed values");
    ...
    PyObject* data(int64_t index) const {
        ...
        /*
        A note on reference counting for the subarray: NewFromDescr will *steal* a reference to the
        type descr, so we need to independently incref m_descr before passing it to NewFromDescr.
        Furthermore, SetBaseObject steals a reference from m_arr to inner_value, so we need to
        incref m_arr before passing it to SetBaseObject. If the order is not as such, deallocation
        may occur prematurely. After fromPython is called in NumpyInputAdapter, the ref count of
        inner_value will be decremented and ownership is transferred to the NumpyInputAdapter.
        */

        Py_XINCREF(m_descr);
        PyArrayObject* inner_value = (PyArrayObject*)PyArray_NewFromDescr(
            &PyArray_Type, m_descr, m_nd - 1, m_innerDims, m_innerStrides,
    ...
    NumpyInputAdapter(Engine* engine, CspTypePtr& type, PyArrayObject* datetimes,
        ...
        if (dt_type != NPY_DATETIMELTR && dt_type != NPY_OBJECTLTR)
            CSP_THROW(ValueError,
                      "timestamps ndarray must be dtype of datetime64 or object, got type code of "
                          << dt_type);
    ...
    void start(DateTime start, DateTime end) override {
        while (m_index < m_size) {
            ...
            if (m_dt_scale == 0) {
                ...
                int64_t numpy_dt =
                    *(static_cast<int64_t*>(PyArray_GETPTR1(m_datetimes.ptr(), m_index)));
...
void NumpyInputAdapter<int64_t>::setValue(int64_t& value, void* data) {
    switch (m_val_type) {
        ...
        default:
            CSP_THROW(ValueError, "NumpyInputAdapter<int64_t>::setValue sees invalid numpy type "
                                      << m_val_type);
    }
}
...
void NumpyInputAdapter<double>::setValue(double& value, void* data) {
    switch (m_val_type) {
        ...
        default:
            CSP_THROW(ValueError,
                      "NumpyInputAdapter<double>::setValue sees invalid numpy type " << m_val_type);
    }
};
