...
template <typename PyBasketInputProxyT>
using ValidKeysIterator =
    TsIterator<InputBasketInfo::valid_iterator, KeyGetter<PyBasketInputProxyT>>;
template <typename PyBasketInputProxyT>
using ValidItemsIterator =
    TsIterator<InputBasketInfo::valid_iterator, ItemGetter<PyBasketInputProxyT>>;

using TickedValuesIterator = TsIterator<InputBasketInfo::ticked_iterator, ValueGetter>;
template <typename PyBasketInputProxyT>
using TickedKeysIterator =
    TsIterator<InputBasketInfo::ticked_iterator, KeyGetter<PyBasketInputProxyT>>;
template <typename PyBasketInputProxyT>
using TickedItemsIterator =
    TsIterator<InputBasketInfo::ticked_iterator, ItemGetter<PyBasketInputProxyT>>;
...
void PyBaseBasketInputProxy::setBufferingPolicy(int32_t tickCount, TimeDelta tickHistory) {
    ...
    // if its dynamic, set the policy on the basket as well so that newly added keys get the same
    // policy applied
...
static PyObject* PyBaseBasketInputProxy_make_passive(PyBaseBasketInputProxy* proxy,
                                                     PyObject* args) {
    CSP_BEGIN_METHOD;
...
static PyObject* PyBaseBasketInputProxy_set_buffering_policy(PyBaseBasketInputProxy* proxy,
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "|O!O", (char**)kwlist, &PyLong_Type, &tickCount,
                                     &tickHistory))
        CSP_THROW(PythonPassthrough, "");

    if (!tickCount && !tickHistory)
        CSP_THROW(TypeError,
                  "csp.set_buffering_policy expected at least one of tick_count or tick_history");
...
PyListBasketInputProxy::PyListBasketInputProxy(PyNode* node, INOUT_ID_TYPE id, size_t shape)
    ...
    for (size_t elemId = 0; elemId < shape; ++elemId)
        m_proxies.emplace_back(
            PyInputProxyPtr::own(PyInputProxy::create(node, InputId(id, elemId))));
}

PyListBasketInputProxy* PyListBasketInputProxy::create(PyNode* node, INOUT_ID_TYPE id,
                                                       size_t shape) {
    if (shape > InputId::maxBasketElements())
        CSP_THROW(ValueError, "List basket size of " << shape << " exceeds basket size limit of "
                                                     << InputId::maxBasketElements() << " in node "
                                                     << node->name());

    PyListBasketInputProxy* proxy =
        (PyListBasketInputProxy*)PyType.tp_new(&PyType, nullptr, nullptr);
...
static PyMethodDef PyListBasketInputProxy_methods[] = {
    {"set_buffering_policy", (PyCFunction)PyBaseBasketInputProxy_set_buffering_policy,
     METH_VARARGS | METH_KEYWORDS, "set basket buffering policy"},
    {"make_active", (PyCFunction)PyBaseBasketInputProxy_make_active, METH_NOARGS,
     "make input active"},
    {"make_passive", (PyCFunction)PyBaseBasketInputProxy_make_passive, METH_NOARGS,
     "make input passive"},
    {"validvalues", (PyCFunction)PyBaseBasketInputProxy_validvalues, METH_NOARGS,
     "iterator of all valid values"},
    {"tickedvalues", (PyCFunction)PyBaseBasketInputProxy_tickedvalues, METH_NOARGS,
     "iterator of all ticked values"},
    {"validkeys", (PyCFunction)PyBaseBasketInputProxy_validkeys<PyListBasketInputProxy>,
     METH_NOARGS, "iterator of all valid input keys"},
    {"validitems", (PyCFunction)PyBaseBasketInputProxy_validitems<PyListBasketInputProxy>,
     METH_NOARGS, "iterator of key,value tuples of all valid inputs"},
    {"tickedkeys", (PyCFunction)PyBaseBasketInputProxy_tickedkeys<PyListBasketInputProxy>,
     METH_NOARGS, "iterator of all ticked input keys"},
    {"tickeditems", (PyCFunction)PyBaseBasketInputProxy_tickeditems<PyListBasketInputProxy>,
     METH_NOARGS, "iterator of key,value tuples of all ticked inputs"},
...
PyDictBasketInputProxy::PyDictBasketInputProxy(PyNode* node, INOUT_ID_TYPE id, PyObject* shape)
    : PyBaseBasketInputProxy(node, id), m_shape(PyObjectPtr::incref(shape)) {
...
PyDictBasketInputProxy* PyDictBasketInputProxy::create(PyNode* node, INOUT_ID_TYPE id,
                                                       PyObject* shape) {
    if (!PyList_Check(shape))
        CSP_THROW(TypeError,
                  "Invalid shape for dict basket, expect list got: " << Py_TYPE(shape)->tp_name);

    if ((size_t)PyList_GET_SIZE(shape) > InputId::maxBasketElements())
        CSP_THROW(ValueError, "Dict basket size of "
                                  << PyList_GET_SIZE(shape) << " exceeds basket size limit of "
                                  << InputId::maxBasketElements() << " in node " << node->name());

    PyDictBasketInputProxy* proxy =
        (PyDictBasketInputProxy*)PyType.tp_new(&PyType, nullptr, nullptr);
...
PyInputProxy* PyDictBasketInputProxy::proxyByKey(PyObject* key) {
    ...
    if (!in_proxy)
        CSP_THROW(KeyError,
                  "key " << PyObjectPtr::incref(key) << " %s is not a member of the dict basket");
    return in_proxy;
}
...
static PyObject* PyDictBasketInputProxy_getvalue(PyDictBasketInputProxy* proxy, PyObject* key) {
    ...
    if (!in_proxy->valid())
        CSP_THROW(RuntimeException,
                  "dict basket element " << PyObjectPtr::incref(key) << " is not valid");
...
static PyMethodDef PyDictBasketInputProxy_methods[] = {
    ...
    {"tickedvalues", (PyCFunction)PyBaseBasketInputProxy_tickedvalues, METH_NOARGS,
     "iterator of all ticked values"},
    {"validkeys", (PyCFunction)PyBaseBasketInputProxy_validkeys<PyDictBasketInputProxy>,
     METH_NOARGS, "iterator of all valid input keys"},
    {"validitems", (PyCFunction)PyBaseBasketInputProxy_validitems<PyDictBasketInputProxy>,
     METH_NOARGS, "iterator of key,value tuples of all valid inputs"},
    {"tickedkeys", (PyCFunction)PyBaseBasketInputProxy_tickedkeys<PyDictBasketInputProxy>,
     METH_NOARGS, "iterator of all ticked input keys"},
    {"tickeditems", (PyCFunction)PyBaseBasketInputProxy_tickeditems<PyDictBasketInputProxy>,
     METH_NOARGS, "iterator of key,value tuples of all ticked inputs"},
    {"keys", (PyCFunction)PyDictBasketInputProxy_keys, METH_NOARGS,
     "list of all keys on the basket"},
...
PyDynamicBasketInputProxy::PyDynamicBasketInputProxy(PyNode* node, INOUT_ID_TYPE id,
                                                     PyObject* shape)
    : PyDictBasketInputProxy(node, id, shape) {
    auto* dynBasket = const_cast<DynamicInputBasketInfo*>(
        static_cast<const DynamicInputBasketInfo*>(basketInfo()));
    dynBasket->setChangeCallback(
        [this](const DialectGenericType& key, bool added, int64_t elemId, int64_t replaceId) {
...
PyDynamicBasketInputProxy* PyDynamicBasketInputProxy::create(PyNode* node, INOUT_ID_TYPE id) {
    ...
    PyDynamicBasketInputProxy* proxy =
        (PyDynamicBasketInputProxy*)PyType.tp_new(&PyType, nullptr, nullptr);
    new (proxy) PyDynamicBasketInputProxy(node, id, shape.get());
    return proxy;
}

void PyDynamicBasketInputProxy::handleShapeChange(const DialectGenericType& key, bool added,
                                                  int64_t elemId, int64_t replaceId) {
    ...
    if (added) {
        ...
        // force resize down of last element.  There is no C-API to do this for some reason.. but
        // this is what the python C code does internally on pop ( essentially )
