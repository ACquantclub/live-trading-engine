...
PyListBasketOutputProxy::PyListBasketOutputProxy(PyObject* pyType, Node* node, INOUT_ID_TYPE id,
                                                 size_t shape)
...
PyListBasketOutputProxy* PyListBasketOutputProxy::create(PyObject* pyType, Node* node,
                                                         INOUT_ID_TYPE id, size_t shape) {
    if (shape > OutputId::maxBasketElements())
        CSP_THROW(ValueError, "List basket size of " << shape << " exceeds basket size limit of "
                                                     << OutputId::maxBasketElements() << " in node "
                                                     << node->name());

    PyListBasketOutputProxy* proxy =
        (PyListBasketOutputProxy*)PyType.tp_new(&PyType, nullptr, nullptr);
...
static PyObject* PyListBasketOutputProxy_output(PyListBasketOutputProxy* proxy, PyObject* obj) {
    CSP_BEGIN_METHOD;
    if (!PyDict_Check(obj))
        CSP_THROW(TypeError, "output called on list basket output proxy with non dict object: "
                                 << PyObjectPtr::incref(obj));

    PyObject *key, *value;
    Py_ssize_t pos = 0;
    while (PyDict_Next(obj, &pos, &key, &value)) {
        if (!PyIndex_Check(key))
            CSP_THROW(TypeError, "output called on list basket with non-index key: "
...
PyDictBasketOutputProxy* PyDictBasketOutputProxy::create(PyObject* pyType, Node* node,
                                                         INOUT_ID_TYPE id, PyObject* shape) {
    ...
    PyDictBasketOutputProxy* proxy =
        (PyDictBasketOutputProxy*)PyType.tp_new(&PyType, nullptr, nullptr);
...
PyOutputProxy* PyDictBasketOutputProxy::proxyByKey(PyObject* key) {
    PyOutputProxy* out_proxy =
        static_cast<PyOutputProxy*>(PyDict_GetItem(m_proxyMapping.ptr(), key));
    if (!out_proxy)
        CSP_THROW(KeyError,
                  "key " << PyObjectPtr::incref(key) << " is not a member of the dict basket");
    return out_proxy;
}
...
static PyObject* PyDictBasketOutputProxy_output(PyDictBasketOutputProxy* proxy, PyObject* obj) {
    CSP_BEGIN_METHOD;
    if (!PyDict_Check(obj))
        CSP_THROW(TypeError, "output called on dict basket output proxy with non dict object: "
                                 << PyObjectPtr::incref(obj));
...
PyDynamicBasketOutputProxy* PyDynamicBasketOutputProxy::create(PyObject* pyType, Node* node,
                                                               INOUT_ID_TYPE id) {
    PyObjectPtr shape = PyObjectPtr::own(PyList_New(0));
    PyDynamicBasketOutputProxy* proxy =
        (PyDynamicBasketOutputProxy*)PyType.tp_new(&PyType, nullptr, nullptr);
...
PyOutputProxy* PyDynamicBasketOutputProxy::getOrCreateProxy(PyObject* key) {
    ...
    if (outproxy == nullptr) {
        ...
        auto proxy = PyOutputProxyPtr::own(
            PyOutputProxy::create(m_elemType.get(), m_node, OutputId(m_id, elemId)));
...
void PyDynamicBasketOutputProxy::removeProxy(PyObject* key) {
    PyOutputProxy* outproxy =
        static_cast<PyOutputProxy*>(PyDict_GetItem(m_proxyMapping.ptr(), key));
    if (outproxy == nullptr)
        CSP_THROW(KeyError, "attempting to remove unknown key " << PyObjectPtr::incref(key)
                                                                << " from dynamic basket");

    // Disallow removing timeseries that ticked in the same cycle
    if (outproxy->ts()->lastCycleCount() == m_node->cycleCount())
        CSP_THROW(RuntimeException, "Attempted to delete dynamic basket key '"
                                        << PyObjectPtr::incref(key)
...
static PyObject* PyDynamicBasketOutputProxy_output(PyDynamicBasketOutputProxy* proxy,
                                                   PyObject* obj) {
    CSP_BEGIN_METHOD;
    if (!PyDict_Check(obj))
        CSP_THROW(TypeError, "output called on dict basket output proxy with non dict object: "
