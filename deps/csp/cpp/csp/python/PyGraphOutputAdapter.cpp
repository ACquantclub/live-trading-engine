...
void PyGraphOutputAdapter::processResults() {
    ...
    if (static_cast<PythonEngine*>(rootEngine())->outputNumpy()) {
        auto res = valuesAtIndexToNumpy(ValueType::TIMESTAMP_VALUE_TUPLE, ts, len - 1, 0,
                                        autogen::TimeIndexPolicy::INCLUSIVE,
                                        autogen::TimeIndexPolicy::INCLUSIVE);
        m_result = PyObjectPtr::check(res);
    } else {
        m_result = PyObjectPtr::check(PyList_New(len));
        switchCspType(ts->type(), [this, ts, len](auto tag) {
            Py_ssize_t idx = 0;
...
static OutputAdapter* creator(csp::AdapterManager* manager, PyEngine* pyengine, PyObject* args) {
    ...
    auto adapter = std::make_shared<PyGraphOutputAdapter>(engine, tickCount,
                                                          fromPython<TimeDelta>(pyTickHistory));
    engine->registerGraphOutput(key, adapter);
    return adapter.get();
}

// for returning __outputs__ from a graph ( as opposed to csp.add_graph_output calls )
static OutputAdapter* return_creator(csp::AdapterManager* manager, PyEngine* pyengine,
                                     PyObject* args) {
    PyObject* pyKey = nullptr;
    ...
    if (engine->isRootEngine()) {
        PyObjectPtr argsEx =
            PyObjectPtr::check(Py_BuildValue("OiN", pyKey, -1, toPython(TimeDelta::NONE())));
    ...
    // for graph outputs keys should always be strings, unnamed single should be "".  For historical
    // reasons unnamed is passed as 0
    std::string key;
