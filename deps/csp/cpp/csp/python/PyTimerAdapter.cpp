...
static InputAdapter* timer_creator(csp::AdapterManager* manager, PyEngine* pyengine,
    ...
    return switchCspType(cspType,
                         [engine = pyengine->engine(), &cspType, interval, pyValue,
                          allowDeviation](auto tag) -> InputAdapter* {
                             using T = typename decltype(tag)::type;
                             return engine->createOwnedObject<TimerInputAdapter<T>>(
                                 cspType, interval, fromPython<T>(pyValue, *cspType),
                                 bool(allowDeviation));
                         });
}
