...
static PyObject* PyNodeWrapper_linkFrom(PyNodeWrapper* self, PyObject* args) {
    ...
    PyObject* source;
    if (!PyArg_ParseTuple(args, "Oiiii", &source, &outputIdx, &outputBasketIdx, &inputIdx,
                          &inputBasketIdx))
        return nullptr;
    ...
    if (PyType_IsSubtype(Py_TYPE(source), &PyNodeWrapper::PyType)) {
        ...
        if (self->node()->isInputBasket(inputIdx) &&
            self->node()->inputBasket(inputIdx)->isDynamicBasket()) {
            CSP_ASSERT(sourceNode->outputBasket(outputIdx)->isDynamicBasket());

            auto* outputBasket = sourceNode->outputBasket(outputIdx);
            static_cast<DynamicOutputBasketInfo*>(outputBasket)
                ->linkInputBasket(self->node(), inputIdx);
        } else
        ...
        CSP_THROW(TypeError, "link_from expected PyNode or PyInputAdapter as source, got "
        ...
        if (shape == Py_None) {
            ...
            if (PyLong_Check(shape))
                ...
                if (!PyList_Check(shape))
                    CSP_THROW(TypeError, "Expected basket shape as int or list, got "
...
static PyMethodDef PyNodeWrapper_methods[] = {
    {"link_from", (PyCFunction)PyNodeWrapper_linkFrom, METH_VARARGS,
     "links node's output to target's input"},
    {"create_alarm", (PyCFunction)PyNodeWrapper_createAlarm, METH_VARARGS,
     "create a single alarm input"},
