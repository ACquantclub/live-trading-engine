...
class PythonPassthrough : public csp::Exception {
  public:
    PythonPassthrough(const char* exType, const std::string& r, const char* file, const char* func,
                      int line)
        ...
        // We need to hold the reference since PyErr_Restore takes back a reference to each of its
        // arguments
        PyErr_Fetch(&m_type, &m_value, &m_traceback);
    }

    PythonPassthrough(PyObject* pyException) : csp::Exception("", "") {
        // Note: all of these methods return strong references, so we own them like in the other
        // constructor
...
#define CSP_CATCH_HELPER(EXC_TYPE, PYEXC_TYPE, RETURN_STMT)                                 \
        ...
        PyErr_SetString(PYEXC_TYPE,                                                         \
                        err.full(csp::python::capture_cpp_exception_trace_flag()).c_str()); \
        RETURN_STMT                                                                         \
    }
