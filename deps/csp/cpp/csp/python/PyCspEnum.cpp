...
DialectCspEnumMeta::DialectCspEnumMeta(PyTypeObjectPtr pyType, const std::string& name,
    ...
    : CspEnumMeta(name, def), m_pyType(pyType) {
}
...

    {
        ...
        while (PyDict_Next(metadata, &pos, &key, &value)) {
            ...
            if (!PyLong_Check(value))
                CSP_THROW(TypeError, "csp.Enum key " << keystr << " expected an integer got "
                                                     << PyObjectPtr::incref(value));
...
PyObject* PyCspEnumMeta_subscript(PyCspEnumMeta* self, PyObject* key) {
    ...
    if (!obj)
        CSP_THROW(ValueError, PyObjectPtr::incref(key) << " is not a valid name on csp.enum type "
...
PyObject* PyCspEnum_new(PyTypeObject* type, PyObject* args, PyObject* kwds) {
    ...
    if (!obj)
        CSP_THROW(ValueError, PyObjectPtr::incref(pyvalue)
                                  << " is not a valid value on csp.enum type " << type->tp_name);
