    ...
    if (PyTuple_Check(pytype)) {
        ...
        if (PyLong_Check(pyshape)) {
            ...
            if (longShape > InputId::maxBasketElements())
                CSP_THROW(ValueError, "basket size exceeds limit of "
                                          << InputId::maxBasketElements() << " on node \""
                                          << nodeName << "\"");
...
PyObject* pycppnode_create(PyObject* args, CppNode::Creator creatorFn) {
    ...
    PyObject* pyoutputs;
    PyObject* pyscalars;
    if (!PyArg_ParseTuple(args, "sO!O!O!O!", &nodeName, &csp::python::PyEngine::PyType, &pyengine,
                          &PyTuple_Type, &pyinputs, &PyTuple_Type, &pyoutputs, &PyDict_Type,
                          &pyscalars))
    ...
    for (int i = 0; i < PyTuple_GET_SIZE(pyinputs); ++i) {
        int isAlarm;
        if (!PyArg_ParseTuple(PyTuple_GET_ITEM(pyinputs, i), "sOip", &name, &pytype, &index,
                              &isAlarm))
            CSP_THROW(PythonPassthrough, "");

        CppNode::Shape shape = extractShape(nodeName, pytype);

        nodedef.inputs[name] = {INOUT_ID_TYPE(index), pyTypeAsCspType(pytype), (bool)isAlarm,
                                shape};
    }
