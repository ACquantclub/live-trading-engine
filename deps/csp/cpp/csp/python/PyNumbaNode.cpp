...
PyNumbaNode::PyNumbaNode(csp::Engine* engine, void* stateObject, CallbackType numbaInitCallback,
                         CallbackType numbaImplCallback, PyObjectPtr inputs, PyObjectPtr outputs,
    ...
    DEBUG_PRINT_WITH_LOC("Created numba node: "
                         << (int64_t)(this) << " stateObject=" << (int64_t)(stateObject)
        ...
        if (PyTuple_Check(inputType)) {
            ...
            if (PyLong_Check(shape))
                ...
                if (!PyList_Check(shape))
                    CSP_THROW(TypeError, "Expected basket type as int or list, got "
...
PyNumbaNode* PyNumbaNode::create(PyEngine* pyengine, PyObject* inputs, PyObject* outputs,
                                 PyObject* stateObject, PyObject* numbaInitCallback,
                                 PyObject* numbaImplCallback, PyObject* dataReference) {
    ...
    CallbackType numbaInitCallbackPtr =
        reinterpret_cast<CallbackType>(fromPython<int64_t>(numbaInitCallback));
    CallbackType numbaImplCallbackPtr =
        reinterpret_cast<CallbackType>(fromPython<int64_t>(numbaImplCallback));
...
static PyObject* PyNumbaNode_create(PyObject* module, PyObject* args) {
    ...
    if (!PyArg_ParseTuple(args, "O!O!O!O!O!O!O!", &PyEngine::PyType, &engine, &PyTuple_Type,
                          &inputs, &PyTuple_Type, &outputs, &PyLong_Type, &state, &PyLong_Type,
                          &initCallback, &PyLong_Type, &implCallback, &PyTuple_Type,
                          &dataReference))
        CSP_THROW(PythonPassthrough, "");

    auto node = PyNumbaNode::create(engine, inputs, outputs, state, initCallback, implCallback,
                                    dataReference);
...
inline csp::TimeSeriesProvider* getNonConstInput(csp::python::PyNumbaNode* node,
                                                 int32_t input_index) {
...
int64_t __csp_create_datetime_nanoseconds__(int32_t year, int32_t month, int32_t day, int32_t hour,
                                            int32_t minute, int32_t second, int32_t nanosecond) {
