...
static OutputAdapter* output_creator(csp::AdapterManager* manager, PyEngine* pyengine,
                                     PyObject* args) {
    PyObject* pyType;
    ...
    return switchCspType(cspType, [pyengine, pyFeedbackInput](auto tag) -> OutputAdapter* {
...
static InputAdapter* input_creator(csp::AdapterManager* manager, PyEngine* engine, PyObject* pyType,
    ...
    auto& cspType = pyTypeAsCspType(pyType);
    return switchCspType(cspType.get(), [engine, &cspType, pushMode](auto tag) -> InputAdapter* {
        using T = typename decltype(tag)::type;
