...
class PyManagedSimInputAdapter : public ManagedSimInputAdapter {
  public:
    PyManagedSimInputAdapter(Engine* engine, AdapterManager* manager, PyObjectPtr pyadapter,
...
class TypedPyManagedSimInputAdapter : public PyManagedSimInputAdapter {
  public:
    TypedPyManagedSimInputAdapter(Engine* engine, AdapterManager* manager, PyObjectPtr pyadapter,
                                  PyObject* pyType, PushMode pushMode)
...
static PyMethodDef PyManagedSimInputAdapter_PyObject_methods[] = {
    {"push_tick", (PyCFunction)PyManagedSimInputAdapter_PyObject::pushTick, METH_O,
     "push new tick"},
    {NULL}};
...
static InputAdapter* pymanagedsimadapter_creator(csp::AdapterManager* manager, PyEngine* engine,
    ...
    if (!PyType_IsSubtype(pyAdapterType, &PyManagedSimInputAdapter_PyObject::PyType))
        CSP_THROW(TypeError,
                  "Expected PyManagedSimInputAdapter derived type, got " << pyAdapterType->tp_name);

    PyManagedSimInputAdapter_PyObject* pyAdapter =
        (PyManagedSimInputAdapter_PyObject*)PyObject_Call((PyObject*)pyAdapterType, adapterArgs,
                                                          nullptr);
    if (!pyAdapter)
        CSP_THROW(PythonPassthrough, "");
    ...
    switchPyType(pyType, [&](auto tag) {
        pyAdapter->adapter =
            engine->engine()
                ->createOwnedObject<TypedPyManagedSimInputAdapter<typename decltype(tag)::type>>(
