// this is included first so that we do include without NO_IMPORT_ARRAY defined, which is done in
// NumpyConversions.h
...
PyObject* valuesAtIndexToNumpy(ValueType valueType, const csp::TimeSeriesProvider* ts,
    ...
    return switchCspType(ts->type(), [valueType, ts, startIndex, endIndex, startPolicy, endPolicy,
                                      startDt, endDt](auto tag) {
    ...
    switch (base) {
        ...
        default:
            CSP_THROW(csp::NotImplemented,
                      "datetime resolution not supported or invalid - saw NPY_DATETIMEUNIT value "
                          << base);
    ...
    switch (numpy_type_char) {
        case NPY_BOOLLTR:
            if (cspType != csp::CspType::Type::BOOL)
                CSP_THROW(ValueError,
                          "numpy type " << numpy_type_char << " requires bool output type");
            break;
        case NPY_BYTELTR:
        case NPY_UBYTELTR:
        case NPY_SHORTLTR:
        ...
        case NPY_LONGLTR:
            if (cspType != csp::CspType::Type::INT64)
                CSP_THROW(ValueError,
                          "numpy type " << numpy_type_char << " requires int output type");
            break;
        case NPY_ULONGLTR:
        ...
        case NPY_HALFLTR:
            CSP_THROW(ValueError,
                      "numpy type " << numpy_type_char << " (numpy half float) not supported");
        case NPY_FLOATLTR:
        case NPY_DOUBLELTR:
            if (cspType != csp::CspType::Type::DOUBLE)
                CSP_THROW(ValueError,
                          "numpy type " << numpy_type_char << " requires float output type");
            break;
        case NPY_LONGDOUBLELTR:
            CSP_THROW(ValueError,
                      "numpy type " << numpy_type_char << " (long double) not supported");
        case NPY_CFLOATLTR:
        case NPY_CDOUBLELTR:
        ...
        case NPY_CHARLTR:
            if (cspType != csp::CspType::Type::STRING)
                CSP_THROW(ValueError,
                          "numpy type " << numpy_type_char << " requires string output type");
            break;
        case NPY_VOIDLTR:
        ...
        case NPY_DATETIMELTR:
            if (cspType != csp::CspType::Type::DATETIME)
                CSP_THROW(ValueError,
                          "numpy type " << numpy_type_char << " requires datetime output type");
            break;
        case NPY_TIMEDELTALTR:
            if (cspType != csp::CspType::Type::TIMEDELTA)
                CSP_THROW(ValueError,
                          "numpy type " << numpy_type_char << " requires timedelta output type");
            break;
        default:
