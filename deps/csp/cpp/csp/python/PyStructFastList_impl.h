...
// The Py_TPFLAGS_SEQUENCE flag is used for 'match' statement to work. It is not available prior
// Python 3.10, so need to use conditional compilation
...
template <typename StorageT>
static PyObject* PyStructFastList_representation_helper(PyStructFastList<StorageT>* self,
                                                        bool show_unset) {
...
template <typename StorageT>
static PyObject* PyStructFastList_Copy(PyStructFastList<StorageT>* self,
...
template <typename StorageT>
static PyObject* PyStructFastList_Reverse(PyStructFastList<StorageT>* self,
                                          PyObject* Py_UNUSED(ignored)) {
    CSP_BEGIN_METHOD;
...
template <typename StorageT>
static PyObject* PyStructFastList_Sort(PyStructFastList<StorageT>* self, PyObject* args,
                                       PyObject* kwargs) {
    CSP_BEGIN_METHOD;
    ...
    // We could have implemented sorting using VectorWrapper method, however, decided not to because
    // of:
    // - When the 'key' argument is present, fuguring out C++ sort is too complex, so still need to
    // fall back to Python method in this case.
    // - In order to implement sort using VectorWrapper method, we would need to implement
    // comparison ops for all CSP types, even dummy ones for types like DialectGeneric.
...
template <typename StorageT>
static PyObject* PyStructFastList_Clear(PyStructFastList<StorageT>* self,
                                        PyObject* Py_UNUSED(ignored)) {
    CSP_BEGIN_METHOD;
...
static int py_struct_fast_list_ass_subscript(PyObject* sself, PyObject* item, PyObject* value) {
    ...
    if (value != NULL) {
        ...
        std::vector<StorageT> v = FromPython<std::vector<StorageT>>::impl(
            value, ((PyStructFastList<StorageT>*)sself)->arrayType);
...
static PyObject* py_struct_fast_list_inplace_concat(PyObject* sself, PyObject* other) {
    ...
    std::vector<StorageT> v = FromPython<std::vector<StorageT>>::impl(
        other, ((PyStructFastList<StorageT>*)sself)->arrayType);
...
static PyObject* py_struct_fast_list_concat(PyObject* sself, PyObject* other) {
    ...
    if ((!PyList_Check(other)) && (Py_TYPE(other) != &PyStructFastList<StorageT>::PyType)) {
        PyErr_SetString(PyExc_TypeError,
                        "can only concatenate typed list or _cspimpl.PyStructFastList to "
                        "_cspimpl.PyStructFastList with the same type");
        return NULL;
    }

    // Convert the operands to Python lists and use Python list concatenation
    PyObjectPtr list =
        PyObjectPtr::own(toPython(((PyStructFastList<StorageT>*)sself)->vector.getVector(),
                                  ((PyStructFastList<StorageT>*)sself)->arrayType));
    PyObjectPtr other_list = PyObjectPtr::incref(other);

    if (!PyList_Check(other))
        other_list =
            PyObjectPtr::own(toPython(((PyStructFastList<StorageT>*)other)->vector.getVector(),
...
static PyObject* py_struct_fast_list_richcompare(PyObject* sself, PyObject* other, int op) {
    ...
    // We could have implemented rich comparison using VectorWrapper, however, decided not to
    // because of:
    // - In order to implement rich comparison using VectorWrapper, we would need to implement
    // comparison ops for all CSP types, even dummy ones for types like DialectGeneric.
...
template <typename StorageT>
static PyObject* PyStructFastList_Sizeof(PyStructFastList<StorageT>* self,
                                         PyObject* Py_UNUSED(ignored)) {
    CSP_BEGIN_METHOD;
...
template <typename StorageT>
static PyObject* PyStructFastList_Reversed(PyStructFastList<StorageT>* self,
                                           PyObject* Py_UNUSED(ignored)) {
    CSP_BEGIN_METHOD;
...
template <typename StorageT>
static PyObject* PyStructFastList_reduce(PyStructFastList<StorageT>* self,
                                         PyObject* Py_UNUSED(ignored)) {
    CSP_BEGIN_METHOD;
...
static PyObject* PyStructFastList_new(PyTypeObject* type, PyObject* args, PyObject* kwds) {
    // Since the PyStructFastList has no real meaning when created from Python, we can reconstruct
    // the FL's value by just treating it as a list. Thus, we simply override the tp_new behaviour
    // to return a list object here. Again, since we don't have tp_init for the FL, we need to rely
    // on the Python list's tp_init function.

    return PyObject_Call((PyObject*)&PyList_Type, args,
                         kwds);  // Calls both tp_new and tp_init for a Python list
}
...
template <typename StorageT>
static int PyStructFastList_tp_traverse(PyStructFastList<StorageT>* self, visitproc visit,
                                        void* arg) {
...
struct PyStructFastListReverseIterator : public PyObject {
    PyStructFastListReverseIterator(PyStructFastList<StorageT>* v)
        : m_v(v), m_index(py_struct_fast_list_len<StorageT>(v) - 1) {
        Py_INCREF(m_v);
    }
