...
class NumPyIterator {
    ...
    // Iterates over the array in a non-necessarily contiguous order, so all operations must be
    // defined only element-wise
  ...
  public:
    ...
    const npy_intp* indices() const {
        return (npy_intp*)m_current.data();
    }
  ...
  private:
    void verify_arr(PyArrayObject* arr) {
        ...
        if (unlikely(PyObject_RichCompareBool((PyObject*)PyArray_DESCR(arr), (PyObject*)expType,
                                              Py_EQ)) != 1) {
...
struct PyShape {
    ...
    PyShape(PyArrayObject* arr) {
        ...
        if (nd == 0)
            ...
            m_n = std::accumulate(std::begin(m_dims), std::end(m_dims), 1,
    ...
    void validateShape(PyArrayObject* arr) {
        ...
        if (nd != m_dims.size())
            CSP_THROW(ValueError,
                      "Inconsistent shape of NumPy arrays for computation: different number of "
                      "dimensions");
        ...
        for (size_t i = 0; i < nd; i++) {
            if (dims[i] != m_dims[i])
                CSP_THROW(
                    ValueError,
                    "Inconsistent shape of NumPy arrays for computation: difference in dimension "
                        << i);
        }
    }
...
class _np_tick_window_updates
    : public _generic_tick_window_updates<PyObjectPtr, _np_tick_window_updates> {
  public:
    using _generic_tick_window_updates<PyObjectPtr,
                                       _np_tick_window_updates>::_generic_tick_window_updates;
    ...
    inline void checkValid() {
        if (!csp.valid(this->x))
            CSP_THROW(
                ValueError,
                "Error: sampler called on a NumPy array before any data ticks - shape is unknown.");
    }
};
...
class _np_time_window_updates
    : public _generic_time_window_updates<PyObjectPtr, _np_time_window_updates> {
  public:
    using _generic_time_window_updates<PyObjectPtr,
                                       _np_time_window_updates>::_generic_time_window_updates;
...
class _cross_sectional_as_np
    : public _generic_cross_sectional<double, PyObjectPtr, _cross_sectional_as_np> {
  public:
    using _generic_cross_sectional<double, PyObjectPtr,
                                   _cross_sectional_as_np>::_generic_cross_sectional;
...
class _np_cross_sectional_as_list
    : public _generic_cross_sectional<PyObjectPtr, std::vector<PyObjectPtr>,
                                      _np_cross_sectional_as_list> {
  public:
...
class _np_cross_sectional_as_np
    : public _generic_cross_sectional<PyObjectPtr, PyObjectPtr, _np_cross_sectional_as_np> {
  public:
    using _generic_cross_sectional<PyObjectPtr, PyObjectPtr,
                                   _np_cross_sectional_as_np>::_generic_cross_sectional;
...
DECLARE_CPPNODE(_np_to_list) {
    ...
    INVOKE() {
        ...
        if (nd != 1)
            CSP_THROW(ValueError,
                      "Cannot convert NumPy array of more than one dimension into listbasket");

        int64_t n_arr = PyArray_SIZE(arr);
        if (n_arr != n)
            CSP_THROW(ValueError, "Cannot convert NumPy array of size "
                                      << n_arr << " into listbasket of " << n << " elements");
  ...
  protected:
    ...
    INVOKE() {
        ...
        for (NumPyIterator itx = {xval}, ity = {yval}; itx && ity; ++itx, ++ity) {
            if (isnan(ity.value()) &&
                !isnan(itx.value()))  // need to set corresponding element in x as nan
            {
                if (!xsync) {
            ...
            } else if (isnan(itx.value()) &&
                       !isnan(ity.value()))  // need to set corresponding element in y as nan
            {
                if (!ysync) {
...
class _npComputeEMA : public _np_compute<C> {
  public:
    ...
    DataValidator<C> initDataValidator() override {
        return DataValidator<C>(this->min_data_points, true, alpha, this->ignore_na, horizon,
                                adjust);
    }
};
...
EXPORT_TEMPLATE_CPPNODE(_np_weighted_var,
                        SINGLE_ARG(_npBivarComputeOneArg<int64_t, WeightedVariance>));
EXPORT_TEMPLATE_CPPNODE(_np_covar, SINGLE_ARG(_npBivarComputeOneArg<int64_t, Covariance>));
EXPORT_TEMPLATE_CPPNODE(_np_weighted_sem,
                        SINGLE_ARG(_npBivarComputeOneArg<int64_t, WeightedStandardError>));
EXPORT_TEMPLATE_CPPNODE(_np_weighted_skew, SINGLE_ARG(_npBivarComputeOneArg<bool, WeightedSkew>));
EXPORT_TEMPLATE_CPPNODE(_np_weighted_kurt,
                        SINGLE_ARG(_npBivarComputeTwoArg<bool, WeightedKurtosis>));

// Other node templates

/*
@csp.node
def _np_quantile(additions: ts[[np.ndarray]], removals: ts[[np.ndarray]], quants:
typing.List[float], nq: int, interpolation_type: int, trigger: ts[object]):
    ...
    INVOKE() {
        ...
        if (csp.ticked(additions)) {
            if (unlikely(s_first)) {
                ...
                s_elem.reserve(s_shp.m_n);
                for (int64_t j = 0; j < s_shp.m_n; j++) {
                    s_elem.emplace_back(DataValidator<Quantile>(min_data_points, ignore_na, quants,
                                                                interpolation_type));
                }
                s_first = false;
            }
    ...
    INVOKE() {
        ...
        if (csp.ticked(sampler) && csp.ticked(x)) {
            ...
            if (unlikely(s_first)) {
                ...
                s_elem.reserve(s_shp.m_n);
                for (int64_t j = 0; j < s_shp.m_n; j++) {
                    s_elem.emplace_back(DataValidator<C>(min_data_points, true, halflife,
                                                         now() - TimeDelta::fromMicroseconds(1)));
                }
                s_first = false;
            }
            // Add upon iteration
        ...
        if (csp.ticked(additions)) {
            ...
            if (unlikely(s_first)) {
                ...
                if (PyArray_NDIM((PyArrayObject*)arr) != 1)
                    CSP_THROW(ValueError,
                              "Covariance or correlation matrix called on an array of more than 1 "
                              "dimension: undefined");
                // Special setup
        ...
        if (csp.ticked(x_add)) {
            ...
            if (unlikely(s_first)) {
                ...
                if (PyArray_NDIM((PyArrayObject*)arr) != 1)
                    CSP_THROW(
                        ValueError,
                        "Covariance matrix called on an array of more than 1 dimension: undefined");
                // Special setup
                int64_t n = PyArray_Size(arr);
                s_shp = PyShape({n, n}, n);
                s_elem.reserve(n * n);
    ...
    INVOKE() {
        ...
        if (csp.ticked(x_add)) {
            ...
            for (size_t i = 0; i < m; i++) {
                ...
                for (NumPyIterator x_iter = {x}, y_iter = {y}, w_iter = {w};
                     x_iter && y_iter && w_iter; ++x_iter, ++w_iter, ++y_iter)
    ...
    INVOKE() {
        ...
        if (csp.ticked(x) && csp.ticked(sampler)) {
            ...
            if (unlikely(s_first)) {
                ...
                for (int64_t j = 0; j < s_shp.m_n; j++) {
                    s_elem.emplace_back(
                        DataValidator<ArgMinMax>(min_data_points, ignore_na, max, recent));
                }
                s_first = false;
            }
            for (NumPyIterator iter = {arr}; iter; ++iter)
        ...
        if (csp.ticked(trigger)) {
            ...
            PyObject* out = PyArray_NewFromDescr(&PyArray_Type, descr, s_shp.m_dims.size(),
                                                 &s_shp.m_dims[0], NULL, values, 0, NULL);
