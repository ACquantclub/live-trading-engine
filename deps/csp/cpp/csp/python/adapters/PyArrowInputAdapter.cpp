...
static InputAdapter* record_batch_input_adapter_creator(csp::AdapterManager* manager,
    ...
    return pyengine->engine()->createOwnedObject<csp::python::arrow::RecordBatchInputAdapter>(
        cspType, std::move(schema), std::move(tsColName), std::move(source),
        expectSmallBatches != 0);
}

REGISTER_INPUT_ADAPTER(_record_batch_input_adapter_creator, record_batch_input_adapter_creator);

static PyModuleDef _arrowadapterimpl_module = {PyModuleDef_HEAD_INIT,
                                               "_arrowadapterimpl",
