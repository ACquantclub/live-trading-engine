...
class RecordBatchIterator {
  public:
    ...
    RecordBatchIterator(PyObjectPtr iter, std::shared_ptr<::arrow::Schema> schema)
        : m_iter(std::move(iter)), m_schema(schema) {
    }
    ...
    std::shared_ptr<::arrow::RecordBatch> next() {
        ...
        if (!PyTuple_Check(py_tuple.get()))
            CSP_THROW(csp::TypeError,
                      "Invalid arrow data, expected tuple (using the PyCapsule C interface) got "
                          << Py_TYPE(py_tuple.get())->tp_name);

        auto num_elems = PyTuple_Size(py_tuple.get());
        if (num_elems != 2)
            CSP_THROW(csp::TypeError,
                      "Invalid arrow data, expected tuple (using the PyCapsule C interface) with 2 "
                      "elements got "
                          << num_elems);
        ...
        if (!PyCapsule_IsValid(py_array, "arrow_array"))
            CSP_THROW(csp::TypeError,
                      "Invalid arrow data, expected tuple from the PyCapsule C interface ");

        ArrowArray* c_array =
            reinterpret_cast<ArrowArray*>(PyCapsule_GetPointer(py_array, "arrow_array"));
...
class RecordBatchInputAdapter : public PullInputAdapter<std::vector<DialectGenericType>> {
  public:
    RecordBatchInputAdapter(Engine* engine, CspTypePtr& type, PyObjectPtr pySchema,
                            std::string tsColName, PyObjectPtr source, bool expectSmallBatches)
        ...
        ArrowSchema* c_schema =
            reinterpret_cast<ArrowSchema*>(PyCapsule_GetPointer(pySchema.get(), "arrow_schema"));
        auto result = ::arrow::ImportSchema(c_schema);
        if (!result.ok())
            CSP_THROW(
                ValueError,
                "Failed to load schema for record batches through the PyCapsule C Data interface: "
                    << result.status().ToString());
    ...
    int64_t findFirstMatchingIndex() {
        ...
        if (m_expectSmallBatches) {
            auto predicate = [this](std::optional<int64_t> new_time) -> bool {
                return new_time.value() >= this->m_startTime;
            };
    ...
    int64_t findNextLargerTimestampIndex(int64_t start_idx) {
        ...
        ::arrow::TimestampArray::IteratorType it;
        if (m_expectSmallBatches) {
            auto predicate = [cur_time](std::optional<int64_t> new_time) -> bool {
                return cur_time != new_time.value();
            };
            it = std::find_if(begin, m_endIt, predicate);
        } else {
    ...
    std::shared_ptr<::arrow::RecordBatch> updateStateFromNextRecordBatch() {
        ...
        if (rb) {
            ...
            if (!array)
                CSP_THROW(ValueError, "Failed to get timestamp column " << m_tsColName
                                                                        << " from record batch "
                                                                        << rb->ToString());
    ...
    void start(DateTime start, DateTime end) override {
        ...
        m_startTime = (start_nanos % m_multiplier == 0) ? start_nanos / m_multiplier
    ...
    DialectGenericType convertRecordBatchToPython(std::shared_ptr<::arrow::RecordBatch> rb) {
        ...
        auto py_schema = csp::python::PyObjectPtr::own(
            PyCapsule_New(rb_schema, "arrow_schema", ReleaseArrowSchemaPyCapsule));
        auto py_array = csp::python::PyObjectPtr::own(
            PyCapsule_New(rb_array, "arrow_array", ReleaseArrowArrayPyCapsule));
        auto py_tuple =
            csp::python::PyObjectPtr::own(PyTuple_Pack(2, py_schema.get(), py_array.get()));
