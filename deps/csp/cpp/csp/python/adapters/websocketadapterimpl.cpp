...
// AdapterManager
csp::AdapterManager* create_websocket_adapter_manager(PyEngine* engine,
                                                      const Dictionary& properties) {
    return engine->engine()->createOwnedObject<ClientAdapterManager>(properties);
}

static InputAdapter* create_websocket_input_adapter(csp::AdapterManager* manager,
    ...
    return websocketManager->getInputAdapter(cspType, pushMode,
                                             fromPython<Dictionary>(pyProperties));
}

static OutputAdapter* create_websocket_output_adapter(csp::AdapterManager* manager,
...
static OutputAdapter* create_websocket_header_update_adapter(csp::AdapterManager* manager,
                                                             PyEngine* pyengine, PyObject* args) {
    auto* websocketManager = dynamic_cast<ClientAdapterManager*>(manager);
    if (!websocketManager)
        CSP_THROW(TypeError, "Expected WebsocketClientAdapterManager");
