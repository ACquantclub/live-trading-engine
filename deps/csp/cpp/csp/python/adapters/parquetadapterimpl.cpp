...
DECLARE_CPPNODE(parquet_dict_basket_writer) {
    ...
    INIT_CPPNODE(parquet_dict_basket_writer) {
        ...
        s_outputWriter = outputAdapterManager->createDictOutputBasketWriter(
            column_name.value().c_str(), input.type());
    }
...
class FileNameGenerator : public csp::Generator<std::string, csp::DateTime, csp::DateTime> {
  public:
    ...
    void init(csp::DateTime start, csp::DateTime end) override {
        ...
        m_iter =
            csp::python::PyObjectPtr::check(PyObject_Call(m_wrappedGenerator.ptr(), tp, nullptr));
...
class ArrowTableGenerator
    : public csp::Generator<std::shared_ptr<arrow::Table>, csp::DateTime, csp::DateTime> {
  public:
    ...
    virtual bool next(std::shared_ptr<arrow::Table>& value) override {
        ...
        if (!PyCapsule_IsValid(nextValPtr.get(), "arrow_array_stream")) {
            CSP_THROW(csp::TypeError, "Invalid arrow data, expected PyCapsule got "
                                          << Py_TYPE(nextValPtr.get())->tp_name);
        }
        // Extract the record batch
        struct ArrowArrayStream* c_stream = reinterpret_cast<struct ArrowArrayStream*>(
            PyCapsule_GetPointer(nextValPtr.get(), "arrow_array_stream"));
        auto reader_result = arrow::ImportRecordBatchReader(c_stream);
        if (!reader_result.ok())
            CSP_THROW(csp::ValueError,
                      "Failed to load record batches through PyCapsule C Data interface: "
                          << reader_result.status().ToString());
        auto reader = std::move(reader_result.ValueUnsafe());
        auto table_result = arrow::Table::FromRecordBatchReader(reader.get());
        if (!table_result.ok())
            CSP_THROW(csp::ValueError, "Failed to load table from record batches "
  ...
  public:
    ...
    void writeItems(const csp::DialectGenericType& listObject) override {
        ...
        if (!PyArray_Check(object)) {
            CSP_THROW(csp::TypeError, "While writing to parquet expected numpy array type, got "
        ...
        if (PyArray_DESCR(arrayObject)->kind != m_expectedArrayDesc->kind) {
            CSP_THROW(
                ...
                "Expected array of type "
                    << csp::python::PyObjectPtr::own(PyObject_Repr((PyObject*)m_expectedArrayDesc))
                    << " got "
        ...
        CSP_TRUE_OR_THROW_RUNTIME(
            ndim == 1,
            "While writing to parquet expected numpy array with 1 dimension" << " got " << ndim);
        switch (npy_type) {
            case NPY_BYTELTR:
  ...
  private:
    ...
    void writeValues(PyArrayObject* arrayObject) {
        ...
        if (PyArray_ISCARRAY_RO(arrayObject)) {
            ...
            for (decltype(arraySize) i = 0; i < arraySize; ++i) {
                this->writeValue(static_cast<CspCType>(
                    *reinterpret_cast<NumpyCType*>(PyArray_GETPTR1(arrayObject, i))));
            }
        }
    }
    ...
    void writeItems(const csp::DialectGenericType& listObject) override {
        ...
        if (PyArray_ISCARRAY_RO(arrayObject)) {
            ...
            for (decltype(arraySize) i = 0; i < arraySize; ++i) {
                ...
                std::string value =
                    converter.to_bytes(reinterpret_cast<char32_t*>(elementPtr),
...
static inline DialectGenericListWriterInterface::Ptr create_numpy_array_writer_impl(
    const csp::CspTypePtr& type) {
    try {
        return csp::PartialSwitchCspType<csp::CspType::Type::DOUBLE, csp::CspType::Type::INT64,
            ...
            invoke(type.get(), [](auto tag) -> DialectGenericListWriterInterface::Ptr {
        ...
        CSP_THROW(csp::TypeError, "Unsupported array value type when writing to parquet:"
                                      << type->type().asString());
    }
}
...
class NumpyUnicodeReaderImpl final : public TypedDialectGenericListReaderInterface<std::string> {
  public:
    ...
    csp::DialectGenericType create(uint32_t size, uint32_t maxElementSize) override {
        ...
        PyArray_Descr* typ;
        PyObject* type_string_descr =
            csp::python::toPython(std::string("U") + std::to_string(maxElementSize));
    ...
    void setValue(const csp::DialectGenericType& list, int index,
                  const std::string& value) override {
        auto arrayObject = reinterpret_cast<PyArrayObject*>(csp::python::toPythonBorrowed(list));
        std::wstring_convert<std::codecvt_utf8<char32_t>, char32_t> converter;
        auto elementSize = PyArray_DESCR(arrayObject)->elsize;
        auto wideValue = converter.from_bytes(value);
        auto nElementsToCopy =
            std::min(int(elementSize / sizeof(char32_t)), int(wideValue.size() + 1));
        std::copy_n(wideValue.c_str(), nElementsToCopy,
                    reinterpret_cast<char32_t*>(PyArray_GETPTR1(arrayObject, index)));
    }
};

inline DialectGenericListReaderInterface::Ptr create_numpy_array_reader_impl(
    const csp::CspTypePtr& type)

{
    try {
        return csp::PartialSwitchCspType<csp::CspType::Type::DOUBLE, csp::CspType::Type::INT64,
                                         csp::CspType::Type::BOOL, csp::CspType::Type::STRING>::
            invoke(type.get(), [](auto tag) -> DialectGenericListReaderInterface::Ptr {
                ...
                if (numpy_dtype->type_num == NPY_UNICODE) {
                    ...
                    return std::make_shared<NumpyArrayReaderImpl<CValueType>>(numpy_dtype);
                }
            });
    } catch (csp::TypeError& e) {
        CSP_THROW(csp::TypeError, "Unsupported array value type when reading from parquet:"
                                      << type->type().asString());
    }
}
...
// AdapterManager
csp::AdapterManager* create_parquet_input_adapter_manager_impl(
    PyEngine* engine, const Dictionary& properties, FileNameGenerator::Ptr fileNameGenerator,
    ArrowTableGenerator::Ptr arrowTableGenerator) {
    auto res = engine->engine()->createOwnedObject<ParquetInputAdapterManager>(
        properties, fileNameGenerator, arrowTableGenerator);
    return res;
}

static InputAdapter* create_parquet_input_adapter(csp::AdapterManager* manager, PyEngine* pyengine,
                                                  PyObject* pyType, PushMode pushMode,
                                                  PyObject* args) {
    ...
    if (propertiesDict.get("is_array", false)) {
        auto&& valueType = pyTypeAsCspType(
            toPythonBorrowed(propertiesDict.get<DialectGenericType>("array_value_type")));
...
static OutputAdapter* create_parquet_output_adapter(csp::AdapterManager* manager,
                                                    PyEngine* pyengine, PyObject* args) {
    PyObject* pyProperties;
    PyObject* pyType;
...
static OutputAdapter* create_parquet_dict_basket_output_adapter(csp::AdapterManager* manager,
                                                                PyEngine* pyengine,
                                                                PyObject* args) {
    PyObject* pyProperties;
    PyObject* pyType;
...
static OutputAdapter* parquet_output_filename_adapter(csp::AdapterManager* manager,
                                                      PyEngine* pyengine, PyObject* args) {
    auto* parquetManager = dynamic_cast<ParquetOutputAdapterManager*>(manager);
    if (!parquetManager)
        CSP_THROW(TypeError, "Expected ParquetAdapterManager");
...
static PyObject* create_parquet_input_adapter_manager(PyObject* args) {
    ...
    if (!PyArg_ParseTuple(args, "O!O!O!", &PyEngine::PyType, &pyEngine, &PyDict_Type, &pyProperties,
    ...
    auto* adapterMgr = create_parquet_input_adapter_manager_impl(
        pyEngine, fromPython<Dictionary>(pyProperties), fileNameGenerator, arrowTableGenerator);
...
// AdapterManager
csp::AdapterManager* create_parquet_output_adapter_manager(PyEngine* engine,
    ...
    if (properties.tryGet("file_visitor", pyFilenameVisitorDG)) {
        ...
        fileVisitor = [pyFilenameVisitor](const std::string& filename) {
            PyObjectPtr rv = PyObjectPtr::own(PyObject_CallFunction(
                pyFilenameVisitor.get(), "O", PyObjectPtr::own(toPython(filename)).get()));
            if (!rv.get())
                CSP_THROW(PythonPassthrough, "");
        };
    }
    return engine->engine()->createOwnedObject<ParquetOutputAdapterManager>(properties,
                                                                            fileVisitor);
}

REGISTER_ADAPTER_MANAGER_CUSTOM_CREATOR(_parquet_input_adapter_manager,
                                        create_parquet_input_adapter_manager);
...
REGISTER_OUTPUT_ADAPTER(_parquet_dict_basket_output_adapter,
                        create_parquet_dict_basket_output_adapter);

REGISTER_OUTPUT_ADAPTER(_parquet_output_filename_adapter, parquet_output_filename_adapter);
static PyModuleDef _parquetadapterimpl_module = {PyModuleDef_HEAD_INIT,
                                                 "_parquetadapterimpl",
