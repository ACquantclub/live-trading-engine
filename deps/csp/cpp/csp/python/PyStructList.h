...
struct PyStructList : public PyObject {
    ...
    PyStructList(PyStruct* p, std::vector<StorageT>& v, const CspType& type)
        : pystruct(p), vector(VectorWrapper<StorageT>(v)), arrayType(type) {
        Py_INCREF(pystruct);
    }
    ...
    const CspType& arrayType;  // We require the type information of any non-primitive type, i.e.
                               // Struct or Enum, since they contain a meta
    static PyTypeObject PyType;
    static bool s_typeRegister;

    inline CspTypePtr elemType() const {
        return static_cast<const CspArrayType&>(arrayType).elemType();
    }
