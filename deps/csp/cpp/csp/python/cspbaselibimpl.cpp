...
DECLARE_CPPNODE(exprtk_impl) {
    class BaseValueContainer {
      public:
        ...
        virtual bool registerValue(exprtk::symbol_table<double>& expr,
    ...
    class ValueContainer : public BaseValueContainer {
      public:
        ...
        bool registerValue(exprtk::symbol_table<double>& expr,
      ...
      private:
        ...
        void registerValueImpl(exprtk::symbol_table<double>& symbolTable,
    ...
    class NumpyArrayValueContainer : public BaseValueContainer {
      public:
        ...
        void validateArray(PyArrayObject* arr) {
            ...
            if (dim != 1)
                CSP_THROW(ValueError, "csp.exprtk recieved an array of dim "
                                          << dim << " but can only take 1D arrays");
        ...
        void setValue(const TimeSeriesProvider* tsProvider) override {
            PyArrayObject* arr = (PyArrayObject*)csp::python::toPythonBorrowed(
                tsProvider->lastValueTyped<DialectGenericType>());
                ...
                if (PyArray_SIZE(arr) != m_arr_size)
                    CSP_THROW(
                        ...
                        "csp.exprtk NumPy array input must have same size each tick, but first saw "
                            << m_arr_size << " and now saw " << PyArray_SIZE(arr) << " for "
                            << m_var_name);
        ...
        bool registerValue(exprtk::symbol_table<double>& symbolTable,
                           const std::string& variableName) override {
            // store symbol table and var name so we can use them to register in setValue, on first
            // tick
    ...
    struct csp_now_fn : public exprtk::ifunction<double> {
      public:
        ...
        double operator()() {
            return (m_engine->rootEngine()->now()).asNanoseconds() / 1e9;
        }
        void setEngine(csp::Engine* engine) {
            m_engine = engine;
        }
    ...
    void compile_expression() {
        ...
        if (!s_parser.compile(expression_str, s_expr))
            CSP_THROW(ValueError, "cannot compile expression: " << std::string(expression_str)
    ...
    START() {
        ...
        for (auto it = state_vars.value()->begin(); it != state_vars.value()->end(); ++it) {
            if (it.hasValue<std::string>()) {
                ...
                CSP_THROW(ValueError, "state_vars dictionary contains "
                                          << it.key()
        ...
        for (auto it = constants.value()->begin(); it != constants.value()->end(); ++it) {
            if (it.hasValue<double>() || it.hasValue<int64_t>())
                ...
                CSP_THROW(ValueError, "constants dictionary contains "
                                          << it.key() << " with unsupported type (need be float)");
        }
        ...
        if (functions.value()->size() > 0) {
            ...
            for (auto it = functions.value()->begin(); it != functions.value()->end(); ++it) {
                csp::python::PyObjectPtr fnInfo = csp::python::PyObjectPtr::own(
                    csp::python::toPython(it.value<DialectGenericType>()));
                const char* body;
                PyObject* vars;

                if (!PyArg_ParseTuple(fnInfo.get(), "O!s", &PyTuple_Type, &vars, &body)) {
                    CSP_THROW(csp::python::PythonPassthrough,
                              "could not parse function info in csp.exprtk");
                }
                ...
                switch (numVars) {
                    case 0:
                        CSP_THROW(ValueError,
                                  "csp.exprtk functions must take at least one variable");
                        break;
                    case 1:
                        if (!PyArg_ParseTuple(vars, "s", &arg1)) {
                            CSP_THROW(csp::python::PythonPassthrough,
                                      "csp.exprtk could not parse variables list");
                        }
                    ...
                    case 4:
                        ...
                        break;
                    default:
                        CSP_THROW(ValueError, "csp.exprtk given too many variables ("
                                                  << numVars << "), max supported is 4");
                }
            }
        }
...
static PyModuleDef _cspbaselibimpl_module = {PyModuleDef_HEAD_INIT,
                                             "_cspbaselibimpl",
