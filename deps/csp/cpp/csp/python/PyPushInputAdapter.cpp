...
static PyObject* PushBatch_exit(PyPushBatch* self, PyObject* args) {
    ...
    // We flush here since we want to flush on exit from scope, not when object is destroyed ( which
    // can outlast the scope! )
    self->batch.flush();
    Py_RETURN_NONE;
}
...
class PyPushInputAdapter : public PushInputAdapter {
  public:
    PyPushInputAdapter(Engine* engine, AdapterManager* manager, PyObjectPtr pyadapter,
...
class TypedPyPushInputAdapter : public PyPushInputAdapter {
  public:
    TypedPyPushInputAdapter(Engine* engine, AdapterManager* manager, PyObjectPtr pyadapter,
...
struct PyPushInputAdapter_PyObject {
    ...
    static PyObject* pushTick(PyPushInputAdapter_PyObject* self, PyObject* args,
                              PyObject** kwargs) {
        CSP_BEGIN_METHOD;
        ...
        if (len < 1 || len > 2)
            CSP_THROW(TypeError,
                      "push_tick takes value and optional batch as positional arguments");
...
static PyMethodDef PyPushInputAdapter_PyObject_methods[] = {
    {"push_tick", (PyCFunction)PyPushInputAdapter_PyObject::pushTick, METH_VARARGS,
     "push new tick"},
    {"shutdown_engine", (PyCFunction)PyPushInputAdapter_PyObject::shutdown_engine, METH_O,
     "shutdown_engine"},
    {NULL}};
...
static InputAdapter* pypushinputadapter_creator(csp::AdapterManager* manager, PyEngine* pyengine,
    ...
    if (!PyType_IsSubtype(pyAdapterType, &PyPushInputAdapter_PyObject::PyType))
        CSP_THROW(TypeError,
                  "Expected PushInputAdapter derived type, got " << pyAdapterType->tp_name);
    ...
    if (pyPushGroup != Py_None) {
        ...
        if (!pushGroup) {
            PyErr_Clear();
            CSP_THROW(TypeError, "Expected PushGroup instance for push group, got: "
    ...
    switchPyType(pyType, [&](auto tag) {
        pyAdapter->adapter =
            pyengine->engine()
                ->createOwnedObject<TypedPyPushInputAdapter<typename decltype(tag)::type>>(
