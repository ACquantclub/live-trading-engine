...
template <typename T>
inline T* getValues(const TickBuffer<T>* tickBuffer, const T& lastValue, int32_t startIndex,
                    int32_t endIndex, int32_t* len, bool tailPadding) {
    T* values = nullptr;
    if (tickBuffer) {
...
inline PyObject* as_nparray(const csp::TimeSeriesProvider* ts, const TickBuffer<T>* tickBuffer,
                            const T& lastValue, int32_t startIndex, int32_t endIndex,
                            bool tailPadding) {
...
inline PyObject* as_nparray(const csp::TimeSeriesProvider* ts,
                            const TickBuffer<DateTime>* tickBuffer, const DateTime& lastValue,
    ...
    auto* array =
        PyArray_NewFromDescr(&PyArray_Type, datetime_descr, 1, dims, nullptr, values, 0, nullptr);
    PyArray_ENABLEFLAGS((PyArrayObject*)array, NPY_ARRAY_OWNDATA);
    return array;
}

inline PyObject* as_nparray(const csp::TimeSeriesProvider* ts,
                            const TickBuffer<TimeDelta>* tickBuffer, const TimeDelta& lastValue,
                            int32_t startIndex, int32_t endIndex, bool tailPadding) {
    int32_t len = startIndex - endIndex + 1;
    if (len <= 0 || !ts->valid() || (!tickBuffer && endIndex != 0))
    ...
    auto* array =
        PyArray_NewFromDescr(&PyArray_Type, timedelta_descr, 1, dims, nullptr, values, 0, nullptr);
    PyArray_ENABLEFLAGS((PyArrayObject*)array, NPY_ARRAY_OWNDATA);
    return array;
}
...
inline PyObject* adjustStartAndEndTime(PyObject* arrayObj, autogen::TimeIndexPolicy startPolicy,
                                       autogen::TimeIndexPolicy endPolicy, DateTime startDt,
                                       DateTime endDt) {
...
template <typename T>
inline PyObject* createNumpyArray(ValueType valueType, const csp::TimeSeriesProvider* ts,
                                  int32_t startIndex, int32_t endIndex,
                                  autogen::TimeIndexPolicy startPolicy,
                                  autogen::TimeIndexPolicy endPolicy, DateTime startDt,
                                  DateTime endDt) {
    bool extrapolateEnd = endPolicy == autogen::TimeIndexPolicy::EXTRAPOLATE &&
                          endIndex < ts->numTicks() && ts->timeAtIndex(endIndex) < endDt;

    // MSVC bug!! They dont eval ternary operators correctly and this code actually evaluates both
    // sides which leads to a crash since lastValueTyped should not be called at all if ts is not
    // valid... T lastValue = ( ts -> valid() ? ts -> lastValueTyped<T>() : T() );
    ...
    switch (valueType) {
        case ValueType::VALUE:
            return as_nparray(ts, ts->dataline<T>(), lastValue, startIndex, endIndex,
                              extrapolateEnd);

        case ValueType::TIMESTAMP: {
            return adjustStartAndEndTime(
                as_nparray(ts, ts->timeline(), lastTime, startIndex, endIndex, extrapolateEnd),
                startPolicy, endPolicy, startDt, endDt);
        }
        case ValueType::TIMESTAMP_VALUE_TUPLE: {
            PyObject* tuple = PyTuple_New(2);
            PyTuple_SET_ITEM(tuple, 0,
                             adjustStartAndEndTime(as_nparray(ts, ts->timeline(), lastTime,
                                                              startIndex, endIndex, extrapolateEnd),
                                                   startPolicy, endPolicy, startDt, endDt));
            PyTuple_SET_ITEM(
                tuple, 1,
...
PyObject* valuesAtIndexToNumpy(ValueType valueType, const csp::TimeSeriesProvider* ts,
                               int32_t startIndex, int32_t endIndex,
                               autogen::TimeIndexPolicy startPolicy,
                               autogen::TimeIndexPolicy endPolicy,
                               DateTime startDt = DateTime::NONE(),
