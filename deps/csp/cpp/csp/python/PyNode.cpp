...
PyNode::PyNode(csp::Engine* engine, PyObjectPtr gen, PyObjectPtr inputs, PyObjectPtr outputs,
               NodeDef def)
    : csp::Node(def, engine), m_gen(gen), m_localVars(nullptr), m_passiveCounts(nullptr) {
    init(inputs, outputs);
}
...
// PY311+ changes
    ...
    for (int stackloc = code->co_argcount; stackloc < code->co_nlocalsplus;
         ++stackloc, ++localPlusIndex) {
        ...
        if (!PyTuple_Check(*var))
            CSP_THROW(TypeError, "expected tuple types in stack, got " << (*var)->ob_type->tp_name
                                                                       << " in node " << name());
            ...
            if (PyTuple_Check(inputType)) {
                ...
                if (shape == Py_None) {
                    ...
                    if (!PyList_Check(shape))
                        CSP_THROW(TypeError, "Expected input basket shape as int or list, got "
                                                 << Py_TYPE(shape)->tp_name << " on node \""
                                                 << name() << "\"");
            ...
            if (PyTuple_Check(outputType)) {
                ...
                if (shape == Py_None) {
                    ...
                    if (basketSize > OutputId::maxBasketElements())
                        CSP_THROW(ValueError, "output basket size exceeds limit of "
                                                  << OutputId::maxBasketElements() << " on node \""
                                                  << name() << "\"");
                    newvalue = PyListBasketOutputProxy::create(outputType, this, index, basketSize);
                } else {
                    if (!PyList_Check(shape))
                        CSP_THROW(TypeError, "Expected output basket shape as int or list, got "
                                                 << Py_TYPE(shape)->tp_name << " on node \""
                                                 << name() << "\"");
...
void PyNode::stop() {
    if (this->rootEngine()->interrupted() && PyErr_CheckSignals() == -1) {
        // When an interrupt occurs a KeyboardInterrupt exception is raised in Python, which we need
        // to clear before calling "close" on the generator. Else, the close method will fail due to
        // the unhandled exception, and we lose the state of the generator before the "finally"
        // block that calls stop() is executed.
        PyErr_Clear();
    }
...
PyNode* PyNode::create(PyEngine* pyengine, PyObject* inputs, PyObject* outputs, PyObject* gen) {
    ...
    if (size_t(numOutputs) > OutputId::maxId())
        CSP_THROW(ValueError,
                  "number of outputs exceeds limit of " << OutputId::maxBasketElements());

    return pyengine->engine()->createOwnedObject<PyNode>(
        PyObjectPtr::incref(gen), PyObjectPtr::incref(inputs), PyObjectPtr::incref(outputs),
...
void PyNode::executeImpl() {
    for (size_t idx = 0; idx < numInputs(); ++idx) {
        if (!isInputBasket(idx)) {
            ...
            // thus: we convert active inputs that ticked this cycle or passive inputs that ticked
            // since we last converted them
            auto* ts = tsinput(idx);
            auto count = ts->count();
...
static PyObject* PyNode_create(PyObject* module, PyObject* args) {
    ...
    if (!PyArg_ParseTuple(args, "O!O!O!O!", &PyEngine::PyType, &engine, &PyTuple_Type, &inputs,
                          &PyTuple_Type, &outputs, &PyGen_Type, &gen))
        CSP_THROW(PythonPassthrough, "");
