  ...
  public:
    ...
    void setValue(Struct* s, const DialectGenericType& obj) const override {
        ...
        if (!PyObject_IsInstance(pyobj.ptr(), (PyObject*)m_pytype.ptr()))
            CSP_THROW(TypeError, "Invalid " << m_pytype->tp_name << " type, expected "
                                            << m_pytype->tp_name << " got "
                                            << Py_TYPE(pyobj.ptr())->tp_name << " for field '"
                                            << fieldname() << "'");
...
DialectStructMeta::DialectStructMeta(PyTypeObject* pyType, const std::string& name,
    ...
    : StructMeta(name, flds, base), m_pyType(pyType) {
}

/*
MetaClass Madness NOTES!!!

It took quite a while to get this straight.  PyStructMeta::PyType is the StructMeta type for
StructMeta type instances. Instances of PyStructMeta::PyType are PyStrutMeta types, which are Struct
class definitions. PyStructMeta types ( aka Struct class types ) are forced to inherit from
PyStruct::PyType. This is done in python code where csp.Struct is defined as class Struct(
_cspimpl.PyStruct, metaclass = StructMeta ). StructMeta is defined in python and is a derived type
of _cspimpl.PyStructMeta so that we can defined some helper code in python.
...
class StructMeta( _cspimpl.PyStructMeta ):
    def __new__( ... ):
        #extract meta info
        super().__new__( ..., metainfo )  <-- this calls into PyStructMeta_new in c++, the base
type, which will return a new PyStructMeta instance ( a new struct type )
...
when MyStruct is declared, it will call StructMeta::__new__ which will call PyStructMeta_new and
create the MyStruct type, which is PyStructMeta instance when MyStruct() instance is created, it'll
defer required calls to _cspimpl.PyStruct, its baseclass
...
static PyObject* PyStructMeta_new(PyTypeObject* subtype, PyObject* args, PyObject* kwds) {
    ...
    // Note that we call ctor without parents so as not to 0-init the base POD PyTypeObject class
    ...
    StructMeta::Fields fields;
    {
        ...
        while (PyDict_Next(metadata, &pos, &key, &type)) {
            ...
            if (!PyType_Check(type) && !PyList_Check(type))
                CSP_THROW(TypeError, "Struct metadata for key " << keystr
                                                                << " expected a type, got "
            ...
            switch (csptype->type()) {
                ...
                case csp::CspType::Type::DIALECT_GENERIC:
                    field = std::make_shared<PyObjectStructField>(
                        keystr, PyTypeObjectPtr::incref((PyTypeObject*)type));
                    break;
                default:
                    CSP_THROW(ValueError,
                              "Unexpected csp type " << csptype->type() << " on struct " << name);
            }
    ...
    for (Py_ssize_t idx = 0; idx < numbases; ++idx) {
        ...
        // Check if base is a PyStruct type.  Note that this might be the python side csp.Struct
        // class, but we dont create structMeta on that ( see above ) so it will be null here, no
        //need to check explicitly
        if (PyType_IsSubtype(base, &PyStruct::PyType)) {
            if (metabase)
                CSP_THROW(TypeError, "Struct " << name
                                               << " defined with multiple struct bases.  Only "
                                                  "single-struct hierarchy is supported");
    ...
    */
    auto structMeta =
        std::make_shared<DialectStructMeta>((PyTypeObject*)pymeta, name, fields, metabase);
    ...
    /*printf( "Struct %s size %lu ( partial %lu )\n", name.c_str(), structMeta -> size(), structMeta
    -> partialSize() ); for( auto & field : structMeta -> fields() )
    {
        printf( "\t%s loc %lu size %lu maskloc %lu maskbit %d type %s\n", field ->
    fieldname().c_str(), field -> offset(), field -> size(), field -> maskOffset(), field ->
    maskBit(), field -> type().asCString() );
                }*/
...
static PyObjectPtr PyStructMeta_typeinfo(const CspType* type) {
    ...
    if (PyDict_SetItemString(out.get(), "type",
                             PyObjectPtr::own(toPython(type->type().asString())).get()) < 0)
        CSP_THROW(PythonPassthrough, "");

    if (PyDict_SetItemString(out.get(), "type_id",
                             PyObjectPtr::own(toPython(type->type().value())).get()) < 0)
        CSP_THROW(PythonPassthrough, "");
    ...
    if (type->type() == CspType::Type::ENUM) {
        ...
        if (PyDict_SetItemString(out.get(), "elemtype",
                                 PyStructMeta_typeinfo(arrayType->elemType().get()).get()) < 0)
            CSP_THROW(PythonPassthrough, "");
    }
...
static PyObjectPtr PyStructMeta_fieldinfo(StructField* field) {
    PyObjectPtr out = PyObjectPtr::own(PyDict_New());
    if (PyDict_SetItemString(out.get(), "fieldname",
                             PyObjectPtr::own(toPython(field->fieldname())).get()) < 0)
        CSP_THROW(PythonPassthrough, "");

    if (PyDict_SetItemString(out.get(), "type", PyStructMeta_typeinfo(field->type().get()).get()) <
        0)
        CSP_THROW(PythonPassthrough, "");

    if (PyDict_SetItemString(
            out.get(), "offset",
            PyObjectPtr::own(toPython(static_cast<std::uint64_t>(field->offset()))).get()) < 0)
        CSP_THROW(PythonPassthrough, "");

    if (PyDict_SetItemString(
            out.get(), "size",
            PyObjectPtr::own(toPython(static_cast<std::uint64_t>(field->size()))).get()) < 0)
        CSP_THROW(PythonPassthrough, "");

    if (PyDict_SetItemString(
            out.get(), "alignment",
            PyObjectPtr::own(toPython(static_cast<std::uint64_t>(field->alignment()))).get()) < 0)
        CSP_THROW(PythonPassthrough, "");

    if (PyDict_SetItemString(
            out.get(), "mask_offset",
            PyObjectPtr::own(toPython(static_cast<std::uint64_t>(field->maskOffset()))).get()) < 0)
        CSP_THROW(PythonPassthrough, "");

    if (PyDict_SetItemString(out.get(), "mask_bit",
                             PyObjectPtr::own(toPython(field->maskBit())).get()) < 0)
        CSP_THROW(PythonPassthrough, "");

    if (PyDict_SetItemString(out.get(), "mask_bitmask",
                             PyObjectPtr::own(toPython(field->maskBitMask())).get()) < 0)
        CSP_THROW(PythonPassthrough, "");
...
static PyObject* PyStructMeta_metadata_info(PyStructMeta* m) {
    ...
    if (PyDict_SetItemString(
            out.get(), "size",
            PyObjectPtr::own(toPython(static_cast<std::uint64_t>(meta->size()))).get()) < 0)
        CSP_THROW(PythonPassthrough, "");

    if (PyDict_SetItemString(
            out.get(), "partial_size",
            PyObjectPtr::own(toPython(static_cast<std::uint64_t>(meta->partialSize()))).get()) < 0)
        CSP_THROW(PythonPassthrough, "");

    if (PyDict_SetItemString(out.get(), "is_native",
                             PyObjectPtr::own(toPython(meta->isNative())).get()) < 0)
        CSP_THROW(PythonPassthrough, "");

    if (PyDict_SetItemString(
            out.get(), "mask_loc",
            PyObjectPtr::own(toPython(static_cast<std::uint64_t>(meta->maskLoc()))).get()) < 0)
        CSP_THROW(PythonPassthrough, "");

    if (PyDict_SetItemString(
            out.get(), "mask_size",
            PyObjectPtr::own(toPython(static_cast<std::uint64_t>(meta->maskSize()))).get()) < 0)
        CSP_THROW(PythonPassthrough, "");
...
static PyMethodDef PyStructMeta_methods[] = {
    {"_layout", (PyCFunction)PyStructMeta_layout, METH_NOARGS,
     "debug view of structs internal mem layout"},
...
PyObject* getarrayattr_(const StructField* field, const PyStruct* pystruct) {
    ...
    PyObject* v = ArraySubTypeSwitch::invoke(arrayType->elemType(), [field, pystruct](auto tag) {
        using StorageT =
            typename CspType::Type::toCArrayStorageType<typename decltype(tag)::type>::type;
...
PyObject* PyStruct::getattr(PyObject* attr) {
    ...
    if (!field->isSet(struct_.get())) {
        ...
        // we also pass the attribute as the exception for efficiency, expensive to format a nice
        // error here that wont get used for hasattr calls
...
void PyStruct::setattr(Struct* s, PyObject* attr, PyObject* value) {
    ...
    if (!field)
        CSP_THROW(AttributeError, "'" << s->meta()->name() << "' object has no attribute '"
...
void repr_field(const Struct* struct_, const StructFieldPtr& field, std::string& tl_repr,
                bool show_unset) {
...
void repr_array(const std::vector<StorageT>& val, const CspType& elemType, std::string& tl_repr,
                bool show_unset) {
...
We ensure to maintain a single PyStruct instance per c++ StructPtr instance.  This instance is
cached in the Struct's hidden() data and will be created on the fly if it doesnt exist when needed (
see Conversions.h ) The instance on the StructPtr does NOT own a python ref.  When the PyStruct
refcount drops to 0, or when its garbage collected and destroyed, we clear the dialectPtr from the
c++ struct. Now in tp_traverse and tp_clear, we ensure not to do traversal or clearing unless we are
the only holders of the c++ StructPtr, otherwise we would end up clearing / freeing data that is
still used by the struct ( instance can still exist in c++ / in the timeseries buffers ) The reason
we ensure a single PyStruct instance exists is to avoid leaks where we can have a self referencing
container with the same struct twice, ie [ S1, S2 ] where S1 and S2 point to the same StructPtr and
hold a ref to the list as well.  If we allowed multiple PyStruct instances, the refcount would stay
at 2 and this would never clean up
****************************/
...
void PyStruct_setattrs(PyStruct* self, PyObject* args, PyObject* kwargs, const char* methodName) {
    if (PyTuple_GET_SIZE(args) > 0)
        CSP_THROW(TypeError, "'" << self->ob_type->tp_name << '.' << methodName
                                 << "' takes 0 positional arguments but " << PyTuple_GET_SIZE(args)
                                 << " were given");
...
PyObject* PyStruct_deepcopy(PyStruct* self) {
    ...
    // deepcopy traversal may kick in a GC collect, so we have to call that first before the
    // PyStruct is created of it may traverse a partially consturcted object and crash
...
PyObject* PyStruct_update_from(PyStruct* self, PyObject* o) {
    ...
    if (!PyType_IsSubtype(Py_TYPE(o), &PyStruct::PyType))
        CSP_THROW(TypeError,
                  "Attempting to update_from from non-struct type " << Py_TYPE(o)->tp_name);
...
PyObject* PyStruct_to_dict(PyStruct* self, PyObject* args, PyObject* kwargs) {
    ...
    if (PyArg_ParseTuple(args, "Op:to_dict", &callable, &preserve_enums)) {
        if ((callable != Py_None) && !PyCallable_Check(callable)) {
            CSP_THROW(TypeError, "Parameter must be callable or None got " +
                                     std::string(Py_TYPE(callable)->tp_name));
        }
    }
    if (callable == Py_None)
        callable = nullptr;
...
static PyMethodDef PyStruct_methods[] = {
    ...
    {"deepcopy_from", (PyCFunction)PyStruct_deepcopy_from, METH_O,
     "deepcopy from struct. struct must be same type or a derived type. unset fields will copy "
     "over"},
    {"update_from", (PyCFunction)PyStruct_update_from, METH_O,
     "update from struct. struct must be same type or a derived type. unset fields will be not be "
     "copied"},
    {"update", (PyCFunction)PyStruct_update, METH_VARARGS | METH_KEYWORDS,
     "update from key=val.  given fields will be set on struct.  other fields will remain as is in "
     "struct"},
    ...
    {"to_dict", (PyCFunction)PyStruct_to_dict, METH_VARARGS | METH_KEYWORDS,
     "return a python dict of the struct by recursively converting struct members into python "
     "dicts"},
    {"to_json", (PyCFunction)PyStruct_to_json, METH_VARARGS | METH_KEYWORDS,
     "return a json string of the struct by recursively converting struct members into json "
     "format"},
    {NULL}};
