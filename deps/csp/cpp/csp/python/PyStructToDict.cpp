...
class StructToDictHelper {
  public:
    StructToDictHelper(bool preserve_enums, PyObject* callable)
        : m_preserveEnums(preserve_enums), m_callable(callable) {
        m_ptrsVisited.clear();
    }
  ...
  private:
    ...
    // instead add support for those python types to csp so that they can be handled more
    // generically and in a language agnostic way
    PyObjectPtr parsePyObject(PyObject* value, bool is_recursing);

    class CircularRefCheck {
      public:
        CircularRefCheck(std::unordered_set<const void*>& ptrs_visited, const void* ptr)
            : m_ptrsVisited(ptrs_visited), m_ptr(ptr) {
...
template <>
inline PyObjectPtr StructToDictHelper::parseCspToPython<CspEnum>(const CspEnum& val,
                                                                 const CspType& typ) {
    if (m_preserveEnums)
...
template <>
inline PyObjectPtr StructToDictHelper::parseCspToPython<StructPtr>(const StructPtr& val,
                                                                   const CspType& typ) {
...
template <>
inline PyObjectPtr StructToDictHelper::parseCspToPython<DialectGenericType>(
    const DialectGenericType& val, const CspType& typ) {
...
template <typename StorageT>
inline PyObjectPtr StructToDictHelper::parseCspToPython(const std::vector<StorageT>& val,
                                                        const CspType& typ) {
...
PyObjectPtr StructToDictHelper::parseStructToDictRecursive(const StructPtr& self) {
    ...
    if (PyObject_HasAttrString(py_struct.get(), "postprocess_to_dict")) {
        auto postprocess_dict_callable =
            PyObjectPtr::own(PyObject_GetAttrString(py_struct.get(), "postprocess_to_dict"));
        new_dict = PyObjectPtr::check(
            PyObject_CallFunction(postprocess_dict_callable.get(), "(O)", new_dict.get()));
    }
    return new_dict;
}
