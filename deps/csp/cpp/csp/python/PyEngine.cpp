...
PythonEngine::PythonEngine(PyEngine* parent, const Dictionary& settings)
    : RootEngine(settings), m_parent(parent), m_pyThreadState(nullptr) {
...
PyObject* PyEngine::collectOutputs() {
    ...
    for (auto& key : m_engine->graphOutputKeys()) {
        // Not sure if there is a more efficient way to do this other than converting per element (
        // aside from using numpy arrays which would impose a dep on numpy )
