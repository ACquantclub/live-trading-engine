...
enum ValueType : uint8_t { VALUE, TIMESTAMP, TIMESTAMP_VALUE_TUPLE };
...
struct DateTimeOrTimeDelta : public std::variant<DateTime, TimeDelta> {
    ...
    bool isDateTime() const {
        return std::holds_alternative<DateTime>(*this);
    }
    bool isTimeDelta() const {
        return std::holds_alternative<TimeDelta>(*this);
    }

    DateTime datetime() const {
        return std::get<DateTime>(*this);
    }
    TimeDelta timedelta() const {
        return std::get<TimeDelta>(*this);
    }
};
...
inline bool validatePyType(const CspType* type, PyObject* pyType, PyObject* value) {
    // helper method to ensure that dialect generic types pushed from user circuits are aligned with
    // expected types we only check dialect generic because other well known types will fail in
    // fromPython conversion code
...
inline StructPtr fromPython(PyObject* o, const CspType& type) {
    ...
    if (!PyType_IsSubtype(Py_TYPE(o), &PyStruct::PyType) ||
        (static_cast<const CspStructType&>(type)
             .meta() &&  // could be csp.Struct as a type annotation which is allowed
        ...
        if (meta)
            name = " " + meta->name();
        CSP_THROW(TypeError,
                  "Invalid struct type, expected struct" << name << " got " << Py_TYPE(o)->tp_name);
    }
...
inline PyObject* toPython(const CspEnum& e, const CspType& type) {
    ...
    if (unlikely(!obj))
        CSP_THROW(ValueError, e.value()
                                  << " is not a valid value on csp.enum type " << emeta->name());
    return obj;
}
...
inline CspEnum fromPython(PyObject* o, const CspType& type) {
    ...
    if (!PyType_IsSubtype(Py_TYPE(o), &PyCspEnum::PyType) ||
        ...
        CSP_THROW(TypeError, "Invalid enum type, expected enum type "
                                 << static_cast<const CspEnumType&>(type).meta()->name() << " got "
                                 << Py_TYPE(o)->tp_name);
...
inline TimeDelta fromPython(PyObject* o) {
    ...
    if (!PyDelta_Check(o))
        CSP_THROW(TypeError,
                  "Invalid timedelta type, expected timedelta got " << Py_TYPE(o)->tp_name);
    ...
    if (d->days > MAX_DAYS || d->days < MIN_DAYS) {
        CSP_THROW(OverflowError,
                  "timedelta " << PyObjectPtr::incref(o) << " out of range for csp timedelta");
    }
...
inline Time fromPython(PyObject* o) {
    ...
    if (t->hastzinfo)
        CSP_THROW(TypeError,
                  "csp time type does not support timezones.  Please use ts[object] for timezone "
                  "time values");
...
inline PyObject* toPython(const DateTime& dt) {
    ...
    DateTimeEx dtEx(dt);
    return toPythonCheck(PyDateTime_FromDateAndTime(dtEx.year(), dtEx.month(), dtEx.day(),
                                                    dtEx.hour(), dtEx.minute(), dtEx.second(),
                                                    dtEx.microseconds()));
}
...
inline DateTime fromPython(PyObject* o) {
    ...
    if (!PyDateTime_Check(o))
        CSP_THROW(TypeError,
                  "Invalid datetime type, expected datetime got " << Py_TYPE(o)->tp_name);
    ...
    if (!PyDateTime_CheckExact(o) && PyObject_HasAttrString(o, "nanosecond")) {
        int64_t ns =
            fromPython<int64_t>(PyObjectPtr::own(PyObject_GetAttrString(o, "nanosecond")).ptr());
        nanosecond += ns;
    }
    ...
    if (year < MIN_DATE.year() + 1 || year > MAX_DATE.year() - 1) {
        CSP_THROW(OverflowError,
                  "datetime " << PyObjectPtr::incref(o) << " is out of range for csp datetime");
    }
    ...
    if (pydt->hastzinfo) {
        ...
        PyObjectPtr utcoffset = PyObjectPtr::own(
            PyObject_CallMethodObjArgs(pydt->tzinfo, utcoffsetStr.ptr(), o, nullptr));
...
inline DateTimeOrTimeDelta fromPython(PyObject* o) {
    ...
    CSP_THROW(TypeError,
              "Invalid type, expected datetime or timedelta got " << Py_TYPE(o)->tp_name);
}
...
inline Dictionary fromPython(PyObject* o) {
    if (!PyDict_Check(o))
        CSP_THROW(TypeError,
                  "Dictionary conversion expected type dict got " << Py_TYPE(o)->tp_name);
...
inline std::vector<Dictionary::Data> fromPython(PyObject* o) {
    if (!PyList_Check(o))
        CSP_THROW(TypeError,
                  "Dictionary conversion expected type list got " << Py_TYPE(o)->tp_name);
...
inline PyObject* toPython(const std::vector<StorageT>& v, const CspType& arrayType,
    ...
    if (cspArrayType.isPyStructFastList()) {
        PyObject* fl =
            PyStructFastList<StorageT>::PyType.tp_alloc(&PyStructFastList<StorageT>::PyType, 0);
    ...
    // TODO: Implement more efficient list allocation by pre-allocating the space and filling it
    // using PyList_SET_ITEM. As of now, the problem is that Python is not allowing to resize the
    // list via API, and it cannot allocate the list at the base of PyStructList, it can only
    // allocate it somewhere in memory not under control.
    PyObject* psl = PyStructList<StorageT>::PyType.tp_alloc(&PyStructList<StorageT>::PyType, 0);
    new (psl) PyStructList<StorageT>(const_cast<PyStruct*>(pystruct),
                                     const_cast<std::vector<StorageT>&>(v), cspArrayType);
...
struct FromPython<std::vector<StorageT>> {
    static std::vector<StorageT> impl(PyObject* o, const CspType& arrayType) {
        ...
        // allow iteratables
            ...
            CSP_THROW(TypeError, "Invalid list / iterator type, expected list or iterator got "
                                     << Py_TYPE(o)->tp_name);
