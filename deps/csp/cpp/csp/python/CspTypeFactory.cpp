...
CspTypePtr& CspTypeFactory::typeFromPyType(PyObject* pyTypeObj) {
    ...
    if (PyList_Check(pyTypeObj)) {
        if (PyList_GET_SIZE((PyObject*)pyTypeObj) != 1 &&
            PyList_GET_SIZE((PyObject*)pyTypeObj) != 2)
            CSP_THROW(TypeError,
                      "Expected list types post-normalization to be one or two elements: sub-type "
                      "and optional FastList flag");
    ...
    if (rv.second) {
        if (pyType == &PyFloat_Type)
            ...
            if (!PyType_Check(pyType))
                CSP_THROW(TypeError, "expected python type for CspType got "
                                         << PyObjectPtr::incref((PyObject*)pyType));
