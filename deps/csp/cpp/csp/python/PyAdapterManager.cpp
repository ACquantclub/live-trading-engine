...
class PyAdapterManager : public AdapterManager {
  public:
    PyAdapterManager(Engine* engine, PyObjectPtr self) : AdapterManager(engine), m_self(self) {
    }
    ...
    void start(DateTime start, DateTime end) override {
        PyObjectPtr rv = PyObjectPtr::own(PyObject_CallMethod(
            m_self.ptr(), "start", "OO", PyObjectPtr::own(toPython(start)).ptr(),
    ...
    DateTime processNextSimTimeSlice(DateTime time) override {
        PyObjectPtr rv =
            PyObjectPtr::own(PyObject_CallMethod(m_self.ptr(), "process_next_sim_timeslice", "O",
...
static int PyAdapterManager_init(PyAdapterManager_PyObject* self, PyObject* args, PyObject* kwds) {
    ...
    self->manager = pyEngine->engine()->createOwnedObject<PyAdapterManager>(
        PyObjectPtr::incref((PyObject*)self));
    CSP_RETURN_INT;
}
...
static PyMethodDef PyAdapterManager_methods[] = {
    ...
    {"shutdown_engine", (PyCFunction)PyAdapterManager_PyObject_shutdown_engine, METH_O,
     "shutdown_engine"},
    {NULL}};
