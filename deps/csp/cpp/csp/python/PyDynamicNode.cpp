...
static PyObject* PyDynamicNode_create(PyObject* module, PyObject* args) {
    ...
    if (!PyArg_ParseTuple(args, "O!sO!O!OO!O!", &PyEngine::PyType, &pyEngine, &name, &PyTuple_Type,
                          &inputs, &PyTuple_Type, &outputs, &pyBuilder, &PyTuple_Type,
                          &pyDynamicArgs, &PyList_Type, &pySubgraphArgs))
    ...
    DynamicNode::EngineBuilder builder = [pyBuilderObj, snapMap = std::move(snapMap),
                                          attachMap = std::move(attachMap),
                                          snapkeyArgs = std::move(snapkeyArgs), pySubgraphArgsObj,
                                          pyOutputsObj](DynamicNode* pNode, DynamicEngine* engine,
                                                        const DialectGenericType& newkey) {
            ...
            if (!ts->valid())
                CSP_THROW(RuntimeException,
                          "csp.snap input ( sub_graph arg "
                              << arg_idx
                              << " ) is not valid at time of dynamic creation on csp.dynamic node '"
                              << pNode->name() << "'");
        ...
        for (auto& entry : attachMap) {
            ...
            // python side wiring already setup the Edge for csp.attach, just need to update the
            // elemId properly
        ...
        PyObjectPtr rv = PyObjectPtr::check(PyObject_CallFunctionObjArgs(
            pyBuilderObj.get(), pyEngineWrapper.get(), pySubgraphArgsObj.get(), NULL));
    ...
    DynamicNode* node =
        pyEngine->engine()->createOwnedObject<DynamicNode>(name, snapIds, builder, def);
