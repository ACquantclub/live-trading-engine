...
static PyObject* PyStructList_Extend(PyStructList<StorageT>* self, PyObject* args) {
    ...
    PyObjectPtr result =
        PyObjectPtr::check(PyObject_CallFunctionObjArgs(func.ptr(), self, iterable, NULL));
...
static PyObject* PyStructList_Remove(PyStructList<StorageT>* self, PyObject* args) {
    ...
    PyObjectPtr result =
        PyObjectPtr::check(PyObject_CallFunctionObjArgs(func.ptr(), self, value, NULL));
...
static int py_struct_list_ass_item(PyObject* sself, Py_ssize_t index, PyObject* value) {
    ...
    // The value is NULL -> erase vector[ index ]
        ...
        PyObjectPtr func =
            PyObjectPtr::own(PyObject_GetAttrString((PyObject*)&PyList_Type, "__delitem__"));
...
static int py_struct_list_ass_subscript(PyObject* sself, PyObject* item, PyObject* value) {
    ...
    if (value != NULL) {
        PyObjectPtr func =
            PyObjectPtr::own(PyObject_GetAttrString((PyObject*)&PyList_Type, "__setitem__"));
    ...
    if (value != NULL) {
        ...
        std::vector<StorageT> v = FromPython<std::vector<StorageT>>::impl(
            value, ((PyStructList<StorageT>*)sself)->arrayType);
...
static PyObject* py_struct_list_inplace_concat(PyObject* sself, PyObject* other) {
    ...
    PyObjectPtr result = PyObjectPtr::check(
        PyStructList_Extend<StorageT>((PyStructList<StorageT>*)sself, arguments.ptr()));
    Py_INCREF(sself);
    return sself;
...
static PyObject* py_struct_list_inplace_repeat(PyObject* sself, Py_ssize_t n) {
    ...
    PyObjectPtr func =
        PyObjectPtr::own(PyObject_GetAttrString((PyObject*)&PyList_Type, "__imul__"));
    PyObjectPtr result = PyObjectPtr::check(
        PyObject_CallFunction(func.ptr(), "On", (PyStructList<StorageT>*)sself, n));
...
template <typename StorageT>
static PyMappingMethods py_struct_list_as_mapping = {PyList_Type.tp_as_mapping->mp_length,
...
static PyObject* PyStructList_new(PyTypeObject* type, PyObject* args, PyObject* kwds) {
    // Since the PyStructList has no real meaning when created from Python, we can reconstruct the
    // PSL's value by just treating it as a list. Thus, we simply override the tp_new behaviour to
    // return a list object here. Again, since we don't have tp_init for the PSL, we need to rely on
    // the Python list's tp_init function.
