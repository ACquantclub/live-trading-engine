...
class VectorWrapper {
  public:
    ...
    bool operator==(const VectorWrapper& rhs) const {
    ...
    // If the index pointed outside of the array, it will continue to do so after the call to this
    // function
    ...
    struct Slice {
        ...
        Slice(IndexType start_i, IndexType stop_i, IndexType sp, IndexType len)
            : start_index(start_i), stop_index(stop_i), step(sp), length(len) {
        }
    ...
    // - if the step is positive, start and stop indices will be between 0 and one-past-the-last
    // array element inclusive
    // - if the step is negative, start and stop indices will be between -1 and the last array
    // element inclusive
    ...
    const std::vector<StorageT>& getVector() const {
        return m_vector;
    }
    IndexType size() const {
        return (IndexType)m_vector.size();
    }
    void append(const StorageT& value) {
    ...
    // Return the 0-based index of the first element that equals value in the subsequence
    ...
    std::vector<StorageT> getSlice(IndexType start_index, IndexType stop_index,
                                   IndexType step) const;
    void setSlice(const std::vector<StorageT>& other, IndexType start_index, IndexType stop_index,
                  IndexType step);
  ...
  private:
    ...
    inline IndexType force_index_in_bounds(IndexType index, IndexType l_bound,
                                           IndexType u_bound) const {
...
template <typename StorageT>
inline typename VectorWrapper<StorageT>::IndexType VectorWrapper<StorageT>::verify_index(
...
template <typename StorageT>
inline typename VectorWrapper<StorageT>::Slice VectorWrapper<StorageT>::normalizeSlice(
    IndexType start_index, IndexType stop_index, IndexType step) const {
    if (unlikely(step == 0))
        CSP_THROW(ValueError, "Slice step cannot be zero.");

    IndexType sz = size();
    // Call Python API method that computes the size of the slice and properly adjusts start_index
    // and stop_index
...
void VectorWrapper<StorageT>::insert(const StorageT& value, IndexType index) {
    // We allow list indices that are negative or point outside of the array
    // We allow indices for insert to be past the array boundaries, in which case the element is
...
template <typename StorageT>
typename VectorWrapper<StorageT>::IndexType VectorWrapper<StorageT>::index(
    const StorageT& value, IndexType start_index, IndexType stop_index) const {
    // We allow list indices that are negative or point outside of the array
    ...
    if (likely(normalized_slice.start_index < sz)) {
        ...
        auto stop_it = (normalized_slice.stop_index < sz)
                           ? m_vector.begin() + normalized_slice.stop_index
                           : m_vector.end();
...
std::vector<StorageT> VectorWrapper<StorageT>::getSlice(IndexType start_index, IndexType stop_index,
    ...
    for (VectorWrapper<StorageT>::IndexType index = normalized_slice.start_index;
         normalized_slice.belongs(index); index += normalized_slice.step)
...
void VectorWrapper<StorageT>::setSlice(const std::vector<StorageT>& other, IndexType start_index,
    ...
    // In case of step size 1, mismatch between lengths of the slice and the iterable to be set on
    // the slice is allowed In that case, we erase the full slice and insert the iterable in place
    // of it
    if (normalized_slice.step == 1 &&
        normalized_slice.length != (VectorWrapper<StorageT>::IndexType)other.size()) {
        auto start_it = (normalized_slice.start_index < sz)
                            ? m_vector.begin() + normalized_slice.start_index
                            : m_vector.end();
        auto stop_it = (normalized_slice.stop_index < sz)
                           ? m_vector.begin() + normalized_slice.stop_index
                           : m_vector.end();
    ...
    // In case of step size other then 1, mismatch between lengths of the slice and the iterable to
    // be set on the slice is not allowed
    else {
        if (unlikely(normalized_slice.length != (VectorWrapper<StorageT>::IndexType)other.size()))
            CSP_THROW(ValueError,
                      "Attempt to assign a sequence of mismatched size to extended slice.");
...
template <typename StorageT>
void VectorWrapper<StorageT>::eraseSlice(IndexType start_index, IndexType stop_index,
                                         IndexType step) {
    // We allow list indices that are negative or point outside of the array
    VectorWrapper<StorageT>::IndexType sz = m_vector.size();
    auto normalized_slice = normalizeSlice(start_index, stop_index, step);
