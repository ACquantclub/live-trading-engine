...
// keep a reference to the python struct type from conversion to/from csp::Struct <-> PyObject
// properly
...
struct CSPTYPESIMPL_EXPORT PyStruct : public PyObject {
    ...
    PyStructMeta* pyStructMeta() {
        return (PyStructMeta*)ob_type;
    };

    const DialectStructMeta* structMeta() {
        return static_cast<const DialectStructMeta*>(struct_->meta());
    }

    // Helper methods
    PyObject* getattr(PyObject* attr);
    void setattr(PyObject* attr, PyObject* value) {
        setattr(struct_.get(), attr, value);
    }
...
void repr_array(const std::vector<ElemT>& val, const CspType& elemType, std::string& tl_repr,
                bool show_unset);
