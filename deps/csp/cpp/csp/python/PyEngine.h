...
class CSPIMPL_EXPORT PyEngine final : public PyObject {
  public:
    ...
    PythonEngine* rootEngine() {
        return static_cast<PythonEngine*>(engine()->rootEngine());
    }
...
inline std::exception_ptr PyEngine_shutdown_make_exception(PyObject* pyException) {
    if (!PyExceptionInstance_Check(pyException)) {
        ...
        std::string desc = "Expected Exception object as argument for shutdown_engine: got " +
                           pyExceptionString + " of type " + Py_TYPE(pyException)->tp_name;
