...
PyInputProxy::PyInputProxy(PyNode* node, InputId id) : m_node(node), m_id(id) {
}
...
Scheduler::Handle PyInputProxy::scheduleAlarm(DateTimeOrTimeDelta dtd, PyObject* value) {
    ...
    auto* alarm =
        static_cast<AlarmInputAdapter<PyObjectPtr>*>(const_cast<TimeSeriesProvider*>(ts()));
...
PyObject* PyInputProxy::valueAt(ValueType valueType, PyObject* indexArg,
                                PyObject* duplicatePolicyArg, PyObject* defaultValueArg) const {
    int32_t valueIndex;

    if (PyLong_Check(indexArg)) {
        valueIndex = fromPython<int32_t>(indexArg);
        CSP_TRUE_OR_THROW_RUNTIME(
            valueIndex <= 0, "Expected non positive value for value_at index, got " << valueIndex);
        // Convert negative value to actual positive index
        valueIndex = -valueIndex;

        if (unlikely(valueIndex >= ts()->tickCountPolicy()))
            CSP_THROW(RangeError, "buffer index out of range.  requesting data at index "
                                      << valueIndex << " with buffer policy set to "
                                      << ts()->tickCountPolicy() << " ticks in node '"
                                      << m_node->name() << "'");
        ...
        CSP_TRUE_OR_THROW_RUNTIME(
            duplicatePolicyIntVal == TimeSeriesProvider::DuplicatePolicyEnum::LAST_VALUE ||
        ...
        if (dtd.isTimeDelta()) {
            ...
            CSP_TRUE_OR_THROW_RUNTIME(dtd.isDateTime(),
                                      "value_at index must be integer, DateTime, or TimeDelta");
...
int32_t PyInputProxy::computeStartIndex(DateTime startDt,
                                        autogen::TimeIndexPolicy startPolicy) const {
    int32_t startIndex;
...
PyObject* PyInputProxy::valuesAt(ValueType valueType, PyObject* startIndexArg,
                                 PyObject* endIndexArg, PyObject* startIndexPolicyArg,
                                 PyObject* endIndexPolicyArg) const {
    int32_t startIndex, endIndex;
    auto startPolicy =
        static_cast<autogen::TimeIndexPolicy>(static_cast<PyCspEnum*>(startIndexPolicyArg)->enum_);
    auto endPolicy =
        static_cast<autogen::TimeIndexPolicy>(static_cast<PyCspEnum*>(endIndexPolicyArg)->enum_);
    ...
    if (PyLong_Check(startIndexArg) || startIndexArg == Py_None) {
        ...
        CSP_TRUE_OR_THROW_RUNTIME(
            endIndex <= 0, "Expected non positive value for values_at end index, got " << endIndex);
        CSP_TRUE_OR_THROW_RUNTIME(
            endIndex >= startIndex,
        ...
        if (startPolicy.enum_value() != autogen::TimeIndexPolicy::enum_::INCLUSIVE)
            CSP_THROW(InvalidArgument,
                      "Unsupported time index policy for integer indexing: " << startPolicy.name());

        if (endPolicy.enum_value() != autogen::TimeIndexPolicy::enum_::INCLUSIVE)
            CSP_THROW(InvalidArgument,
                      "Unsupported time index policy for integer indexing: " << endPolicy.name());

        if (unlikely(startIndex >= ts()->tickCountPolicy()) && startIndexArg != Py_None)
            CSP_THROW(RangeError, "buffer index out of range.  requesting data at index "
                                      << startIndex << " with buffer policy set to "
                                      << ts()->tickCountPolicy() << " ticks in node '"
                                      << m_node->name() << "'");
        if (unlikely(endIndex >= ts()->tickCountPolicy()))
            CSP_THROW(RangeError, "buffer index out of range.  requesting data at index "
                                      << endIndex << " with buffer policy set to "
                                      << ts()->tickCountPolicy() << " ticks in node '"
                                      << m_node->name() << "'");
        ...
        if (startIndexArg != Py_None) {
            ...
            if (startDtd.isTimeDelta()) {
                ...
                if (unlikely(-startTd > ts()->tickTimeWindowPolicy()))
                    CSP_THROW(RangeError,
                              "buffer timedelta out of range.  requesting data at timedelta "
                                  << PyObjectPtr::incref(startIndexArg)
                                  << " with buffer policy set to "
                                  << PyObjectPtr::own(toPython(ts()->tickTimeWindowPolicy()))
                                  << " in node '" << m_node->name() << "'");
                ...
                CSP_TRUE_OR_THROW_RUNTIME(startDt <= m_node->now(),
                                          "requesting data from future time");
        ...
        if (endIndexArg != Py_None) {
            ...
            if (endDtd.isTimeDelta()) {
                ...
                if (startIndexArg != Py_None)
                    CSP_TRUE_OR_THROW_RUNTIME(
                        endTd >= startTd, "Start timedelta (got "
                                              << startTd << ") must come before end timedelta (got "
                                              << endTd << ")");

                if (unlikely(-endTd > ts()->tickTimeWindowPolicy()))
                    CSP_THROW(RangeError,
                              "buffer timedelta out of range.  requesting data at timedelta "
                                  << PyObjectPtr::incref(endIndexArg)
                                  << " with buffer policy set to "
                                  << PyObjectPtr::own(toPython(ts()->tickTimeWindowPolicy()))
                                  << " in node '" << m_node->name() << "'");
                ...
                CSP_TRUE_OR_THROW_RUNTIME(endDt <= m_node->now(),
                                          "requesting data from future time");

                if (startIndexArg != Py_None)
                    CSP_TRUE_OR_THROW_RUNTIME(
                        endDt >= startDt, "Start datetime (got "
                                              << startDt << ") must come before end datetime (got "
                                              << endDt << ")");
        ...
        if (startPolicy == autogen::TimeIndexPolicy::EXTRAPOLATE) {
            if (!startTd.isNone())
                return valuesAtIndexToNumpy(valueType, ts(), startIndex, endIndex, startPolicy,
                                            endPolicy, m_node->now() + startTd,
                                            m_node->now() + endTd);
            if (!startDt.isNone())
                return valuesAtIndexToNumpy(valueType, ts(), startIndex, endIndex, startPolicy,
...
static PyObject* PyInputProxy_set_buffering_policy(PyInputProxy* proxy, PyObject* args,
                                                   PyObject* kwargs) {
    CSP_BEGIN_METHOD;
...
static inline PyObject* PyInputProxy_value_at_impl(ValueType valueType, PyInputProxy* proxy,
                                                   PyObject* args) {
    CSP_BEGIN_METHOD;
    PyObject* indexArg;
...
static inline PyObject* PyInputProxy_values_at_impl(ValueType valueType, PyInputProxy* proxy,
                                                    PyObject* args) {
    CSP_BEGIN_METHOD;
    ...
    if (!PyArg_ParseTuple(args, "OOO!O!", &startIndexArg, &endIndexArg, &PyCspEnum::PyType,
                          &startExclusiveArg, &PyCspEnum::PyType, &endExclusiveArg))
        CSP_THROW(RuntimeException, "Invalid arguments passed to values_at");

    return proxy->valuesAt(valueType, startIndexArg, endIndexArg, startExclusiveArg,
                           endExclusiveArg);
    CSP_RETURN_NONE;
}
...
static PyMethodDef PyInputProxy_methods[] = {
    ...
    {"reschedule_alarm", (PyCFunction)PyInputProxy_reschedule_alarm, METH_VARARGS,
     "reschedule alarm"},
    ...
    {"set_buffering_policy", (PyCFunction)PyInputProxy_set_buffering_policy,
     METH_VARARGS | METH_KEYWORDS, "set buffering policy"},
    {"value_at", (PyCFunction)PyInputProxy_value_at, METH_VARARGS,
     "Get historical value from time series"},
    {"time_at", (PyCFunction)PyInputProxy_time_at, METH_VARARGS,
     "Get historical timestamp from time series"},
    {"item_at", (PyCFunction)PyInputProxy_item_at, METH_VARARGS,
     "Get historical timestamp and value tuple from time series"},
    {"values_at", (PyCFunction)PyInputProxy_values_at, METH_VARARGS,
     "Get historical values from time series"},
    {"times_at", (PyCFunction)PyInputProxy_times_at, METH_VARARGS,
     "Get historical timestamps from time series"},
