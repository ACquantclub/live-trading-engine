...
class PyBaseBasketInputProxy : public PyObject {
  public:
    ...
    const InputBasketInfo* basketInfo() const {
        return const_cast<PyBaseBasketInputProxy*>(this)->basketInfo();
    }
...
class PyListBasketInputProxy final : public PyBaseBasketInputProxy {
  public:
    ...
    int64_t key(int64_t elemId) const {
        return elemId;
    }
...
class PyDictBasketInputProxy : public PyBaseBasketInputProxy {
  public:
    ...
    PyObjectPtr key(int64_t elemId) const {
        return PyObjectPtr::incref(PyList_GET_ITEM(m_shape.ptr(), elemId));
    }
...
class PyDynamicBasketInputProxy final : public PyDictBasketInputProxy {
  ...
  private:
    ...
    void handleShapeChange(const DialectGenericType& key, bool added, int64_t elemId,
                           int64_t replaceId);
};
