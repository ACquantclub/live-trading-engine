...
class PyPushPullInputAdapter : public PushPullInputAdapter {
  public:
    PyPushPullInputAdapter(Engine* engine, AdapterManager* manager, PyObjectPtr pyadapter,
                           PyObject* pyType, PushMode pushMode, PushGroup* pushGroup)
    ...
    void start(DateTime start, DateTime end) override {
        PyObjectPtr rv = PyObjectPtr::own(PyObject_CallMethod(
            m_pyadapter.ptr(), "start", "OO", PyObjectPtr::own(toPython(start)).ptr(),
...
class TypedPyPushPullInputAdapter : public PyPushPullInputAdapter {
  public:
    TypedPyPushPullInputAdapter(Engine* engine, AdapterManager* manager, PyObjectPtr pyadapter,
                                PyObject* pyType, PushMode pushMode, PyObjectPtr pyPushGroup,
                                PushGroup* pushGroup)
    ...
    void pushPyTick(bool live, PyObject* time, PyObject* value, PushBatch* batch) override {
        ...
        try {
            ...
            CSP_THROW(TypeError, "\"" << Py_TYPE(m_pyadapter.ptr())->tp_name
                                      << "\" push adapter expected output type to be of type \""
                                      << pyTypeToString(m_pyType.ptr()) << "\" got type \""
...
struct PyPushPullInputAdapter_PyObject {
    ...
    static PyObject* pushTick(PyPushPullInputAdapter_PyObject* self, PyObject* args,
                              PyObject** kwargs) {
        CSP_BEGIN_METHOD;
        PyObject* pylive;
        PyObject* pytime;
        ...
        if (pybatch) {
            if (pybatch->ob_type != &PyPushBatch::PyType)
                CSP_THROW(TypeError, "push_tick expected PushBatch type as second argument, got "
                                         << pybatch->ob_type->tp_name);
    ...
    static PyObject* flagReplayComplete(PyPushPullInputAdapter_PyObject* self, PyObject* args,
                                        PyObject** kwargs) {
        CSP_BEGIN_METHOD;
...
static PyMethodDef PyPushPullInputAdapter_PyObject_methods[] = {
    {"push_tick", (PyCFunction)PyPushPullInputAdapter_PyObject::pushTick, METH_VARARGS,
     "push new tick"},
    {"flag_replay_complete", (PyCFunction)PyPushPullInputAdapter_PyObject::flagReplayComplete,
     METH_VARARGS, "finish replay ticks"},
    {"shutdown_engine", (PyCFunction)PyPushPullInputAdapter_PyObject::shutdown_engine, METH_O,
     "shutdown engine"},
    {NULL}};
...
static InputAdapter* pypushpullinputadapter_creator(csp::AdapterManager* manager,
                                                    PyEngine* pyengine, PyObject* pyType,
                                                    PushMode pushMode, PyObject* args) {
    PyTypeObject* pyAdapterType = nullptr;
    PyObject* adapterArgs = nullptr;
    PyObject* pyPushGroup;

    if (!PyArg_ParseTuple(args, "O!OO!", &PyType_Type, &pyAdapterType, &pyPushGroup, &PyTuple_Type,
                          &adapterArgs))
        CSP_THROW(PythonPassthrough, "");

    if (!PyType_IsSubtype(pyAdapterType, &PyPushPullInputAdapter_PyObject::PyType))
        CSP_THROW(TypeError,
                  "Expected PushPullInputAdapter derived type, got " << pyAdapterType->tp_name);
    ...
    PyPushPullInputAdapter_PyObject* pyAdapter = (PyPushPullInputAdapter_PyObject*)PyObject_Call(
        (PyObject*)pyAdapterType, adapterArgs, nullptr);
    if (!pyAdapter)
        CSP_THROW(PythonPassthrough, "");

    switchPyType(pyType, [&](auto tag) {
        pyAdapter->adapter =
            pyengine->engine()
                ->createOwnedObject<TypedPyPushPullInputAdapter<typename decltype(tag)::type>>(
