...
rapidjson::Value toJsonRecursive(const StructPtr& self, rapidjson::Document& doc,
                                 PyObject* callable);
...
rapidjson::Value pyObjectToJson(PyObject* value, rapidjson::Document& doc, PyObject* callable,
                                bool is_recursing);

// Helper fallback function to convert any type into json format recursively
template <typename T>
inline rapidjson::Value toJson(const T& val, const CspType& typ, rapidjson::Document& doc,
                               PyObject* callable) {
...
inline rapidjson::Value doubleToJson(const double& val, rapidjson::Document& doc) {
    // NOTE: Rapidjson adds support for this in a future release. Remove this when we upgrade
    // rapidjson to a version after 07/16/2023 and use kWriteNanAndInfNullFlag in the writer.
    //
    // To be compatible with other JSON libraries, we cannot use the default approach that rapidjson
    // has to serializing NaN, and (+/-)Infs. We need to manually convert them to NULLs. Rapidjson
    // adds support for this in a future release.
...
// Helper function to convert doubles into json format recursively, by properly handlings NaNs, and
// Infs
template <>
...
template <>
inline rapidjson::Value toJson(const Date& val, const CspType& typ, rapidjson::Document& doc,
                               PyObject* callable) {
    if (val.isNone())
        return rapidjson::Value();
...
template <>
inline rapidjson::Value toJson(const Time& val, const CspType& typ, rapidjson::Document& doc,
                               PyObject* callable) {
    if (val.isNone())
        return rapidjson::Value();

    // Convert the Time to <hours>:<minutes>:<seconds>.<microseconds>
    // hours( 2 ) + ':'( 1 ) + minutes( 2 ) + ':'( 1 ) + seconds( 2 ) + '.'( 1 ) + micros( 6 ) +
    // '\0'( 1 )
    char buf[48] = {};
...
inline rapidjson::Value toJson(const DateTime& val, const CspType& typ, rapidjson::Document& doc,
    ...
    auto len =
        sprintf(iso_str, "%04u-%02u-%02uT%02u:%02u:%02u.%06u%s", dtx.year(), dtx.month(), dtx.day(),
...
template <>
inline rapidjson::Value toJson(const DialectGenericType& val, const CspType& typ,
                               rapidjson::Document& doc, PyObject* callable) {
...
inline rapidjson::Value toJson(const std::vector<StorageT>& val, const CspType& typ,
                               rapidjson::Document& doc, PyObject* callable) {
...
rapidjson::Value pyDictKeyToName(PyObject* py_key, rapidjson::Document& doc, PyObject* callable) {
    // NOTE: Only support None, bool, strings, ints, and floats, date, time, datetime, enums,
    // csp.Enums as keys JSON encoding requires all names to be strings so convert them to strings
    ...
    if (s_tl_enum_type.get() == nullptr) [[unlikely]] {
        ...
        if (py_enum_module.get()) {
            s_tl_enum_type = PyTypeObjectPtr::own(reinterpret_cast<PyTypeObject*>(
                PyObject_GetAttrString(py_enum_module.get(), "Enum")));
        } else {
            CSP_THROW(RuntimeException,
                      "Unable to import enum module from the python standard library");
        }
    }
    ...
    if (py_key == Py_None) {
        ...
        CSP_THROW(ValueError,
                  "Cannot serialize key of type: " + std::string(Py_TYPE(py_key)->tp_name));
        // Never reaches here
    }
    return val;
}
...
rapidjson::Value pyObjectToJson(PyObject* value, rapidjson::Document& doc, PyObject* callable,
    ...
    if (value == Py_None) {
        ...
        // NOTE: Remove this logic when generic python tuples are supported as an internal type in
        // csp
        return pyTupleToJson(value, doc, callable);
    } else if (PyList_CheckExact(value)) {
        // NOTE: Remove this logic when generic python lists are supported as an internal type in
        // csp
        return pyListToJson(value, doc, callable);
    } else if (PyDict_CheckExact(value)) {
        // NOTE: Remove this logic when generic python dicts are supported as an internal type in
        // csp
        ...
        if (is_recursing) {
            ...
            CSP_THROW(ValueError,
                      "Cannot serialize value of type: " + std::string(Py_TYPE(value)->tp_name));
            // Never reaches here
        } else {
            // Not a known type
            // We expect callback to return a py object consisting of either csp types or dicts and
            // lists
