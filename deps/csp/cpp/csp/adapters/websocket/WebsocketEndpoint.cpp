...
WebsocketEndpoint::WebsocketEndpoint(Dictionary properties) : m_properties(properties){};
...
void WebsocketEndpoint::run() {
    ...
    if (m_properties.get<bool>("use_ssl")) {
        ...
        m_session = new WebsocketSessionTLS(m_ioc, ctx, &m_properties, m_on_open, m_on_fail,
                                            m_on_message, m_on_close, m_on_send_fail);
    } else {
        m_session = new WebsocketSessionNoTLS(m_ioc, &m_properties, m_on_open, m_on_fail,
                                              m_on_message, m_on_close, m_on_send_fail);
    }
    m_session->run();
