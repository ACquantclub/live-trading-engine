...
class WebsocketSession : public BaseWebsocketSession {
  public:
    WebsocketSession(net::io_context& ioc, Dictionary* properties, void_cb& on_open,
                     string_cb& on_fail, char_cb& on_message, void_cb& on_close,
    ...
    void do_read() override {
        derived().ws().async_read(m_buffer,
    ...
    void send(const std::string& s) override {
        net::post(derived().ws().get_executor(), [this, s]() {
            m_queue.push_back(s);
            if (m_queue.size() > 1)
                return;
...
class WebsocketSessionNoTLS final : public WebsocketSession<WebsocketSessionNoTLS> {
  public:
    WebsocketSessionNoTLS(net::io_context& ioc, Dictionary* properties, void_cb& on_open,
                          string_cb& on_fail, char_cb& on_message, void_cb& on_close,
                          string_cb& on_send_fail)
            ...
            [this](beast::error_code ec, tcp::resolver::results_type results) {
                ...
                beast::get_lowest_layer(m_ws).async_connect(
                    ...
                    [this](beast::error_code ec, tcp::resolver::results_type::endpoint_type ep) {
                        ...
                        std::string host_ = m_properties->get<std::string>("host") + ':' +
                        ...
                        m_ws.async_handshake(
                            ...
                            [this](beast::error_code ec) {
                                ...
                                m_on_open();
                                m_ws.async_read(m_buffer, [this](beast::error_code ec,
...
class WebsocketSessionTLS final : public WebsocketSession<WebsocketSessionTLS> {
  public:
    WebsocketSessionTLS(net::io_context& ioc, ssl::context& ctx, Dictionary* properties,
                        void_cb& on_open, string_cb& on_fail, char_cb& on_message,
                        void_cb& on_close, string_cb& on_send_fail)
        : WebsocketSession(ioc, properties, on_open, on_fail, on_message, on_close, on_send_fail),
            ...
            [this](beast::error_code ec, tcp::resolver::results_type results) {
                ...
                beast::get_lowest_layer(m_ws).async_connect(
                    ...
                    [this](beast::error_code ec, tcp::resolver::results_type::endpoint_type ep) {
                        ...
                        m_complete_host = m_properties->get<std::string>("host") + ':' +
                                          std::to_string(ep.port());
