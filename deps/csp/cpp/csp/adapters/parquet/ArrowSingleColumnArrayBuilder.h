...
class ArrowSingleColumnArrayBuilder {
  public:
    ...
    // Called for each row of the output parquet row, if no value was provided a null should be
    // appended
  ...
  public:
    ...
    StructColumnArrayBuilder(std::string columnName, std::uint32_t chunkSize,
        ...
        : ArrowSingleColumnArrayBuilder(columnName, chunkSize),
          ...
          m_builderPtr(std::make_shared<::arrow::StructBuilder>(
              type, arrow::default_memory_pool(), getArrowChildArrayBuilders(childArrayBuilders))),
...
class ListColumnArrayBuilder : public ArrowSingleColumnArrayBuilder {
  public:
    ...
    ListColumnArrayBuilder(std::string columnName, std::uint32_t chunkSize,
        ...
        : ArrowSingleColumnArrayBuilder(columnName, chunkSize),
          ...
          m_builderPtr(
              std::make_shared<::arrow::ListBuilder>(arrow::default_memory_pool(), m_valueBuilder)),
    ...
    virtual std::shared_ptr<arrow::Array> buildArray() override {
        std::shared_ptr<arrow::Array> array;
        STATUS_OK_OR_THROW_RUNTIME(m_builderPtr->Finish(&array),
                                   "Failed to create arrow list array");
        return array;
    }
  ...
  private:
    static std::shared_ptr<arrow::ArrayBuilder> createValueArrayBuilderForType(
        const std::shared_ptr<::arrow::DataType>& childType) {
        switch (childType->id()) {
...
class BaseTypedArrayBuilder : public ArrowSingleColumnArrayBuilder {
  public:
    ...
    BaseTypedArrayBuilder(std::string columnName, std::uint32_t chunkSize)
        ...
        CSP_TRUE_OR_THROW_RUNTIME(m_builderPtr->Reserve(getChunkSize()).ok(),
                                  "Failed to reserve arrow array size");
    }
    ...
    void handleRowFinished() override {
        if (m_value) {
            ...
            STATUS_OK_OR_THROW_RUNTIME(m_builderPtr->AppendNull(),
                                       "Failed to append null to arrow array");
        }
        m_value = nullptr;
    }
    ...
    std::shared_ptr<arrow::Array> buildArray() override {
        std::shared_ptr<arrow::Array> array;
        CSP_TRUE_OR_THROW_RUNTIME(m_builderPtr->Finish(&array).ok(),
                                  "Failed to create arrow array");
        return array;
    }
...
class [[maybe_unused]] CStringArrayBuilder
    : public BaseTypedArrayBuilder<const char*, arrow::StringBuilder> {
  public:
  ...
  protected:
    void pushValueToArray() {
        ...
        STATUS_OK_OR_THROW_RUNTIME(this->m_builderPtr->Append(value),
                                   "Failed to append value to arrow array");
    }
