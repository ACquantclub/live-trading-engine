...
class ParquetInputAdapterManager final : public csp::AdapterManager {
  public:
    ...
    const char* name() const override {
        return "ParquetInputAdapterManager";
    }
    ...
    ManagedSimInputAdapter* getInputAdapter(
  ...
  private:
    ...
    std::unique_ptr<ParquetReader> initializeParquetReader(
        const std::optional<std::string>& symbolColumn, const std::set<std::string>& neededColumns,
        const AdaptersBySymbol& adaptersBySymbol, bool subscribeAllOnEmptySymbol = true,
        bool nullOnEmpty = false) const;

    ManagedSimInputAdapter* getRegularAdapter(
    ...
    ManagedSimInputAdapter* getOrCreateSingleColumnAdapter(
        AdaptersBySymbol& inputAdaptersContainer, const CspTypePtr& type,
        const utils::Symbol& symbol, const std::string& field, const PushMode& pushMode,
        const DialectGenericListReaderInterface::Ptr& listReaderInterface = nullptr);
    ManagedSimInputAdapter* getSingleColumnAdapter(
