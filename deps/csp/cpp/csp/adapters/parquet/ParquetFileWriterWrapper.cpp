...
void ParquetFileWriterWrapper::openImpl(const std::string& fileName,
                                        const std::string& compression) {
    CSP_TRUE_OR_THROW_RUNTIME(m_outputStream == nullptr,
                              "Trying to open parquet file while previous was not closed");
    ...
    auto res =
        ::parquet::arrow::FileWriter::Open(*getSchema(), arrow::default_memory_pool(),
                                           m_outputStream, builder.build(), arrowBuilder.build());
...
void ParquetFileWriterWrapper::close() {
    if (m_outputStream) {
        ...
        if (outputStream)
            STATUS_OK_OR_THROW_RUNTIME(outputStream->Close(),
                                       "Failed to close parquet output stream");
    }
}
