...
SingleColumnParquetOutputHandler::SingleColumnParquetOutputHandler(Engine* engine,
    ...
    switch (m_type->type()) {
        ...
        default: {
            CSP_THROW(TypeError,
                      "Writing of " << m_type->type().asString() << " to parquet is not supported");
        }
    }
}
...
inline void SingleColumnParquetOutputHandler::createColumnBuilder(const std::string& columnName) {
    ...
    m_valueHandler = std::make_unique<ValueHandler>([this](const TimeSeriesProvider* input) {
...
ListColumnParquetOutputHandler::ListColumnParquetOutputHandler(
    Engine* engine, ParquetWriter& parquetWriter, CspTypePtr& elemType,
    const std::string& columnName, DialectGenericListWriterInterface::Ptr& listWriterInterface)
    : ParquetOutputHandler(parquetWriter, CspType::DIALECT_GENERIC()),
      m_columnArrayBuilder(std::make_shared<ListColumnArrayBuilder>(
          columnName, getChunkSize(), createValueBuilder(elemType, listWriterInterface),
          listWriterInterface)) {
    m_valueHandler = std::make_unique<ValueHandler>([this](const TimeSeriesProvider* input) {
...
inline std::shared_ptr<::arrow::ArrayBuilder> makeArrayAndAttachToWriter(
    ...
    CSP_TRUE_OR_THROW(typedWriter != nullptr, TypeError,
                      "Expected " << typeid(TypedDialectGenericListWriterInterface<V>).name() << " "
                                  << " got " << typeid(listWriterInterfaceRef).name());

    auto res = std::make_shared<A>();
    typedWriter->setWriteFunction([res](const V& value) {
...
std::shared_ptr<::arrow::ArrayBuilder> ListColumnParquetOutputHandler::createValueBuilder(
    const CspTypePtr& elemType, DialectGenericListWriterInterface::Ptr& listWriterInterface) {
    switch (elemType->type()) {
        ...
        case CspType::TypeTraits::STRING:
            return makeArrayAndAttachToWriter<arrow::StringBuilder, std::string>(
                listWriterInterface);
        default: {
            CSP_THROW(TypeError, "Writing of list with elements of type "
...
StructParquetOutputHandler::createColumnBuilder(const StructMeta* structMeta,
    ...
    switch (fieldPtr->type()->type()) {
        ...
        default:
            CSP_THROW(TypeError, "Writing of column " << columnName << " of type "
                                                      << fieldPtr->type()->type().asString()
                                                      << " to parquet is not supported");
    }
}
...
template <typename ColumnBuilder>
inline StructParquetOutputHandler::ColumnBuilderResultType
StructParquetOutputHandler::createColumnBuilder(const StructField* field,
                                                const std::string& columnName,
                                                const std::string* path) {
    ...
    // We need to collect on the top level value handlers and array builders. Value handlers and
    // builders of nested structs are stored in the struct field handlers. Path is non null in this
    // case.
    if (!path) {
...
inline StructParquetOutputHandler::ColumnBuilderResultType
StructParquetOutputHandler::createEnumColumnBuilder(const StructField* field,
                                                    const std::string& columnName,
                                                    const std::string* path) {
    auto columnBuilderPtr{std::make_shared<StringArrayBuilder>(
...
inline StructParquetOutputHandler::ColumnBuilderResultType
StructParquetOutputHandler::createStructColumnBuilder(const StructField* structField,
                                                      const std::string& columnName,
                                                      const std::string* path) {
    ...
    auto structFieldMetaPtr =
        std::static_pointer_cast<const CspStructType>(structField->type())->meta().get();

    for (auto& subField : structFieldMetaPtr->fields()) {
        std::string fieldSubPath = resolveFullColumnName(path, subField->fieldname());

        auto childBuilderRes = createColumnBuilder(structFieldMetaPtr, subField->fieldname(),
                                                   subField->fieldname(), &fieldSubPath);
        ...
        fields.push_back(std::make_shared<::arrow::Field>(
            subField->fieldname(), childBuilderRes.m_columnBuilder->getDataType()));
    }
    ...
    auto columnBuilderPtr{std::make_shared<StructColumnArrayBuilder>(
        resolveFullColumnName(path, columnName), getChunkSize(),
        std::make_shared<::arrow::StructType>(fields), childArrayBuilders,
