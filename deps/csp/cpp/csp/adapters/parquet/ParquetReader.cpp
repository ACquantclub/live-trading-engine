...
inline std::vector<FieldInfo> getFieldsInfo(
...
ParquetStructAdapter& ParquetReader::getStructAdapter(const StructAdapterInfo& structAdapterInfo) {
    ...
    if (it == m_structInfoToAdapterIndex.end()) {
        ...
        m_structAdapters.push_back(
            std::make_unique<ParquetStructAdapter>(*this, structAdapterInfo));
...
void ParquetReader::setSymbolColumnAdapter(ColumnAdapterReference adapter) {
    ...
    try {
        ...
        try {
            ...
            CSP_THROW(TypeError,
                      "Invalid symbol column type.  Only string and int64 symbols are currently "
                      "supported");
        }
    }
}
    ...
    for (const auto& columnName : m_columns) {
        ...
        if (existingRecordIt != m_columnNameToAdapterIndex.end()) {
            CSP_TRUE_OR_THROW_RUNTIME(existingRecordIt->second == m_columnAdapters.size(),
                                      "Unexpected index change of column "
                                          << columnName << " was " << existingRecordIt->second
                                          << " became " << m_columnAdapters.size());
        } else {
        ...
        if (index >= 0) {
            ...
            columnAdapter =
                createColumnAdapter(*this, *field, getCurFileOrTableName(), &getStructColumnMeta());
            ...
            CSP_TRUE_OR_THROW_RUNTIME(isAllowMissingColumns(), "Missing column "
                                                                   << columnName << " in file "
                                                                   << getCurFileOrTableName());
...
SingleFileParquetReader::SingleFileParquetReader(GeneratorPtr generatorPtr,
                                                 std::vector<std::string> columns, bool arrowIPC,
                                                 bool allowMissingColumns, bool allowMissingFiles,
                                                 std::optional<std::string> symbolColumnName)
...
bool SingleFileParquetReader::readNextRowGroup() {
    ...
    while (!endOfData) {
        ...
        while (m_fileReader->readNextRowGroup(m_neededColumnIndices, m_curTable) &&
               m_curTable->num_rows() == 0) {
        }
...
InMemoryTableParquetReader::InMemoryTableParquetReader(GeneratorPtr generatorPtr,
...
bool InMemoryTableParquetReader::openNextFile() {
    ...
    CSP_TRUE_OR_THROW_RUNTIME(table->num_columns() > 0,
                              "Provided in memory arrow table with 0 columns");
    schema = table->schema();
    ...
    for (int i = 0; i < table->num_columns(); ++i) {
        CSP_TRUE_OR_THROW_RUNTIME(
            table->column(i)->num_chunks() == refNumChunks,
        ...
        for (auto colIndex : m_neededColumnIndices) {
            ...
            CSP_TRUE_OR_THROW_RUNTIME(curArrayChunk->length() == refChunkLength,
                                      "Found in memory table with non aligned chunks, for chunk "
                                          << m_nextChunkIndex << "found arrays of lenght "
                                          << refChunkLength << " and " << curArrayChunk->length());
            ...
            STATUS_OK_OR_THROW_RUNTIME(newChunkedArrayResult.status(),
                                       "Failed to creae a new chunked array");
...
MultipleFileParquetReader::MultipleFileParquetReader(
    FileNameGeneratorReplicator::Ptr generatorReplicatorPtr, std::vector<std::string> columns,
    bool arrowIPC, bool allowMissingColumns, std::optional<std::string> symbolColumnName)
    ...
    for (auto&& column : columns) {
        m_columnReaders.push_back(std::make_unique<SingleFileParquetReader>(
            m_generatorReplicatorPtr->getGeneratorReplica(std::string("/") + '/' + column +
                                                          ".parquet"),
...
bool MultipleFileParquetReader::skipRow() {
    ...
    if (unlikely(successfulSkips != m_columnReaders.size())) {
        CSP_THROW(RuntimeException,
                  "Input files are not alligned - some columns have more data than the others");
    }
    return true;
}

void MultipleFileParquetReader::dispatchRow(bool doReadNextRow) {
    // By default dispatchRow of columnReaders will dispatch row and read the next row (clear out
    // the currently read row). If we have structAdapters, we can't move on to the next row before
    // we populate the struct adapter fields. So if any struct adapters exist, we need to postpone
    // reading of next row for child readers and explicitly call them after dispatching the struct
    // values.
