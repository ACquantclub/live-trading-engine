...
ParquetWriter::ParquetWriter(ParquetOutputAdapterManager* mgr,
                             std::optional<bool> writeTimestampColumn)
    : m_adapterMgr(*mgr),
      m_engine(mgr->engine()),
      m_curChunkSize(0),
...
ParquetWriter::ParquetWriter(ParquetOutputAdapterManager* mgr, const Dictionary& properties)
    ...
    if (properties.exists("column_metadata")) {
        ...
        for (auto colIt = column_metadata->begin(); colIt != column_metadata->end(); ++colIt) {
            ...
            if (!cmeta)
                CSP_THROW(TypeError,
                          "parquet column metadata expects dictionary entry per column, got "
                          "unrecognized type for column '"
                              << colIt.key() << "'");
            ...
            for (auto it = (*cmeta)->begin(); it != (*cmeta)->end(); ++it) {
                ...
                if (!value)
                    CSP_THROW(TypeError,
                              "parquet column metadata can only have string values, got non-string "
                              "value for metadata on column '"
                                  << colIt.key() << "'");
...
SingleColumnParquetOutputAdapter* ParquetWriter::getScalarOutputAdapter(
    CspTypePtr& type, const std::string& columnName) {
    auto res =
        static_cast<SingleColumnParquetOutputAdapter*>(getScalarOutputHandler(type, columnName));
    return res;
}

StructParquetOutputAdapter* ParquetWriter::getStructOutputAdapter(CspTypePtr& type,
                                                                  csp::DictionaryPtr fieldMap) {
    auto res = static_cast<StructParquetOutputAdapter*>(getStructOutputHandler(type, fieldMap));
    return res;
}

SingleColumnParquetOutputHandler* ParquetWriter::getScalarOutputHandler(
    CspTypePtr& type, const std::string& columnName) {
...
StructParquetOutputHandler* ParquetWriter::getStructOutputHandler(CspTypePtr& type,
                                                                  csp::DictionaryPtr fieldMap) {
...
ListColumnParquetOutputHandler* ParquetWriter::getListOutputHandler(
...
void ParquetWriter::start() {
    ...
    if (!m_writeTimestampColumn.has_value() && !m_adapterMgr.getTimestampColumnName().empty()) {
        ...
        m_columnBuilders.push_back(std::make_shared<DatetimeArrayBuilder>(
    ...
    if (!m_columnMetaData.empty())
        CSP_THROW(ValueError, "parquet column metadata has unmapped column: '"
                                  << m_columnMetaData.begin()->first << "'");
...
void ParquetWriter::onEndCycle() {
    if (likely(isFileOpen())) {
        // This must be defined outside of the "if" below, the datetime object must live till the
        // end of cycle since We pass all values by reference
        DateTime now;
...
void ParquetWriter::onFileNameChange(const std::string& fileName) {
    ...
    m_fileWriterWrapperContainer->close();
    if (!fileName.empty()) {
        m_fileWriterWrapperContainer->open(fileName, m_adapterMgr.getCompression(),
                                           m_adapterMgr.isAllowOverwrite());
    }
}

SingleColumnParquetOutputHandler* ParquetWriter::createScalarOutputHandler(
    CspTypePtr type, const std::string& name) {
    return m_engine->createOwnedObject<SingleColumnParquetOutputAdapter>(*this, type, name);
}

ListColumnParquetOutputHandler* ParquetWriter::createListOutputHandler(
    CspTypePtr& elemType, const std::string& columnName,
...
StructParquetOutputHandler* ParquetWriter::createStructOutputHandler(
    CspTypePtr type, const DictionaryPtr& fieldMap) {
...
void ParquetWriter::initFileWriterContainer(std::shared_ptr<arrow::Schema> schema) {
    if (m_adapterMgr.isSplitColumnsToFiles()) {
        m_fileWriterWrapperContainer = std::make_unique<MultipleFileWriterWrapperContainer>(
            schema, m_adapterMgr.isWriteArrowBinary());
    } else {
        m_fileWriterWrapperContainer = std::make_unique<SingleFileWriterWrapperContainer>(
            schema, m_adapterMgr.isWriteArrowBinary());
    }
    if (!m_adapterMgr.getFileName().empty()) {
        m_fileWriterWrapperContainer->open(m_adapterMgr.getFileName(),
                                           m_adapterMgr.getCompression(),
                                           m_adapterMgr.isAllowOverwrite());
    }
}
