...
ParquetInputAdapterManager::ParquetInputAdapterManager(csp::Engine* engine,
                                                       const Dictionary& properties,
                                                       GeneratorPtr generatorPtr,
                                                       TableGeneratorPtr tableGeneratorPtr)
    : AdapterManager(engine),
      m_fileNameGeneratorReplicator(
          generatorPtr ? std::make_shared<FileNameGeneratorReplicator>(generatorPtr) : nullptr),
      m_time_shift(0, 0),
...
void ParquetInputAdapterManager::start(DateTime starttime, DateTime endtime) {
    ...
    CSP_TRUE_OR_THROW_RUNTIME(m_reader == nullptr,
                              "Starting parquet adapter manager more than once");
    ...
    for (auto&& it : m_dictBasketInputAdapters) {
        ...
        record.m_reader =
            initializeParquetReader(basketSymbolColumn, {basketSymbolColumn}, it.second, false);
...
std::unique_ptr<ParquetReader> ParquetInputAdapterManager::initializeParquetReader(
    ...
    bool subscribeAllOnEmptySymbol, bool nullOnEmpty) const {
    ...
    if (m_splitColumnsToFiles) {
        ...
        reader.reset(new MultipleFileParquetReader(m_fileNameGeneratorReplicator, columns,
                                                   m_isArrowIPC, m_allowMissingColumns,
                                                   symbolColumn));
    } else {
        if (m_fileNameGeneratorReplicator) {
            reader.reset(new SingleFileParquetReader(
                m_fileNameGeneratorReplicator->getGeneratorReplica(), columns, m_isArrowIPC,
                m_allowMissingColumns, m_allowMissingFiles, symbolColumn));
        } else {
            reader.reset(new InMemoryTableParquetReader(m_tableGenerator, columns,
    ...
    if (!reader->isEmpty()) {
        for (auto& symMapPair : adaptersBySymbol) {
            ...
            for (auto& columnAdapterPair : symMapPair.second.m_adaptersByColumnName) {
                ...
                CSP_TRUE_OR_THROW_RUNTIME(
                    columnAdapter->isListType() || listReaderInterface == nullptr,
                    "Column " << columnAdapterPair.first
                              << " is a list is non list in parquet file "
                              << reader->getCurFileOrTableName()
                              << " while subscribing to it as list");

                if (columnAdapter->isListType()) {
                    reader->addListSubscriber(columnAdapterPair.first,
                                              columnAdapterPair.second.m_adapter, symbol,
                                              listReaderInterface);

                } else {
                    reader->addSubscriber(columnAdapterPair.first,
                                          columnAdapterPair.second.m_adapter, symbol);
                }
            }
            for (auto& structAdapterPair : symMapPair.second.m_structAdapters) {
                CSP_TRUE_OR_THROW_RUNTIME(
                    structAdapterPair.second.m_listReaderInterface == nullptr,
...
DateTime ParquetInputAdapterManager::processNextSimTimeSlice(DateTime time) {
    ...
    while (!nextDataTime.value().isNone() && nextDataTime.value() < data_reference_time) {
        for (auto&& dictBasketRecord : m_dictBasketReaders) {
            auto numValuesToSkip =
                dictBasketRecord.m_valueCountColumn->getCurValue<uint16_t>().value();
    ...
    CSP_TRUE_OR_THROW_RUNTIME(
        data_reference_time == nextDataTime,
        "Expected time " << nextDataTime.value() << " got " << data_reference_time);
    do {
        for (auto&& dictBasketRecord : m_dictBasketReaders) {
            auto numValuesToDispatch =
                dictBasketRecord.m_valueCountColumn->getCurValue<uint16_t>().value();
        ...
        nextDataTime = m_reader->hasData() ? m_timestampColumnAdapter->getCurValue<DateTime>()
                                           : DateTime::NONE();
...
ManagedSimInputAdapter* ParquetInputAdapterManager::getInputAdapter(
...
ManagedSimInputAdapter* ParquetInputAdapterManager::getRegularAdapter(
    const CspTypePtr& type, const Dictionary& properties, const PushMode& pushMode,
    ...
    if (pushMode == PushMode::NON_COLLAPSING) {
        ...
        // If we subscribe both by symbol and subscribe all, we might have weird issues. Consider
        // the following scenario: ts1: subscribe AAPL ts2: subscribe IBM ts3: subscribe_all Assume
        // that the file has 2 entries, both on the same timestamp, one for AAPL, one for IBM. Then
        // ts1 and ts2 will output on the first cycle the AAPL and IBM data, ts3 will output data
        // for AAPL only, and on next cycle will output the data for IBM. This behavior of same data
        // ticking on different cycles could create a lot of issues and generally buggy so we want
        // to avoid it and forbid subscribing both ways.
        CSP_TRUE_OR_THROW(!m_subscribedBySymbol || !m_subscribedForAll, NotImplemented,
                          "Subscribing both by symbol and without symbol for same parquet reader "
...
ManagedSimInputAdapter* ParquetInputAdapterManager::getDictBasketAdapter(
    const CspTypePtr& type, const Dictionary& properties, const PushMode& pushMode,
    const utils::Symbol& symbol, const std::string& basketName) {
    auto fieldMap = properties.getUntypedValue("field_map");
...
ManagedSimInputAdapter* ParquetInputAdapterManager::getOrCreateSingleColumnAdapter(
    ...
    const CspTypePtr& adapterType =
        (listReaderInterface == nullptr) ? type : CspType::DIALECT_GENERIC();

    if (itByColumn == itBySymbol->second.m_adaptersByColumnName.end()) {
        itByColumn =
            itBySymbol->second.m_adaptersByColumnName
                .emplace(field, AdapterInfo{engine()->createOwnedObject<ManagedSimInputAdapter>(
                                                adapterType, this, pushMode),
                                            listReaderInterface})
                .first;
    }
...
ManagedSimInputAdapter* ParquetInputAdapterManager::getSingleColumnAdapter(
    const CspTypePtr& type, const utils::Symbol& symbol, const std::string& field,
    PushMode pushMode, const DialectGenericListReaderInterface::Ptr& listReaderInterface) {
...
ManagedSimInputAdapter* ParquetInputAdapterManager::getOrCreateStructColumnAdapter(
    ...
    if (itByColumn == itBySymbol->second.m_structAdapters.end()) {
        itByColumn = itBySymbol->second.m_structAdapters
                         .emplace(key, engine()->createOwnedObject<ManagedSimInputAdapter>(
                                           type, this, pushMode))
                         .first;
    }
    return itByColumn->second.m_adapter;
...
ManagedSimInputAdapter* ParquetInputAdapterManager::getStructAdapter(
    const CspTypePtr& type, const utils::Symbol& symbol, const csp::DictionaryPtr& fieldMap,
    PushMode pushMode) {
