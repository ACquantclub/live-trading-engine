...
ParquetOutputAdapterManager::ParquetOutputAdapterManager(csp::Engine* engine,
...
OutputAdapter* ParquetOutputAdapterManager::getOutputAdapter(CspTypePtr& type,
                                                             const Dictionary& properties) {
...
OutputAdapter* ParquetOutputAdapterManager::getListOutputAdapter(
    CspTypePtr& elemType, const Dictionary& properties,
...
ParquetDictBasketOutputWriter* ParquetOutputAdapterManager::createDictOutputBasketWriter(
    const char* columnName, const CspTypePtr& cspTypePtr) {
    auto&& existingAdapterIt = m_dictBasketWriterIndexByName.find(columnName);
    CSP_TRUE_OR_THROW_RUNTIME(
        existingAdapterIt == m_dictBasketWriterIndexByName.end(),
...
OutputAdapter* ParquetOutputAdapterManager::getScalarOutputAdapter(CspTypePtr& type,
                                                                   const Dictionary& properties) {
    auto columnName = properties.get<std::string>("column_name");
...
OutputAdapter* ParquetOutputAdapterManager::getStructOutputAdapter(CspTypePtr& type,
                                                                   const Dictionary& properties) {
