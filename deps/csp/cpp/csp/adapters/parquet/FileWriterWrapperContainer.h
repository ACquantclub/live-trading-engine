...
class FileWriterWrapperContainer {
  public:
    ...
    virtual void writeData(
        const std::vector<std::shared_ptr<ArrowSingleColumnArrayBuilder>>& columnBuilders) = 0;
  ...
  protected:
    ...
    static WriterPtr createSingleFileWrapper(const std::shared_ptr<arrow::Schema>& schema,
...
class SingleFileWriterWrapperContainer final : public FileWriterWrapperContainer {
  public:
    SingleFileWriterWrapperContainer(std::shared_ptr<arrow::Schema> schema,
                                     bool isWriteArrowBinary);
    virtual void open(const std::string& fileName, const std::string& compression,
                      bool allowOverwrite = false) override;
    virtual void close() override;
    virtual void writeData(
        const std::vector<std::shared_ptr<ArrowSingleColumnArrayBuilder>>& columnBuilders) override;
...
class MultipleFileWriterWrapperContainer : public FileWriterWrapperContainer {
  public:
    MultipleFileWriterWrapperContainer(std::shared_ptr<arrow::Schema> schema,
                                       bool isWriteArrowBinary);
    virtual void open(const std::string& fileName, const std::string& compression,
                      bool allowOverwrite = false) override;
    virtual void close() override;
