...
void ArrowIPCFileWriterWrapper::openImpl(const std::string& fileName,
    ...
    STATUS_OK_OR_THROW_RUNTIME(
        ::arrow::ipc::MakeStreamWriter(m_outputStream.get(), getSchema(), writeOptions)
            .Value(&m_fileWriter),
    ...
    if (m_outputStream) {
        ...
        if (outputStream)
            STATUS_OK_OR_THROW_RUNTIME(outputStream->Close(),
                                       "Failed to close arrow output stream");
    }
}
