...
FileWriterWrapperContainer::WriterPtr FileWriterWrapperContainer::createSingleFileWrapper(
    const std::shared_ptr<arrow::Schema>& schema, bool isWriteArrowBinary) {
...
SingleFileWriterWrapperContainer::SingleFileWriterWrapperContainer(
    std::shared_ptr<arrow::Schema> schema, bool isWriteArrowBinary)
    : m_fileWriterWrapper(createSingleFileWrapper(schema, isWriteArrowBinary)) {
}

void SingleFileWriterWrapperContainer::SingleFileWriterWrapperContainer::open(
    const std::string& fileName, const std::string& compression, bool allowOverwrite) {
...
MultipleFileWriterWrapperContainer::MultipleFileWriterWrapperContainer(
    std::shared_ptr<arrow::Schema> schema, bool isWriteArrowBinary) {
...
void MultipleFileWriterWrapperContainer::open(const std::string& fileName,
                                              const std::string& compression, bool allowOverwrite) {
    for (auto&& record : m_fileWriterWrappers) {
        record.m_fileWriterWrapper->open(fileName + '/' + record.m_fileName, compression,
                                         allowOverwrite);
    }
    setOpen(true);
}
...
void MultipleFileWriterWrapperContainer::writeData(
    ...
    CSP_TRUE_OR_THROW_RUNTIME(
        columnBuilders.size() == m_fileWriterWrappers.size(),
