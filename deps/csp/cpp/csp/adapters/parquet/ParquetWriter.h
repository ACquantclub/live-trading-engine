...
class ParquetWriter : public EndCycleListener {
  public:
    ...
    SingleColumnParquetOutputAdapter* getScalarOutputAdapter(CspTypePtr& type,
                                                             const std::string& columnName);
    StructParquetOutputAdapter* getStructOutputAdapter(CspTypePtr& type,
                                                       csp::DictionaryPtr fieldMap);
    ListColumnParquetOutputAdapter* getListOutputAdapter(
        CspTypePtr& elemType, const std::string& columnName,
        const DialectGenericListWriterInterface::Ptr& listWriterInterface);

    SingleColumnParquetOutputHandler* getScalarOutputHandler(CspTypePtr& type,
                                                             const std::string& columnName);
    StructParquetOutputHandler* getStructOutputHandler(CspTypePtr& type,
                                                       csp::DictionaryPtr fieldMap);
    ListColumnParquetOutputHandler* getListOutputHandler(
        CspTypePtr& elemType, const std::string& columnName,
        const DialectGenericListWriterInterface::Ptr& listWriterInterface);
    ...
    std::uint32_t getChunkSize() const {
        return m_adapterMgr.getBatchSize();
    }
  ...
  protected:
    virtual SingleColumnParquetOutputHandler* createScalarOutputHandler(CspTypePtr type,
                                                                        const std::string& name);
    virtual ListColumnParquetOutputHandler* createListOutputHandler(
        CspTypePtr& elemType, const std::string& columnName,
        DialectGenericListWriterInterface::Ptr listWriterInterface);
    virtual StructParquetOutputHandler* createStructOutputHandler(CspTypePtr type,
