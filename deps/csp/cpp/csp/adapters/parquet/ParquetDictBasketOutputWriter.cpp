...
ParquetDictBasketOutputWriter::ParquetDictBasketOutputWriter(
    ParquetOutputAdapterManager* outputAdapterManager, const std::string& columnName)
    ...
    m_cycleIndexOutputAdapter =
        createScalarOutputHandler(CspType::UINT16(), columnName + "__csp_value_count");
}
...
void ParquetDictBasketOutputWriter::start() {
    ...
    m_indexFileWriterContainer = std::make_unique<MultipleFileWriterWrapperContainer>(
        arrow::schema(
            {arrow::field(m_cycleIndexOutputAdapter->getColumnArrayBuilder(0)->getColumnName(),
...
void ParquetDictBasketOutputWriter::writeValue(const std::string& valueKey,
...
void ParquetDictBasketOutputWriter::onFileNameChange(const std::string& fileName) {
    ...
    if (m_cycleIndexOutputAdapter->getColumnArrayBuilder(0)->length() > 0) {
        ...
        m_indexFileWriterContainer->writeData(
            {m_cycleIndexOutputAdapter->getColumnArrayBuilder(0)});
    }
    if (m_indexFileWriterContainer->isOpen()) {
        m_indexFileWriterContainer->close();
    }
    if (!fileName.empty()) {
        m_indexFileWriterContainer->open(fileName, m_adapterMgr.getCompression(),
                                         m_adapterMgr.isAllowOverwrite());
    }
}

SingleColumnParquetOutputHandler* ParquetDictBasketOutputWriter::createScalarOutputHandler(
    CspTypePtr type, const std::string& name) {
    m_allHandlers.push_back(
        std::make_unique<SingleColumnParquetOutputHandler>(m_engine, *this, type, name));
    return static_cast<SingleColumnParquetOutputHandler*>(m_allHandlers.back().get());
}

StructParquetOutputHandler* ParquetDictBasketOutputWriter::createStructOutputHandler(
    CspTypePtr type, const DictionaryPtr& fieldMap) {
    m_allHandlers.push_back(
        std::make_unique<StructParquetOutputHandler>(m_engine, *this, type, fieldMap));
...
void ParquetScalarDictBasketOutputWriter::writeValue(const std::string& valueKey,
                                                     const TimeSeriesProvider* ts) {
...
void ParquetStructDictBasketOutputWriter::writeValue(const std::string& valueKey,
                                                     const TimeSeriesProvider* ts) {
    m_valueOutputAdapter->writeValueFromTs(ts);
    ParquetDictBasketOutputWriter::writeValue(valueKey, ts);
}
