...
class DialectGenericListReaderInterface {
  public:
    ...
    /**
     * This form of creation is needed for elements that need to know the element size in advance,
     * such as numpy arrays of strings
     * @param size
     * @return
     */
...
class TypedDialectGenericListReaderInterface : public DialectGenericListReaderInterface {
  public:
    ...
    CspTypePtr getValueType() override {
        return CspType::fromCType<T>::type();
    }

    /**
     * Return a raw data buffer for the object that was generated using the call to create. Should
     * return the pointer to internal buffer that can be written directly into if that's supported,
     * otherwise, should return nullptr in which case setValue will be used for inserting the
     * elements
     * @param list: The list object that was created using the call to create function
     * @return Internal write buffer of the list of nullptr if the list doesn't support writing to
     * internal buffer
     */
    virtual T* getRawDataBuffer(const csp::DialectGenericType& list) const {
        return nullptr;
    }
