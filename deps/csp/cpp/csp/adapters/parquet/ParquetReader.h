...
/**
 * Wrapper of "GeneratorPtr". Generator is a general a single generator that provides a sequence of
 * filenames (or folder names) from which the files should be read. Since we might have multiple
 * file readers that need the same sequence of folders, this class wraps the generator and is able
 * to create new generators that will produce exactly the same sequence as the original one. Another
 * "service" that this class provides is that it can append suffix to a given generator replica. For
 * example consider input folders sequence of :
 *  - "folder1"
 *  - "folder2".
 * We can ask for a suffix of "/column1.parquet" when calling to getGeneratorReplica and the replica
 * will produce:
...
class FileNameGeneratorReplicator {
  public:
    ...
    FileNameGeneratorReplicator(GeneratorPtr source) : m_generatorPtr(source) {
    }
...
// When we read multiple files with different schemas, column adapters may change, the adapter
// reference will persist and still be valid.
class ColumnAdapterReference {
  public:
    ...
    ParquetColumnAdapter* get() const {
        return &(**this);
    };
...
class ParquetReader {
  public:
    ...
    ParquetReader(std::optional<std::string> symbolColumnName, bool arrowIPC,
    ...
    // Check if the reader can't read any data at all (no input files). In that case even the file
    // schema can't be deduces
    virtual bool isEmpty() const {
        return false;
    }
    // Check if the reader has any more data left to read. Note it's different from "isEmpty",
    // isEmpty, will always return the same value, while this function will return false only when
    // all data that could be read is already read. This function should be called only after start,
    // before that it is not expected to be valid.
    virtual bool hasData() const = 0;

    ParquetStructAdapter& getStructAdapter(
        const csp::adapters::utils::StructAdapterInfo& structAdapterInfo);
    ...
    virtual void addListSubscriber(
        const std::string& column, ManagedSimInputAdapter* inputAdapter,
        const std::optional<utils::Symbol>& symbol,
    ...
    /**
     * Add parquet struct adapter that depends on columns from "this" and needs to be updated if the
     * schema of the read files changes.
     * @param adapter
     */
    void addDependentStructAdapter(ParquetStructAdapter* adapter) {
        m_dependentAdapters.insert(adapter);
    }

    /**
     * Set a known schema of a struct column. Any future files that will have the column will
     * automatically bind to the given csp struct metadata
     * @param columnName
     * @param metaData
     */
    void setStructColumnMeta(const std::string& columnName,
                             const std::shared_ptr<StructMeta>& metaData) {
        m_structMetaByColumnName[columnName] = metaData;
    }

    /**
     * @return The known mappings of struct columns to the corresponding csp metadata. NOTE: the
     * values might not be set for all struct columns, currently the values are set only for struct
     * columns that are used as fields inside a wrapping struct (i.e only for columns which are user
     * as substructs).
     */
  ...
  protected:
    ...
    ColumnAdapterReference& getSymbolColumnAdapter() {
        return m_symbolColumn;
    }

    const std::optional<std::string>& getSymbolColumnName() const {
        return m_symbolColumnName;
    }
    const std::set<ParquetStructAdapter*>& getDependentAdapters() const {
        return m_dependentAdapters;
    }
...
class SingleTableParquetReader : public ParquetReader {
  public:
    ...
    SingleTableParquetReader(std::vector<std::string> columns, bool arrowIPC,
                             bool allowMissingColumns,
    ...
    int64_t getCurRow() const override {
        return m_curTableNextRow;
    }
  ...
  protected:
    ...
    virtual void clear();
    void resubscribeAll() {
        for (auto&& column_vector_pair :
             m_columnSubscriptionContainer.m_scalarColumnSubscriptions) {
            for (auto&& record : column_vector_pair.second) {
                // We need to call the base class subscribe since we don't want on resubscription to
                // add columns again to the container
                ParquetReader::addSubscriber(column_vector_pair.first, record.m_inputAdapter,
                                             record.m_symbol);
            }
        }
        for (auto&& column_vector_pair : m_columnSubscriptionContainer.m_listColumnSubscriptions) {
            for (auto&& record : column_vector_pair.second) {
                // We need to call the base class subscribe since we don't want on resubscription to
                // add columns again to the container
                ParquetReader::addListSubscriber(column_vector_pair.first, record.m_inputAdapter,
                                                 record.m_symbol, record.listReader);
            }
        }
...
class SingleFileParquetReader final : public SingleTableParquetReader {
  public:
    ...
    SingleFileParquetReader(GeneratorPtr generatorPtr, std::vector<std::string> columns,
                            bool arrowIPC, bool allowMissingColumns, bool allowMissingFiles = false,
                            std::optional<std::string> symbolColumnName = {});

    std::string getCurFileOrTableName() const override {
        return m_fileName;
    }
...
class InMemoryTableParquetReader final : public SingleTableParquetReader {
  public:
    using GeneratorPtr =
        csp::Generator<std::shared_ptr<arrow::Table>, csp::DateTime, csp::DateTime>::Ptr;
