...
class ParquetOutputHandler {
  public:
    ...
    virtual std::shared_ptr<ArrowSingleColumnArrayBuilder> getColumnArrayBuilder(
        unsigned index) = 0;
...
class SingleColumnParquetOutputHandler : public ParquetOutputHandler {
  public:
    ...
    uint32_t getNumColumns() override {
        return 1;
    }
...
class ListColumnParquetOutputHandler : public ParquetOutputHandler {
  public:
    ListColumnParquetOutputHandler(Engine* engine, ParquetWriter& parquetWriter,
  ...
  private:
    std::shared_ptr<arrow::ArrayBuilder> createValueBuilder(
...
class SingleColumnParquetOutputAdapter final : public OutputAdapter,
                                               public SingleColumnParquetOutputHandler {
  public:
    ...
    const char* name() const override {
        return "ParquetSingleColumnOutputAdapter";
    }
...
class ListColumnParquetOutputAdapter : public OutputAdapter, public ListColumnParquetOutputHandler {
  public:
    ListColumnParquetOutputAdapter(Engine* engine, ParquetWriter& parquetWriter, CspTypePtr& type,
                                   ...
                                   DialectGenericListWriterInterface::Ptr listWriterInterface)
        : OutputAdapter(engine),
          ListColumnParquetOutputHandler(engine, parquetWriter, type, columnName,
                                         listWriterInterface) {
    }

    const char* name() const override {
        return "ListColumnParquetOutputAdapter";
    }
...
class StructParquetOutputHandler : public ParquetOutputHandler {
  ...
  private:
    ...
    inline std::string resolveFullColumnName(const std::string* path,
                                             const std::string& columnName) {
...
class StructParquetOutputAdapter final : public OutputAdapter, public StructParquetOutputHandler {
  public:
    ...
    const char* name() const override {
        return "ParquetStructOutputAdapter";
    }
