...
std::unordered_map<std::string, ::arrow::Compression::type>
    FileWriterWrapper::m_compressionNameMapping{{"", ::arrow::Compression::UNCOMPRESSED},
...
void FileWriterWrapper::open(const std::string& fileName, const std::string& compression,
                             bool allowOverwrite) {
    if (!allowOverwrite) {
...
::arrow::Compression::type FileWriterWrapper::resolveCompression(const std::string& compression) {
    ...
    CSP_TRUE_OR_THROW_RUNTIME(it != m_compressionNameMapping.end(),
                              "Unable to resolve compression: " << compression);
    return it->second;
}

std::unique_ptr<arrow::util::Codec> FileWriterWrapper::resolveCompressionCodec(
    const std::string& compression) {
    auto compressionType = resolveCompression(compression);
    auto res = arrow::util::Codec::Create(compressionType);
    STATUS_OK_OR_THROW_RUNTIME(res.status(),
                               "Failed to create arrow codec for " << compressionType);
