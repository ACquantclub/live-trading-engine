...
static inline std::unique_ptr<ParquetColumnAdapter> createDateColumnAdapter(
    ParquetReader& reader, const std::string& columnName,
    ...
    switch (dateType->unit()) {
        ...
        case arrow::DateUnit::DAY:
            return std::unique_ptr<ParquetColumnAdapter>(
                new DateColumnAdapter<1000000000LL * 3600 * 24, ArrowArrayType>(reader,
                                                                                columnName));
    }
    CSP_THROW(csp::TypeError,
              "Unexpected day unit: " << (int)dateType->unit() << " for column " << columnName);
}
...
static inline std::unique_ptr<ParquetColumnAdapter> createListColumnAdapter(
    ParquetReader& parquetReader, const ::arrow::Field& field, const std::string& fileName) {
    auto listType = std::static_pointer_cast<arrow::ListType>(field.type());
    auto valueType = listType->value_type();
    switch (valueType->id()) {
        case arrow::Type::INT64:
            return std::make_unique<NativeListColumnAdapter<ArrowListArrayType, arrow::Int64Array>>(
                parquetReader, field.name());
        case arrow::Type::DOUBLE:
            return std::make_unique<
                NativeListColumnAdapter<ArrowListArrayType, arrow::DoubleArray>>(parquetReader,
                                                                                 field.name());
        case arrow::Type::BOOL:
            return std::make_unique<
                NativeListColumnAdapter<ArrowListArrayType, arrow::BooleanArray>>(parquetReader,
                                                                                  field.name());
        case arrow::Type::STRING:
            return std::make_unique<BytesListColumnAdapter<ArrowListArrayType, arrow::StringArray>>(
                parquetReader, field.name());
        case arrow::Type::BINARY:
            return std::make_unique<BytesListColumnAdapter<ArrowListArrayType, arrow::BinaryArray>>(
                parquetReader, field.name());
        case arrow::Type::LARGE_STRING:
            return std::make_unique<
                BytesListColumnAdapter<ArrowListArrayType, arrow::LargeStringArray>>(parquetReader,
                                                                                     field.name());
        case arrow::Type::LARGE_BINARY:
            return std::make_unique<
                BytesListColumnAdapter<ArrowListArrayType, arrow::LargeBinaryArray>>(parquetReader,
                                                                                     field.name());
        default:
            CSP_THROW(TypeError,
                      "Trying to create arrow list array reader for unsupported element type "
                          << valueType->name() << " for column " << field.name() << " in file "
                          << fileName);
    }
}
...
std::unique_ptr<ParquetColumnAdapter> createColumnAdapter(
    ...
    switch (typeId) {
        case arrow::Type::BOOL:
            return std::unique_ptr<ParquetColumnAdapter>(
                new NativeTypeColumnAdapter<bool, arrow::BooleanArray>(parquetReader,
                                                                       field.name()));
        case arrow::Type::UINT8:
            return std::unique_ptr<ParquetColumnAdapter>(
                new NativeTypeColumnAdapter<std::uint8_t, arrow::UInt8Array>(parquetReader,
                                                                             field.name()));
        case arrow::Type::INT8:
            return std::unique_ptr<ParquetColumnAdapter>(
                new NativeTypeColumnAdapter<std::int8_t, arrow::Int8Array>(parquetReader,
                                                                           field.name()));
        case arrow::Type::UINT16:
            return std::unique_ptr<ParquetColumnAdapter>(
                new NativeTypeColumnAdapter<std::uint16_t, arrow::UInt16Array>(parquetReader,
                                                                               field.name()));
        case arrow::Type::INT16:
            return std::unique_ptr<ParquetColumnAdapter>(
                new NativeTypeColumnAdapter<std::int16_t, arrow::Int16Array>(parquetReader,
                                                                             field.name()));
        case arrow::Type::UINT32:
            return std::unique_ptr<ParquetColumnAdapter>(
                new NativeTypeColumnAdapter<std::uint32_t, arrow::UInt32Array>(parquetReader,
                                                                               field.name()));
        case arrow::Type::INT32:
            return std::unique_ptr<ParquetColumnAdapter>(
                new NativeTypeColumnAdapter<std::int32_t, arrow::Int32Array>(parquetReader,
                                                                             field.name()));
        case arrow::Type::UINT64:
            return std::unique_ptr<ParquetColumnAdapter>(
                new NativeTypeColumnAdapter<std::uint64_t, arrow::UInt64Array>(parquetReader,
                                                                               field.name()));
        case arrow::Type::INT64:
            return std::unique_ptr<ParquetColumnAdapter>(
                new NativeTypeColumnAdapter<std::int64_t, arrow::Int64Array>(parquetReader,
                                                                             field.name()));
        case arrow::Type::HALF_FLOAT:
            return std::unique_ptr<ParquetColumnAdapter>(
                new NativeTypeColumnAdapter<double, arrow::HalfFloatArray>(parquetReader,
                                                                           field.name()));
        case arrow::Type::FLOAT:
            return std::unique_ptr<ParquetColumnAdapter>(
                new NativeTypeColumnAdapter<double, arrow::FloatArray>(parquetReader,
                                                                       field.name()));
        case arrow::Type::DOUBLE:
            return std::unique_ptr<ParquetColumnAdapter>(
                new NativeTypeColumnAdapter<double, arrow::DoubleArray>(parquetReader,
                                                                        field.name()));
        case arrow::Type::STRING:
            return std::unique_ptr<ParquetColumnAdapter>(
                new StringColumnAdapter<arrow::StringArray>(parquetReader, field.name()));
        case arrow::Type::BINARY:
            return std::unique_ptr<ParquetColumnAdapter>(
                new BytesColumnAdapter<arrow::BinaryArray>(parquetReader, field.name()));
        case arrow::Type::DATE32:
            return createDateColumnAdapter<arrow::Date32Array>(
                parquetReader, field.name(),
                std::static_pointer_cast<arrow::Date32Type>(field.type()));
        case arrow::Type::DATE64:
            return createDateColumnAdapter<arrow::Date64Array>(
                parquetReader, field.name(),
                std::static_pointer_cast<arrow::Date64Type>(field.type()));
        ...
        case arrow::Type::TIMESTAMP: {
            ...
            //                    timestampType -> timezone().empty() || timestampType -> timezone()
            //                    == "UTC" || timestampType -> timezone() == "utc", "Unexpected
            //                    parquet column timezone: " << timestampType -> timezone() );
        ...
        case arrow::Type::DICTIONARY: {
            ...
            if (dictionaryType->value_type()->id() != arrow::Type::STRING) {
                CSP_THROW(ParquetColumnTypeError, "Unsupported dictionary column type "
                                                      << field.type()->name() << " in file "
                                                      << fileName
        ...
        case arrow::Type::STRUCT: {
            auto res = std::make_unique<StructColumnAdapter>(
                parquetReader, std::static_pointer_cast<::arrow::StructType>(field.type()),
                field.name());
        ...
        case arrow::Type::LARGE_STRING:
            return std::unique_ptr<ParquetColumnAdapter>(
                new StringColumnAdapter<arrow::LargeStringArray>(parquetReader, field.name()));
        case arrow::Type::LARGE_BINARY:
            return std::unique_ptr<ParquetColumnAdapter>(
                new BytesColumnAdapter<arrow::LargeBinaryArray>(parquetReader, field.name()));
        case arrow::Type::LARGE_LIST:
            return createListColumnAdapter<arrow::LargeListArray>(parquetReader, field, fileName);
        default:
            CSP_THROW(ParquetColumnTypeError, "Unsupported column type "
                                                  << field.type()->name() << " for column "
                                                  << field.name() << " in file " << fileName);
    }
}
...
void BaseTypedColumnAdapter<ValueType, ArrowArrayType, ValueDispatcherT>::dispatchValue(
...
template <typename ValueType, typename ArrowArrayType, typename ValueDispatcherT>
void BaseTypedColumnAdapter<ValueType, ArrowArrayType, ValueDispatcherT>::ensureType(
    CspType::Ptr cspType) {
    try {
        CompatibleTypeSwitch::invoke(cspType.get(), [cspType, this](auto tag) {
            // No need to check native types here, they won't be assignable so it will raise
            // exception.
            if (cspType->type() > CspType::Type::MAX_NATIVE_TYPE) {
                CSP_TRUE_OR_THROW(
                    (std::is_same<ValueType, typename decltype(tag)::type>::value), TypeError,
        ...
        CSP_THROW(TypeError, "Unexpected column type for column "
                                 << getColumnName() << " , expected " << cspType->type().asCString()
...
void BaseTypedColumnAdapter<ValueType, ArrowArrayType, ValueDispatcherT>::addSubscriber(
    ...
    try {
        auto callback =
            CompatibleTypeSwitch::invoke(inputAdapter->type(), [inputAdapter](auto tag) {
                return std::function<void(const ValueType*)>([inputAdapter](const ValueType* val) {
                    if (val) {
...
ParquetStructAdapter::ParquetStructAdapter(ParquetReader& parquetReader,
    ...
    // Note the columns for single struct can come from multiple different readers, so we have to
    // iterate over all columns. MultipleFileParquetReader uses the columns from child
    // SingleFileParquetReader readers.
    for (auto it = adapterInfo.fieldMap()->begin(); it != adapterInfo.fieldMap()->end(); ++it) {
        ...
        if (fieldPtr->type()->type() == CspType::TypeTraits::STRUCT) {
            auto& fieldMeta =
                std::static_pointer_cast<const CspStructType>(fieldPtr->type())->meta();
            columnReader.setStructColumnMeta(parquetColumnName, fieldMeta);
        }
    }
}

ParquetStructAdapter::ParquetStructAdapter(
    ParquetReader& parquetReader, std::shared_ptr<::arrow::StructType> arrowType,
    const std::shared_ptr<StructMeta>& structMeta,
    const std::vector<std::unique_ptr<ParquetColumnAdapter>>& columnAdapters)
    : m_parquetReader(parquetReader), m_structMeta(structMeta) {
    m_resetFunc = []() {
    ...
    if (fieldPtr->type()->type() == CspType::TypeTraits::ENUM) {
        ...
        fieldSetter = [&columnAdapter, fieldPtr, enumMetaPtr](StructPtr& s) {
            ...
            if (curValue.has_value()) {
                fieldPtr->setValue<CspEnum>(s.get(),
                                            enumMetaPtr->fromString(curValue.value().c_str()));
            }
        };
    } else {
        ...
        if (columnAdapter.isNativeType()) {
            fieldSetter = NativeCspTypeSwitch::invoke(
                ...
                [&columnAdapter, &fieldPtr](auto columnTag) {
                    ...
                    return ConstructibleTypeSwitch<ColType>::invoke(
                        fieldPtr->type().get(), [&columnAdapter, &fieldPtr](auto tag) {
                            ...
                            return FieldSetter([&columnAdapter, fieldPtr](StructPtr& s) {
                                ...
                                if (curValue.has_value()) {
                                    fieldPtr->setValue<FieldType>(
                                        s.get(), csp::cast<FieldType>(curValue.value()));
                                }
                            });
                        });
                }
            ...
            fieldSetter =
                TypeSwitch::invoke(fieldPtr->type().get(), [&columnAdapter, fieldPtr](auto tag) {
                    using FieldType = typename decltype(tag)::type;
                    return FieldSetter([&columnAdapter, fieldPtr](StructPtr& s) {
...
void ParquetStructAdapter::addSubscriber(ManagedSimInputAdapter* inputAdapter,
                                         std::optional<utils::Symbol> symbol) {
    CSP_TRUE_OR_THROW(
        inputAdapter->type()->type() == CspType::Type::STRUCT, TypeError,
...
void ParquetStructAdapter::dispatchValue(const utils::Symbol* symbol, bool isNull) {
    ...
    auto symbolSubscribers =
        symbol != nullptr ? m_valueDispatcher.getSubscribersForSymbol(*symbol) : nullptr;
...
void StringColumnAdapter<ArrowStringArrayType>::addSubscriber(ManagedSimInputAdapter* inputAdapter,
                                                              std::optional<utils::Symbol> symbol) {
    if (inputAdapter->type()->type() == CspType::TypeTraits::ENUM) {
        ...
        BaseTypedColumnAdapter<std::string, ArrowStringArrayType>::addSubscriber(inputAdapter,
                                                                                 symbol);
    }
}
...
struct ArrayValidValueProvider {
    ...
    static T getValue(const V& array, int index) {
        if (!array->IsValid(index)) {
            CSP_THROW(ValueError, "Can't read empty value to array from arrow array of type "
...
void BaseListColumnAdapter<ArrowListArrayType, ValueArrayType, ValueType>::addSubscriber(
    ManagedSimInputAdapter* inputAdapter, std::optional<utils::Symbol> symbol) {
...
void BaseListColumnAdapter<ArrowListArrayType, ValueArrayType, ValueType>::addSubscriber(
    ...
    BaseTypedColumnAdapter<DialectGenericType, ArrowListArrayType>::addSubscriber(inputAdapter,
                                                                                  symbol);

    m_listReader =
        std::dynamic_pointer_cast<TypedDialectGenericListReaderInterface<ValueType>>(listReader);
    CSP_TRUE_OR_THROW_RUNTIME(m_listReader != nullptr,
                              "Subscribed to parquet column "
                                  << getColumnName() << " with type " << "NumpyArray["
                                  << listReader->getValueType()->type().asString() << "] while "
...
void NativeListColumnAdapter<ArrowListArrayType, ValueArrayType, ValueType>::readCurValue() {
    ...
    if (this->m_curChunkArray->IsValid(curRow)) {
        ...
        if (internalBuffer != nullptr) {
            ...
            for (int64_t i = 0; i < typedValues->length(); ++i) {
                m_listReader->setValue(
                    arrayValue, i, ArrayValidValueProvider<ValueType>::getValue(typedValues, i));
            }
        }
...
void BytesListColumnAdapter<ArrowListArrayType, ArrowBytesArrayType>::readCurValue() {
    ...
    if (this->m_curChunkArray->IsValid(curRow)) {
        ...
        for (int64_t i = 0; i < typedValues->length(); ++i) {
            maxStringLength = std::max(
                (uint32_t)ArrayValidValueProvider<std::string_view>::getValue(typedValues, i)
                    .length(),
                maxStringLength);
        }
...
void StructColumnAdapter::addSubscriber(ManagedSimInputAdapter* inputAdapter,
                                        std::optional<utils::Symbol> symbol) {
    CSP_TRUE_OR_THROW_RUNTIME(
        inputAdapter->type()->type() == CspType::TypeTraits::STRUCT,
...
void StructColumnAdapter::readCurValue() {
    // TODO: In the future we should make optimization here and skip creating structs for symbols
    // for which there's no subscription
    auto curRow = this->m_parquetReader.getCurRow();
    if (this->m_curChunkArray->IsValid(curRow)) {
...
void StructColumnAdapter::handleNewBatch(const std::shared_ptr<::arrow::ChunkedArray>& data) {
    ...
    CSP_TRUE_OR_THROW_RUNTIME(
        childArrays.size() == m_childColumnAdapters.size(),
...
void StructColumnAdapter::initFromStructMeta(const std::shared_ptr<StructMeta>& structMeta) {
    if (m_structAdapter) {
        CSP_TRUE_OR_THROW_RUNTIME(m_structAdapter->getStructMeta() == structMeta,
                                  "Trying to subscribe to structure field with struct "
                                      << structMeta->name() << " and "
                                      << m_structAdapter->getStructMeta()->name());
        return;
    }
    ...
    m_structAdapter = std::make_unique<ParquetStructAdapter>(m_parquetReader, m_arrowType,
                                                             structMeta, m_childColumnAdapters);
