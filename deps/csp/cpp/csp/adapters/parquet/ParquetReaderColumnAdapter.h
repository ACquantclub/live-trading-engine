...
class ParquetColumnAdapter {
  public:
    ParquetColumnAdapter(ParquetReader& parquetReader, const std::string& columnName)
        : m_parquetReader(parquetReader), m_columnName(columnName) {
    }
    ...
    virtual void addSubscriber(ManagedSimInputAdapter* inputAdapter,
                               ...
                               const DialectGenericListReaderInterface::Ptr& listReader) = 0;
    ...
    ParquetReader& getReader() {
        return m_parquetReader;
    }

    const ParquetReader& getReader() const {
        return m_parquetReader;
    }

    virtual bool isNativeType() const {
        return getNativeCspType() &&
               getNativeCspType()->type() < CspType::TypeTraits::MAX_NATIVE_TYPE;
    }
    virtual bool isMissingColumn() const {
        return false;
    }
...
class ParquetStructAdapter final {
  public:
    ...
    void addSubscriber(ManagedSimInputAdapter* inputAdapter,
                       std::optional<utils::Symbol> symbol = {});
    void addSubscriber(ValueDispatcher::SubscriberType subscriber,
                       std::optional<utils::Symbol> symbol = {});

    void dispatchValue(const utils::Symbol* symbol, bool isNull = false);

    const std::shared_ptr<StructMeta>& getStructMeta() {
        return m_structMeta;
    }
...
class MissingColumnAdapter : public ParquetColumnAdapter {
  public:
    ...
    virtual void addSubscriber(ManagedSimInputAdapter* inputAdapter,
                               std::optional<utils::Symbol> symbol,
                               const DialectGenericListReaderInterface::Ptr& listReader) override {
    };
    ...
    bool isNativeType() const override {
        CSP_THROW(csp::RuntimeException,
                  "Trying to check type of a missing column " << getColumnName());
    }

    bool isMissingColumn() const override {
        return true;
    }

    virtual CspTypePtr getNativeCspType() const override {
        CSP_THROW(csp::RuntimeException,
                  "Trying to get native type of a missing column " << getColumnName());
    }

    virtual void handleNewBatch(const std::shared_ptr<::arrow::ChunkedArray>& data) override {
        CSP_THROW(csp::RuntimeException,
                  "Trying to handle new batch for a missing column " << getColumnName());
    }
  ...
  protected:
    void* getCurValueUntyped() override {
        CSP_THROW(csp::RuntimeException,
                  "Trying to get value of a missing column " << getColumnName());
    }
};
...
class NativeTypeColumnAdapter : public BaseTypedColumnAdapter<ValueType, ArrowArrayType> {
  public:
    ...
    virtual CspTypePtr getNativeCspType() const override {
        return CspType::fromCType<ValueType>::type();
    }
...
class DatetimeColumnAdapter : public BaseTypedColumnAdapter<csp::DateTime, arrow::TimestampArray> {
  public:
    ...
    virtual CspTypePtr getNativeCspType() const override {
        return CspType::fromCType<csp::DateTime>::type();
    }
...
class DurationColumnAdapter : public BaseTypedColumnAdapter<csp::TimeDelta, arrow::DurationArray> {
  public:
    using BaseTypedColumnAdapter::BaseTypedColumnAdapter;
    virtual CspTypePtr getNativeCspType() const override {
        return CspType::fromCType<csp::TimeDelta>::type();
    }
...
class DateColumnAdapter : public BaseTypedColumnAdapter<csp::Date, ArrowDateArray> {
  public:
    ...
    virtual CspTypePtr getNativeCspType() const override {
        return CspType::fromCType<csp::Date>::type();
    }
...
class TimeColumnAdapter : public BaseTypedColumnAdapter<csp::Time, ArrowTimeArray> {
  public:
    ...
    virtual CspTypePtr getNativeCspType() const override {
        return CspType::fromCType<csp::Time>::type();
    }
...
class FixedSizeBinaryColumnAdapter
    : public BaseTypedColumnAdapter<std::string, arrow::FixedSizeBinaryArray> {
  public:
    using BaseTypedColumnAdapter::BaseTypedColumnAdapter;
    virtual CspTypePtr getNativeCspType() const override {
        return nullptr;
...
class BaseListColumnAdapter
    : public BaseTypedColumnAdapter<DialectGenericType, ArrowListArrayType> {
  public:
    ...
    void addSubscriber(ManagedSimInputAdapter* inputAdapter, std::optional<utils::Symbol> symbol,
                       const DialectGenericListReaderInterface::Ptr& listReader) override;
    CspTypePtr getNativeCspType() const override {
        return nullptr;
    }
    bool isListType() const override {
        return true;
    };
    CspTypePtr getContainerValueType() const override {
        return CspType::fromCType<ValueType>::type();
    }

  protected:
    // For now we allow only one subscription for list columns. In the future we might allow
    // multiple subscriptions using different readers. For example we could subscritbe to the same
    // column as a list and as array, we would need to do more book keepting in this case.
...
template <typename ArrowListArrayType, typename ValueArrayType,
          typename ValueType = typename ValueArrayType::TypeClass::c_type>
class NativeListColumnAdapter
    : public BaseListColumnAdapter<ArrowListArrayType, ValueArrayType, ValueType> {
  public:
    using BaseListColumnAdapter<ArrowListArrayType, ValueArrayType,
                                ValueType>::BaseListColumnAdapter;
...
class BytesListColumnAdapter
    : public BaseListColumnAdapter<ArrowListArrayType, ArrowBytesArrayType, std::string> {
  public:
    using BaseListColumnAdapter<ArrowListArrayType, ArrowBytesArrayType,
                                std::string>::BaseListColumnAdapter;
    using BaseListColumnAdapter<ArrowListArrayType, ArrowBytesArrayType,
                                std::string>::addSubscriber;
...
class StructColumnAdapter
    : public BaseTypedColumnAdapter<StructPtr, arrow::StructArray,
                                    csp::adapters::utils::ValueDispatcher<StructPtr&>> {
  public:
    using BASE = BaseTypedColumnAdapter<StructPtr, arrow::StructArray,
                                        csp::adapters::utils::ValueDispatcher<StructPtr&>>;

    StructColumnAdapter(ParquetReader& parquetReader,
                        const std::shared_ptr<::arrow::StructType>& arrowType,
    ...
    void setStructMeta(const std::shared_ptr<StructMeta>& structMeta) {
