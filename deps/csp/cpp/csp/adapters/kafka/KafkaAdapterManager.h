...
class KafkaAdapterManager final : public csp::AdapterManager {
  public:
    ...
    PushInputAdapter* getInputAdapter(CspTypePtr& type, PushMode pushMode,
    ...
    RdKafka::Conf* getConsumerConf() {
        return m_consumerConf.get();
    }

    const Dictionary::Value& startOffsetProperty() const {
        return m_startOffsetProperty;
    }

    int pollTimeoutMs() const {
        return m_pollTimeoutMs;
    }
  ...
  private:
    ...
    using StaticPublishers =
        std::unordered_map<TopicKeyPair, std::unique_ptr<KafkaPublisher>, hash::hash_pair>;
    ...
    using Subscribers =
        std::unordered_map<TopicKeyPair, std::unique_ptr<KafkaSubscriber>, hash::hash_pair>;
