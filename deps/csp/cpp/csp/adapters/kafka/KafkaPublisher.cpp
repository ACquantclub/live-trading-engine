...
KafkaPublisher::KafkaPublisher(KafkaAdapterManager* mgr, const Dictionary& properties,
                               std::string topic)
    : m_adapterMgr(*mgr),
      m_engine(mgr->engine()),
...

{
    ...
    if (protocol == "JSON")
        ...
        CSP_THROW(NotImplemented, "msg protocol "
                                      << protocol
                                      << " not currently supported for kafka output adapters");
}
...
OutputAdapter* KafkaPublisher::getOutputAdapter(CspTypePtr& type, const Dictionary& properties,
    ...
    auto adapter =
        m_engine->createOwnedObject<KafkaOutputAdapter>(*this, type, properties, keyFields);
    m_adapters.emplace_back(adapter);
    return m_adapters[m_adapters.size() - 1];
}
...
void KafkaPublisher::start(std::shared_ptr<RdKafka::Producer> producer) {
    ...
    if (!m_kafkaTopic)
        CSP_THROW(RuntimeException, "Failed to create RdKafka::Topic for producer on topic "
                                        << m_topic << ":" << errstr);
}

void KafkaPublisher::stop() {
    // Note its important to release the topic here because its underlying mem lives in the kafka
    // context of the Producer if Producer is destroyed first, when we try to destroy the topic its
    // a memory error
    m_kafkaTopic.reset();
}
...
void KafkaPublisher::send(const void* data, size_t len) {
    ...
    err = m_producer->produce(m_kafkaTopic.get(), RdKafka::Topic::PARTITION_UA,
                              RdKafka::Producer::RK_MSG_COPY, const_cast<void*>(data), len,
                              m_key.c_str(), m_key.length(), nullptr);
