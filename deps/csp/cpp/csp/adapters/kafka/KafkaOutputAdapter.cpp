...
KafkaOutputAdapter::KafkaOutputAdapter(Engine* engine, KafkaPublisher& publisher, CspTypePtr& type,
    ...
    : OutputAdapter(engine), m_publisher(publisher) {
    if (!publisher.isRawBytes())
        m_dataMapper = utils::OutputDataMapperCache::instance().create(
            type, *properties.get<DictionaryPtr>("field_map"));
...
void KafkaOutputAdapter::addFields(const std::vector<std::string>& keyFields, CspTypePtr& type,
                                   size_t i) {
