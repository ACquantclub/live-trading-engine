...
INIT_CSP_ENUM(csp::adapters::kafka::KafkaStatusMessageType, "OK", "MSG_DELIVERY_FAILED",
              "MSG_SEND_ERROR", "MSG_RECV_ERROR");
...
class DeliveryReportCb : public RdKafka::DeliveryReportCb {
  public:
    ...
    void dr_cb(RdKafka::Message& message) final {
         ...
         * for the message. */
        if (message.err()) {
            std::string msg = "KafkaPublisher: Message delivery failed for topic " +
                              message.topic_name() + ". Failure: " + message.errstr();
            m_adapterManager->pushStatus(StatusLevel::ERROR,
                                         KafkaStatusMessageType::MSG_DELIVERY_FAILED, msg);
        }
    }
    ...
    void event_cb(RdKafka::Event& event) override {
        if (event.severity() < RdKafka::Event::EVENT_SEVERITY_NOTICE) {
            std::string errmsg = "KafkaConsumer: error ( " + std::to_string(event.err()) +
                                 "): " + RdKafka::err2str((RdKafka::ErrorCode)event.err()) +
                                 ". Reason: " + event.str();
            m_adapterManager->pushStatus(StatusLevel::ERROR, KafkaStatusMessageType::GENERIC_ERROR,
                                         errmsg);
        }

        if (event.type() == RdKafka::Event::EVENT_ERROR) {
            // We shutdown the app if its a fatal error OR if its an authentication issue which has
            // plagued users multiple times Adding ERR__ALL_BROKERS_DOWN which happens when all
            // brokers are down
            if (event.fatal() || event.err() == RdKafka::ErrorCode::ERR__AUTHENTICATION ||
                event.err() == RdKafka::ErrorCode::ERR__ALL_BROKERS_DOWN) {
                m_adapterManager->forceShutdown(RdKafka::err2str((RdKafka::ErrorCode)event.err()) +
                                                event.str());
            }
        }
    }
...
KafkaAdapterManager::KafkaAdapterManager(csp::Engine* engine, const Dictionary& properties)
    ...
    const Dictionary& rdKafkaProperties =
        *properties.get<DictionaryPtr>("rd_kafka_conf_properties");

    m_consumerConf.reset(RdKafka::Conf::create(RdKafka::Conf::CONF_GLOBAL));
    setConfProperties(m_consumerConf.get(), rdKafkaProperties);
    setConfProperties(m_consumerConf.get(),
                      *properties.get<DictionaryPtr>("rd_kafka_consumer_conf_properties"));
    ...
    setConfProperties(m_producerConf.get(),
                      *properties.get<DictionaryPtr>("rd_kafka_producer_conf_properties"));
        ...
        while (true) {
            ...
            if (rc != RdKafka::ERR__TIMED_OUT)
                CSP_THROW(RuntimeException,
                          "KafkaProducer failed to flush pending msgs on shutdown: "
                              << RdKafka::err2str(rc));
        }
    } catch (...) {
        rootEngine()->shutdown(std::current_exception());
...
OutputAdapter* KafkaAdapterManager::getOutputAdapter(CspTypePtr& type,
                                                     const Dictionary& properties) {
    std::string topic = properties.get<std::string>("topic");
    try {
...
KafkaConsumer* KafkaAdapterManager::getConsumer(const std::string& topic,
                                                const Dictionary& properties) {
    ...
    // If we have reached m_maxThreads, then round-robin the topic onto a consumer (and insert it
    // into the map)
...
KafkaSubscriber* KafkaAdapterManager::getSubscriber(const std::string& topic,
...
KafkaPublisher* KafkaAdapterManager::getStaticPublisher(const TopicKeyPair& pair,
                                                        const Dictionary& properties) {
...
KafkaPublisher* KafkaAdapterManager::getDynamicPublisher(const std::string& topic,
                                                         const Dictionary& properties) {
