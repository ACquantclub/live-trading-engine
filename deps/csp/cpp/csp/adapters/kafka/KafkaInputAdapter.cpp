...
KafkaInputAdapter::KafkaInputAdapter(Engine* engine, CspTypePtr& type, PushMode pushMode,
                                     PushGroup* group, const Dictionary& properties)
    ...
    if (properties.exists("meta_field_map")) {
        ...
        if (metaFieldMap.exists("partition")) {
            ...
            if (!m_partitionField)
                CSP_THROW(ValueError, "field " << partitionFieldName
                                               << " is not a valid field on struct type "
                                               << structType.meta()->name());
            if (m_partitionField->type()->type() != CspType::Type::INT64)
                CSP_THROW(ValueError, "field " << partitionFieldName
                                               << " must be of type int on struct type "
                                               << structType.meta()->name());
        }
        if (metaFieldMap.exists("offset")) {
            ...
            if (!m_offsetField)
                CSP_THROW(ValueError, "field " << offsetFieldName
                                               << " is not a valid field on struct type "
                                               << structType.meta()->name());
        ...
        if (metaFieldMap.exists("live")) {
            ...
            if (!m_liveField)
                CSP_THROW(ValueError, "field " << liveFieldName
                                               << " is not a valid field on struct type "
                                               << structType.meta()->name());
            if (m_liveField->type()->type() != CspType::Type::BOOL)
                CSP_THROW(ValueError, "field " << liveFieldName
                                               << " must be of type bool on struct type "
                                               << structType.meta()->name());
        }
        if (metaFieldMap.exists("timestamp")) {
            ...
            if (!m_timestampField)
                CSP_THROW(ValueError, "field " << timestampFieldName
                                               << " is not a valid field on struct type "
                                               << structType.meta()->name());
            if (m_timestampField->type()->type() != CspType::Type::DATETIME)
                CSP_THROW(ValueError, "field " << timestampFieldName
                                               << " must be of type datetime on struct type "
                                               << structType.meta()->name());
        }
        if (metaFieldMap.exists("key")) {
            ...
            if (!m_keyField)
                CSP_THROW(ValueError, "field " << keyFieldName
                                               << " is not a valid field on struct type "
                                               << structType.meta()->name());
            if (m_keyField->type()->type() != CspType::Type::STRING)
                CSP_THROW(ValueError, "field " << keyFieldName
                                               << " must be of type string on struct type "
                                               << structType.meta()->name());
        }
        if (properties.exists("tick_timestamp_from_field")) {
            std::string timestampFieldName =
                properties.get<std::string>("tick_timestamp_from_field");
...
void KafkaInputAdapter::processMessage(RdKafka::Message* message, bool live,
                                       csp::PushBatch* batch) {
    DateTime msgTime;
    ...
    if (type()->type() == CspType::Type::STRUCT) {
        ...
        bool pushLive = shouldPushLive(live, msgTime);
        if (shouldProcessMessage(pushLive, msgTime))
            pushTick(pushLive, msgTime,
                     std::string((const char*)message->payload(), message->len()));
    }
}
