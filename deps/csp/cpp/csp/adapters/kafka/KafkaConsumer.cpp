        ...
        if (err == RdKafka::ERR__ASSIGN_PARTITIONS) {
            if (!m_doneSeeking) {
                ...
                if (!m_startTime.isNone()) {
                    ...
                    if (rc)
                        CSP_THROW(RuntimeException, "Failed to get kafka offsets for starttime "
                                                        << m_startTime << ": "
                                                        << RdKafka::err2str(rc));
                } else {
                    for (auto* partition : partitions)
                ...
                if (rc)
                    CSP_THROW(RuntimeException, "Failed to get kafka offsets for starttime "
                                                    << m_startTime << ": " << RdKafka::err2str(rc));
...
void KafkaConsumer::addSubscriber(const std::string& topic, const std::string& key,
...
void KafkaConsumer::start(DateTime starttime) {
    ...
    // This is a bit convoluted, but basically if we dont have rebalanceCB set, that means we are in
    // "groupid" mode which doesnt support seeking.  We force the adapters into a live mode, because
    // groupid mode leads to deadlocks on adapters that dont received any data since we dont have
    // partition information available to declare them done ( we dont even connect to them all )
    else
    ...
    if (err)
        CSP_THROW(RuntimeException, "Failed to subscribe to "
                                        << m_topics.size() << " topics: " << RdKafka::err2str(err));
    ...
    try {
        while (m_running) {
            ...
            if (topicIt == m_topics.end()) {
                std::string errmsg =
                    "KafkaConsumer: Message received on unknown topic: " + msg->topic_name() +
                    " errcode: " + RdKafka::err2str(msg->err()) + " error: " + msg->errstr();
                m_mgr->pushStatus(StatusLevel::ERROR, KafkaStatusMessageType::MSG_RECV_ERROR,
                                  errmsg);

                // We tend to accumulate more cases over time of error states that leave the engine
                // deadlocked on PushPull adapters. This section is for cases where we get an error
                // that is not topic specific, but is consumer specific, but we know its
                // non-recoverable if it gets too long, or we realize that ANY error here should
                // stop the engine, we can just always make it stop
            ...
            if (msg->err() == RdKafka::ERR_NO_ERROR && msg->len()) {
                if (!msg->key()) {
                    std::string errmsg = "KafkaConsumer: Message received with null key on topic " +
                                         msg->topic_name() + ".";
                    m_mgr->pushStatus(StatusLevel::ERROR, KafkaStatusMessageType::MSG_RECV_ERROR,
                                      errmsg);
                    continue;
                ...
                // Note we always have to tick wildcard as live because it can get messages from
                // multiple partitions, some which may have done replaying and some not ( not to
                // mention that data can be out of order )
                if (topicData.wildcardSubscriber)
                    topicData.wildcardSubscriber->onMessage(msg.get(), true);
            }

            // Not sure why, but it looks like we repeatedly get EOF callbacks even after the
            // original one may want to look into this.  Not an issue in practice, but seems like
            // unnecessary overhead
                ...
                if (!topicData.flaggedReplayComplete) {
                    ...
                    // we need to flag end in case the topic doesnt have any incoming data, we cant
                    // stall the engine on the pull side of the adapter
                    if (allDone)
                ...
                std::string errmsg =
                    "KafkaConsumer: Message error on topic \"" + msg->topic_name() +
                    "\". errcode: " + RdKafka::err2str(msg->err()) + " error: " + msg->errstr();
                m_mgr->pushStatus(StatusLevel::ERROR, KafkaStatusMessageType::MSG_RECV_ERROR,
                                  errmsg);
            }
