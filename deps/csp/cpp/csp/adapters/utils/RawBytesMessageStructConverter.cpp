...
RawBytesMessageStructConverter::RawBytesMessageStructConverter(const CspTypePtr& type,
    ...
    : MessageStructConverter(type, properties), m_targetField(nullptr) {
    ...
    if (fieldMap.size() == 1) {
        if (type->type() != CspType::Type::STRUCT)
            CSP_THROW(ValueError,
                      "field_map provided on non-struct type " << type->type() << " in adapter");

        if (!fieldMap.exists(""))
            CSP_THROW(ValueError,
                      "RawBytesMessageStructConverter expects one entry in fieldMap with empty "
                      "string as source key");
        ...
        if (!sField || sField->type()->type() != CspType::Type::STRING)
            CSP_THROW(TypeError, "field " << targetKey << " on struct " << m_structMeta->name()
        ...
        CSP_THROW(TypeError,
                  "TestMessageStructConverter expected type of STRING for empty field_map got "
                      << type->type());
}
