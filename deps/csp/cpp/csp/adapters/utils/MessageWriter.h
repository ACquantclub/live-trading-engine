...
// This is used to map data from an output adapter- > message writer ( can have multiple ouput
// adapters writing to same message )
...
// Derived types will be used to create and write timeseries data -> target message protocol ( ie
// JSON, proto ) and convert it to bytes for the output adapter
class MessageWriter {
  public:
  ...
  private:
    virtual void processTickImpl(const OutputDataMapper& dataMapper,
                                 const TimeSeriesProvider* sourcets) = 0;
};
...
inline void OutputDataMapper::apply(MessageWriterT& writer, const TimeSeriesProvider* ts) const {
    ...
    if (ts->type()->type() == CspType::Type::STRUCT)
        ...
        MessageWriterT::SupportedCspTypeSwitch::template invoke<
            typename MessageWriterT::SupportedArrayCspTypeSwitch>(ts->type(), [&](auto tag) {
...
inline void OutputDataMapper::applyStruct(MessageWriterT& writer, const Struct* struct_) const {
    ...
    for (auto& entry : m_messageFields) {
        ...
        SwitchT::template invoke<typename MessageWriterT::SupportedArrayCspTypeSwitch>(
            entry.sField->type().get(), [&](auto tag) {
                using T = typename decltype(tag)::type;
                writer.setField(entry.outField, entry.sField->value<T>(struct_),
                                *entry.sField->type(), entry);
            });
    };
}
