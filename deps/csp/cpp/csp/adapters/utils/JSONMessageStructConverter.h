...
class JSONMessageStructConverter : public MessageStructConverter {
  ...
  private:
    ...
    struct FieldEntry {
        ...
        std::shared_ptr<
            std::unordered_map<const char*, FieldEntry, csp::hash::CStrHash, csp::hash::CStrEq>>
            nestedFields;
    };
    ...
    template <typename T>
    T convertJSON(const char* fieldname, const CspType& type, const FieldEntry& entry,
                  const rapidjson::Value& v, T* foo) {
        return convertJSON(fieldname, v, foo);
    }

    template <typename StorageT>
    std::vector<StorageT> convertJSON(const char* fieldname, const CspType& type,
                                      const FieldEntry& entry, const rapidjson::Value& v,
                                      std::vector<StorageT>*);

    Fields m_fields;
    DateTimeWireType m_datetimeType;
    std::list<std::string>
        m_jsonkeys;  // intentionally stored as list so they dont invalidate on push
};
