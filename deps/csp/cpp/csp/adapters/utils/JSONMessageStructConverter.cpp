...
using SupportedCspTypeSwitch =
    PartialSwitchCspType<csp::CspType::Type::BOOL, csp::CspType::Type::INT32,
                         csp::CspType::Type::INT64, csp::CspType::Type::DOUBLE,
                         csp::CspType::Type::DATE, csp::CspType::Type::DATETIME,
                         csp::CspType::Type::STRING, csp::CspType::Type::ENUM,
                         csp::CspType::Type::STRUCT, csp::CspType::Type::ARRAY>;

using SupportedArrayCspTypeSwitch =
    PartialSwitchCspType<csp::CspType::Type::BOOL, csp::CspType::Type::INT32,
                         csp::CspType::Type::INT64, csp::CspType::Type::DOUBLE,
                         csp::CspType::Type::DATETIME, csp::CspType::Type::STRING,
                         csp::CspType::Type::ENUM>;

template <>
bool JSONMessageStructConverter::convertJSON(const char* fieldname, const rapidjson::Value& jValue,
                                             bool*) {
    if (jValue.IsBool())
...
template <>
int32_t JSONMessageStructConverter::convertJSON(const char* fieldname,
                                                const rapidjson::Value& jValue, int32_t*) {
    if (jValue.IsInt())
...
template <>
int64_t JSONMessageStructConverter::convertJSON(const char* fieldname,
                                                const rapidjson::Value& jValue, int64_t*) {
    if (jValue.IsInt64())
...
template <>
double JSONMessageStructConverter::convertJSON(const char* fieldname,
                                               const rapidjson::Value& jValue, double*) {
    if (jValue.IsNumber())
...
template <>
std::string JSONMessageStructConverter::convertJSON(const char* fieldname,
                                                    const rapidjson::Value& jValue, std::string*) {
    if (jValue.IsString())
...
template <>
Date JSONMessageStructConverter::convertJSON(const char* fieldname, const rapidjson::Value& jValue,
                                             Date*) {
    if (jValue.IsString())
...
template <>
DateTime JSONMessageStructConverter::convertJSON(const char* fieldname,
                                                 const rapidjson::Value& jValue, DateTime*) {
    if (jValue.IsUint64()) {
        ...
        switch (m_datetimeType) {
            ...
            case DateTimeWireType::NUM_TYPES:
                CSP_THROW(TypeError, "json field "
                                         << fieldname
                                         << " is datetime but datetimeType is not configured");
        }
...
template <>
CspEnum JSONMessageStructConverter::convertJSON(const char* fieldname, const CspType& type,
...
template <>
StructPtr JSONMessageStructConverter::convertJSON(const char* fieldname, const CspType&,
    ...
    for (auto jit = jValue.MemberBegin(); jit != jValue.MemberEnd(); ++jit) {
        ...
        SupportedCspTypeSwitch::invoke<SupportedArrayCspTypeSwitch>(
            nestedEntry.sField->type().get(), [this, &jit, &nestedEntry, &struct_](auto tag) {
                ...
                nestedEntry.sField->setValue(
                    struct_.get(), convertJSON(jit->name.GetString(), *nestedEntry.sField->type(),
...
std::vector<StorageT> JSONMessageStructConverter::convertJSON(const char* fieldname,
                                                              const CspType& type,
                                                              const FieldEntry&,
...
JSONMessageStructConverter::Fields JSONMessageStructConverter::buildFields(
    const CspStructType& type, const Dictionary& fieldMap) {
    ...
    for (auto it = fieldMap.begin(); it != fieldMap.end(); ++it) {
        ...
        if (it.hasValue<std::string>())
            ...
            if (!it.hasValue<DictionaryPtr>())
                CSP_THROW(TypeError, "fieldMap expected string or dict for field "
                                         << fieldName << " on struct " << type.meta()->name());
            auto nestedDict = it.value<DictionaryPtr>();
            if (nestedDict->size() != 1)
                CSP_THROW(ValueError, "Expected nested fieldmap for incoming JSON field "
                                          << fieldName << " to have a single key : map entry");
        ...
        if (!sField)
            CSP_THROW(ValueError, "field " << structField << " is not a valid field on struct type "
                                           << type.meta()->name());

        std::shared_ptr<Fields> nestedFields;
        if (sField->type()->type() == CspType::Type::STRUCT) {
            if (!nestedFieldMap)
                CSP_THROW(ValueError, "invalid field_map entry for nested struct field "
                                          << sField->fieldname() << " on struct type "
                                          << type.meta()->name());
    ...
    for (auto jit = document.MemberBegin(); jit != document.MemberEnd(); ++jit) {
        ...
        SupportedCspTypeSwitch::invoke<SupportedArrayCspTypeSwitch>(
            entry.sField->type().get(), [this, &jit, &entry, &data](auto tag) {
                using T = typename decltype(tag)::type;
                auto& jValue = jit->value;
                entry.sField->setValue(
                    data.get(), convertJSON(jit->name.GetString(), *entry.sField->type(), entry,
                                            jValue, static_cast<T*>(nullptr)));
            });
    }
