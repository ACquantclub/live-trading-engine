...
class ValueDispatcher final {
  public:
    ...
    void addSubscriber(SubscriberType subscriber, std::optional<Symbol> symbol = {}) {
        if (symbol.has_value()) {
            ...
            if (it == m_subscriberBySymbol.end()) {
                it = m_subscriberBySymbol.emplace(symbol.value(), std::vector<SubscriberType>())
                         .first;
            }
    ...
    void dispatch(ValueType* v, SubscriberContainer& subscriberContainer,
                  SubscriberArgs... extraArgs) {
