    ...
    if (isStruct)
        ...
        if (fieldMap.size() != 1 || !fieldMap.exists(""))
            CSP_THROW(ValueError,
                      "non-struct output adapter expected single field_map entry with blank key");
    ...
    for (auto it = fieldMap.begin(); it != fieldMap.end(); ++it) {
        ...
        if (it.hasValue<std::string>()) {
            if (sField->type()->type() == CspType::Type::STRUCT)
                CSP_THROW(TypeError, "Expected non-struct type for fieldmap on struct field "
            ...
            if (sField->type()->type() != CspType::Type::STRUCT)
                CSP_THROW(TypeError, "Expected structed type for nested struct field \""
                                         << structField << "\" on struct \"" << structMeta->name()
                                         << "\" got: \"" << sField->type()->type() << "\"");
            ...
            auto nestedFields = std::make_shared<Fields>(
                populateStructFields(nestedStructType->meta(), *nestedFieldMap));
...
OutputDataMapperPtr OutputDataMapperCache::create(const CspTypePtr& type,
                                                  const Dictionary& fieldMap) {
