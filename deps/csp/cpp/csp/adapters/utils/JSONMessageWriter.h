...
class JSONMessageWriter : public MessageWriter {
  public:
    using SupportedCspTypeSwitch = PartialSwitchCspType<
        ...
        csp::CspType::Type::DATE, csp::CspType::Type::DATETIME, csp::CspType::Type::ENUM,
        csp::CspType::Type::STRING, csp::CspType::Type::STRUCT, csp::CspType::Type::ARRAY>;

    using SupportedArrayCspTypeSwitch = PartialSwitchCspType<
        csp::CspType::Type::BOOL, csp::CspType::Type::UINT8, csp::CspType::Type::INT16,
        csp::CspType::Type::INT32, csp::CspType::Type::INT64, csp::CspType::Type::DOUBLE,
    ...
    template <typename T>
    void setField(const std::string& field, const T& value, const CspType& type,
  ...
  private:
    void processTickImpl(const OutputDataMapper& dataMapper,
                         const TimeSeriesProvider* sourcets) override {
    ...
    auto convertValue(const std::vector<StorageT>& value, const CspType& type,
                      const FieldEntry& entry);
...
inline auto JSONMessageWriter::convertValue(const csp::DateTime& value) {
    switch (m_datetimeWireType) {
        ...
        default:
            CSP_THROW(NotImplemented, "datetime wire type "
                                          << m_datetimeWireType
                                          << " not supported for json msg publishing");
    }
}
...
template <>
inline auto JSONMessageWriter::convertValue(const StructPtr& struct_, const CspType& type,
                                            const FieldEntry& entry) {
    ...
    for (auto& nestedEntry : *entry.nestedFields) {
        ...
        SupportedCspTypeSwitch::template invoke<SupportedArrayCspTypeSwitch>(
            nestedEntry.sField->type().get(), [&](auto tag) {
...
template <typename T>
inline void JSONMessageWriter::setField(const std::string& field, const T& value,
                                        const CspType& type, const FieldEntry& entry) {
    m_doc.AddMember(rapidjson::StringRef(field.c_str()), convertValue(value, type, entry),
                    m_doc.GetAllocator());
}
