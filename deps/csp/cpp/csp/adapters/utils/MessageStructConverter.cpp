...
bool MessageStructConverterCache::registerConverter(std::string protocol, Creator creator) {
    if (m_creators.find(protocol) != m_creators.end())
        CSP_THROW(RuntimeException, "Attempted to register creator for MessageStructConverter type "
...
MessageStructConverterPtr MessageStructConverterCache::create(const CspTypePtr& type,
