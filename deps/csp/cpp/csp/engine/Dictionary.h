...
class Dictionary {
  public:
    ...
    using Value = std::variant<std::monostate, bool, int32_t, uint32_t, int64_t, uint64_t, double,
  ...
  public:
    ...
    struct const_iterator {
        const_iterator(const DataVector::const_iterator& it) : m_it(it) {
        }
        ...
        const Value& getUntypedValue() const {
            return m_it->second._data;
        }

        // check if value exists as given type
        // note this checks exact types, not coercible types ( it if its int32 and you check int64
        // itr will be false )
        template <typename T>
        bool hasValue() {
...
template <typename T>
inline typename Dictionary::ReturnType<T>::Type Dictionary::get(const std::string& key,
...
inline int32_t Dictionary::cast<uint32_t, int32_t>(const uint32_t& value) {
    if (value > (uint32_t)std::numeric_limits<int32_t>::max())
        CSP_THROW(RangeError, "Dictionary value for uint32_t ( "
                                  << value << " ) is out of range for int32_t cast");
    return value;
}
...
inline uint32_t Dictionary::cast<int32_t, uint32_t>(const int32_t& value) {
    if (value < 0)
        CSP_THROW(RangeError, "Dictionary value for int32_t ( "
                                  << value << " ) is out of range for uint32_t cast");
    return value;
}
...
inline int64_t Dictionary::cast<uint64_t, int64_t>(const uint64_t& value) {
    if (value > (uint64_t)std::numeric_limits<int64_t>::max())
        CSP_THROW(RangeError, "Dictionary value for uint64_t ( "
                                  << value << " ) is out of range for int64_t cast");
    return value;
}
...
inline uint64_t Dictionary::cast<int64_t, uint64_t>(const int64_t& value) {
    if (value < 0)
        CSP_THROW(RangeError, "Dictionary value for int64_t ( "
                                  << value << " ) is out of range for uint64_t cast");
    return value;
}
...
template <typename T>
inline typename Dictionary::ReturnType<T>::Type Dictionary::extractValue(const std::string& key,
                                                                         const Value& value) {
    try {
        ...
        try {
            ...
            std::string curtypename = std::visit(
                [](auto&& arg) -> std::string {
                    return cpp_type_name<std::decay_t<decltype(arg)>>();
                },
                value);
            CSP_THROW(TypeError, "Dictionary type-mismatch on key \""
                                     << key << "\".  Expected type \"" << cpp_type_name<T>()
