...
class TickBuffer {
  public:
    ...
    // note subtelty of returning m_writeIndex if we arent full, this is to account for cases where
    // we were full but buffer was grown ( m_count can be < m_capacity but m_count is not number of
    // ticks available in that case )
    uint32_t numTicks() const {
        return unlikely(!m_full) ? m_writeIndex : m_capacity;
    }
    ...
    const T& operator[](uint32_t index) const {
        return const_cast<TickBuffer<T>*>(this)->operator[](index);
    }
    ...
    // this is needed for fast copies of the data, since it is internally stored in a circular
    // buffer
  ...
  private:
    ...
    //  - for size 1 case
    //  can try a union of { T t; T * t_buffer; } and a capacity and T * that always points to
    //  either &t ot t_buffer for quick read access
...
inline TickBuffer<T>::TickBuffer(uint32_t capacity)
    : m_capacity(capacity), m_writeIndex(0), m_full(false) {
...
inline void TickBuffer<T>::raiseRangeError(uint32_t index) const {
    CSP_THROW(RangeError, "Invalid buffer access: index "
                              << index << " out of range for buffer with numTicks: " << numTicks()
                              << " capacity: " << m_capacity);
}
...
T* TickBuffer<T>::flatten(uint32_t startIndex, uint32_t endIndex, uint32_t tailPadding) const {
    ...
    if (startIndex < endIndex)
        CSP_THROW(RangeError, "Invalid buffer flatten: endIndex "
                                  << endIndex << " greater than startIndex " << startIndex);
    if (startIndex >= m_capacity)
        CSP_THROW(RangeError, "Invalid buffer flatten: startIndex "
                                  << startIndex << " greater than capacity " << m_capacity);
    ...
    if (unlikely(raw_index + len > m_capacity))  // buffer wraps around
    {
        ...
        // copy the values after the wrap (starting from index 0 on the raw buffer, and index
        // beforeWrap on values
