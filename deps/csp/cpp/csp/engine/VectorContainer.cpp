...
std::unique_ptr<VectorContainer> VectorContainer::createForCspType(CspTypePtr& type,
                                                                   bool optionalValues) {
    return createForCspType(type.get());
}

std::unique_ptr<VectorContainer> VectorContainer::createForCspType(const CspType* type,
                                                                   bool optionalValues) {
    return AllCspTypeSwitch::invoke(type, [optionalValues](auto tag) {
        if (optionalValues) {
            ...
            return std::unique_ptr<VectorContainer>(
                new TypedVectorContainer<typename decltype(tag)::type>);
        }
    });
}
