...
struct PushEvent {
    ...
    PushInputAdapter* adapter() {
        return (PushInputAdapter*)(((uint64_t)m_adapter) & ~0x1);
    }
    bool isGroupEnd() const {
        return ((uint64_t)m_adapter) & 0x1;
    }
    void flagGroupEnd() {
        m_adapter = (PushInputAdapter*)(((uint64_t)m_adapter) | 0x1);
    }
...
struct TypedPushEvent : public PushEvent {
    TypedPushEvent(PushInputAdapter* adapter, T&& d)
        : PushEvent(adapter), data(std::forward<T>(d)) {
    }
