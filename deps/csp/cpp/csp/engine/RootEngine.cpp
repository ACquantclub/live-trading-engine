...
An interrupt should cause all running engines to stop, but should not affect future runs in the same
process. Thus, each root engine keeps track of the signal count when its created. When an interrupt
occurs, one engine thread handles the interrupt by incrementing the count. Then, all other root
engines detect the signal by comparing their initial count to the current count.

Future runs after the interrupt remain unaffected since they are initialized with the updated signal
count, and will only consider themselves "interupted" if another signal is received during their
execution.
*/
...
void RootEngine::runRealtime(DateTime end) {
    struct DialectReleaseGIL {
        DialectReleaseGIL(RootEngine* e) : engine(e) {
            engine->dialectUnlockGIL();
        }
        ~DialectReleaseGIL() {
            engine->dialectLockGIL();
        }
        RootEngine* engine;
    };
    ...
    while (m_state == State::RUNNING && !interrupted()) {
        ...
        if (!haveEvents) {
            ...
            // timers in the previous cycle, then we shouldnt wait again ( which can actually lead
            // to cases where we miss triggers )
        ...
        // We made a conscious decision to execute timers exactly on their requested time in
        // realtime mode therefore timers that are ready are executed on their own time cycle before
        // realtime events all processed
...
DictionaryPtr RootEngine::engine_stats() const {
    if (!m_profiler)
        CSP_THROW(ValueError,
                  "Cannot profile a graph unless the graph is run in a profiler context.");
