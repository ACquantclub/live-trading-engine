...
DynamicNode::DynamicNode(Engine* engine, const std::string& name,
                         std::vector<INOUT_ID_TYPE> snapIds, EngineBuilder builder, NodeDef def)
    : Node(def, engine), m_engineBuilder(builder), m_name(name), m_snapIds(std::move(snapIds)) {
...
void DynamicNode::handleEvent(InputId id) {
    // see comment in addDynamicInstance... these ids can be stub ids, we dont rely on them, skip
    // Node::handleEvent logic we do need to let dynamic basket input flow through the regular
    // channels though
...
void DynamicNode::executeImpl() {
    ...
    if (shapeTicked) {
        ...
        for (auto& event : events) {
            ...
            // note that we handle removes AFTER the end of the dynamic cycle, the reason is
            // two-fold 1) there can be cases where remove and some tick happen in the same cycle.
            // to be consistent with runing on endtime, we process that tick 2) its possible some
            // nodes in the to-be-removed sub engine are already scheduled in the cycle step table.
            // it would be difficult to get them removed

            // HOWEVER there is the pitfall that a csp.attach() input will be accessed on the same
            // cycle as it being shutdown.  It will no longer be a valid ts in this last cycle since
            // it was already removed.  In python code it still "works" because the last value is
            // cached in a localframe var.  in c++ nodes access will raise since buffer will be
            // reset to empty. In BOTH cases, a proper check of csp.valid() will safely return false
            // on the removed edge
        }
    }
    ...
    // the dynamic engine while the node within the dynamic which is calling csp.stop_engine is
    // being executed!
...
void DynamicNode::addDynamicInstance(const DialectGenericType& key) {
    ...
    auto instanceEngine =
        std::make_unique<DynamicEngine>(m_cycleStepTable, rootEngine(), shutdownFn);
    ...
    {
        // monitor will keep track of all events scheduled for "now" which we want to execute
        // immediately after start otherwise they will defer to the next engine cycle.  The class
        // destructore will then process those events
        Scheduler::DynamicEngineStartMonitor monitor(rootEngine()->dynamicEngineStartMonitor());
        instanceEngine->start();
    }

    // Note that all external inputs are wired by graph building as we add them all as inputs to the
    // dynamic node up front. However, there may be external inputs that ticked the same cycle as
    // creating this engine.  In such a case we need to force-propagate the input on startup to all
    // nodes in this engine so that the new graph will see it ticked properly on the first cycle
    ...
    // Need to attach all inputs created in the dynamic instance as inputs to this outter
    // DynamicNode as those inputs tick, we need to get scheduled in the root engine so that we
    // execute the cycle step in here we provide a stub id that is never intended to be used.
    // DynamicNode::handleEvent is overriden to ensure Node::handleEvent doesnt act on these ids
...
void DynamicNode::removeDynamicInstance(const DialectGenericType& key) {
    ...
    // to optimize expensive removeConsumer calls we search for all inputs that are sourced
    // externally
