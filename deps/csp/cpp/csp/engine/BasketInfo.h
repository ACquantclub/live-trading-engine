...
class InputBasketInfo {
  ...
  public:
    ...
    bool allValid() const {
        ...
        // Unforunately we have to loop over every input if we're not valid yet.  Originally we were
        // counting valid in handleEvent but this breaks if the basket is passive
    ...
    struct input_iterator {
        ...
        input_iterator(TimeSeriesProvider const* const* itBegin,
                       TimeSeriesProvider const* const* itEnd, int64_t startId = 0)
            : m_elemId(startId) {
            m_it = itBegin;
            m_itEnd = itEnd;
        }
        ...
        const TimeSeriesProvider* operator->() const {
            return get();
        }
    ...
    struct ticked_iterator {
        ticked_iterator(TimeSeriesProvider const* const* inputs, TickedInputs::const_iterator it,
            ...
            : m_inputs(inputs), m_it(it), m_itEnd(itEnd) {
        }
    ...
    struct valid_iterator {
        valid_iterator(TimeSeriesProvider const* const* itBegin,
                       TimeSeriesProvider const* const* itEnd)
            : m_it(itBegin), m_itEnd(itEnd), m_elemId(0) {
    ...
    input_iterator begin_inputs() const {
    ...
    valid_iterator begin_valid() const {
        return valid_iterator(m_inputs, m_inputs + m_size);
    }
...
class DynamicInputBasketInfo : public InputBasketInfo {
  public:
    using ChangeCallback = std::function<void(const DialectGenericType& key, bool added,
                                              int64_t elemId, int64_t replaceId)>;
    using Keys = std::vector<DialectGenericType>;

    DynamicInputBasketInfo(RootEngine* rootEngine)
        : InputBasketInfo(rootEngine, 0, true), m_capacity(0), m_tickCountPolicy(1) {
    ...
    const TimeSeriesProvider* shapeTs() const {
        return m_inputs[-1];
    }

    int64_t addDynamicKey(const DialectGenericType& key, const TimeSeriesProvider* ts);

    // removeID is element to remove, replaceId ( if >= 0 ) is element to take its place (
    // compaction )
    void removeDynamicKey(uint64_t engineCycle, const DialectGenericType& key, int64_t removeId,
                          int64_t replaceId);

    // for dynamic input baskets we maintain bufffering policy on the dynamic basket so that they
    // get applied to newly added inputs
    void setTickCountPolicy(int32_t tickCount) {
        m_tickCountPolicy = tickCount;
    }
    void setTickTimeWindowPolicy(TimeDelta window) {
        m_timeWindowPolicy = window;
    }
...
class DynamicOutputBasketInfo : public OutputBasketInfo {
  ...
  private:
    ...
    // for dynamic graph basket outputs we take an existing ts and stick it in the dynamic output
    // basket for efficiency ( alternative is to own a copy on the outpu basket and copy every tick,
