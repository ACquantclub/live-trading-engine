...
class CspEnumInstance {
  public:
    CspEnumInstance(std::string name, int64_t value, csp::CspEnumMeta* meta)
        : m_name(name), m_value(value), m_meta(meta) {
    }
...
// As an optimization we do NOT attach meta or value to every instance of an enum.  Instead, the
// enum holds only a pointer to a singleton CspEnumInstance, which holds the value, name, and meta
// pointer.
class CspEnum {
  public:
    CspEnum();
    CspEnum(const CspEnum& other) {
    ...
    bool operator==(const CspEnum& rhs) const {
...
class CspEnumMeta {
  ...
  private:
    ...
    using Mapping =
        std::unordered_map<const char*, InstanceMapping::iterator, hash::CStrHash, hash::CStrEq>;
