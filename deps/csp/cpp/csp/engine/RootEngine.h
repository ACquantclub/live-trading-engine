...
class RootEngine : public Engine {
  ...
  public:
    ...
    Scheduler::Handle scheduleCallback(Scheduler::Handle reservedHandle, DateTime time,
                                       Scheduler::Callback cb);
    Scheduler::Handle scheduleCallback(Scheduler::Handle reservedHandle, TimeDelta time,
                                       Scheduler::Callback cb);
    ...
    void schedulePushEvent(PushEvent* event) {
        m_pushEventQueue.push(event);
    }
    void schedulePushBatch(PushEventQueue::Batch& batch) {
        m_pushEventQueue.push(batch);
    }

    bool scheduleEndCycleListener(EndCycleListener* l);

    // returns true if the engine is currently in runRealtime
    bool inRealtime() const {
        return m_inRealtime;
    }

    // returns true if engine is configured realtime ( inRealtime can still be false if
    // realtime run starts in the past )
    bool configuredRealtime() const {
        return m_settings.realtime;
    }

    bool configuredProfile() const {
        return (bool)m_profiler;
    }
...
inline Scheduler::Handle RootEngine::scheduleCallback(Scheduler::Handle reservedHandle,
                                                      TimeDelta delta, Scheduler::Callback cb) {
    return scheduleCallback(m_now + delta, std::move(cb));
}

inline Scheduler::Handle RootEngine::scheduleCallback(Scheduler::Handle reservedHandle,
                                                      DateTime time, Scheduler::Callback cb) {
    if (unlikely(time < m_now))
        CSP_THROW(ValueError,
                  "Cannot schedule event in the past.  new time: " << time << " now: " << m_now);
