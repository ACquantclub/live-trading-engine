...
class WindowBuffer {
  public:
    ...
    // if index is non-negative, treat as offset from the rightIndex (i.e. buf[1] is the second most
    // recent value ) if index is negative, treat as offset from the leftIndex (i.e. buf[-1] is
    // earliest value)
    ...
    /*
    NOTE: The concept of begin() and end() does not extend naturally to the window buffer because of
    its circularity. In the case the buffer is at capacity, the begin() and end() iterators point to
    the same location. Thus, the only way you can properly iterate is by starting at the left() and
    keeping track of when you've visited the correct number of elements. You could similarly iterate
    in reverse order by starting at right() and decrementing.
    */

    const_iterator left() const {
        return const_iterator(m_values, m_leftIndex, m_capacity);
    }

    const_iterator right() const {
        return const_iterator(m_values, m_rightIndex, m_capacity);
    }
...
inline void WindowBuffer<T>::remove_left(int64_t n) {
    if (n > m_count)
        CSP_THROW(RangeError, "Window buffer of size " << m_count << " does not contain " << n
...
inline FixedSizeWindowBuffer<T>::FixedSizeWindowBuffer(int64_t capacity) {
    ...
    try {
        ...
        CSP_THROW(
            ValueError,
            "Tick specified interval is too large To use an expanding window, set interval=None.");
    }
...
template <typename T>
inline FixedSizeWindowBuffer<T>& FixedSizeWindowBuffer<T>::operator=(
    FixedSizeWindowBuffer<T>&& rhs) {
    WindowBuffer<T>::operator=(std::move(rhs));
    return *this;
}
...
template <typename T>
inline VariableSizeWindowBuffer<T>& VariableSizeWindowBuffer<T>::operator=(
    VariableSizeWindowBuffer<T>&& rhs) {
    WindowBuffer<T>::operator=(std::move(rhs));
    return *this;
}
