...
// DynamicNode Is an instance of an outter node that is responsible for a single csp.dynamic() call
// in the graph Internally it will maintain N DynamicEngines, 1 per instantiated key.  It is
// responsible for reacting to all external inputs and triggering a cycle step. All dynamic engine
// input adapters as well as all external timeseries wired into dynamic are registered as inputs
...
class DynamicNode final : public Node {
  public:
    ...
    using EngineBuilder =
        std::function<Outputs(DynamicNode*, DynamicEngine*, const DialectGenericType& key)>;
