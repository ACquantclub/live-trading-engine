...
Engine::Engine(CycleStepTable& stepTable, RootEngine* root)
    : m_rootEngine(root ? root : static_cast<RootEngine*>(this)), m_cycleStepTable(stepTable) {
}
...
void Engine::registerOwnedObject(std::shared_ptr<AdapterManager> manager) {
    // AdapterManagers may get created in a dynamic graph the first time around, but we want them
    // shared across dynamics so they always get registered and effectively owned by root engine
    if (!isRootEngine())
...
void Engine::registerGraphOutput(const DialectGenericType& key,
                                 std::shared_ptr<GraphOutputAdapter> adapter) {
    ...
    // If were in a dynamic engine we register the key to catch duplicates up front.  We also
    // register in root so that it gets processed at the end of the run.
    if (!isRootEngine())
...
static int32_t calcRank(Consumer* consumer, std::unordered_set<Consumer*>& visited,
                        std::vector<Consumer*>& path) {
    if (consumer->rank() >= 0)
        return consumer->rank();

    // we dont have to check visited vs done in our cycle check since done is checked above by
    // seeing if rank was already computed.  So we only insert into visited
    auto rv = visited.insert(consumer);
    if (!rv.second) {
        std::stringstream oss;
        oss << "Illegal cycle found in graph, path:\n\t** ";
        for (size_t idx = path.size() - 1; idx > 0; --idx) {
            // flag the end of the cycle clearly in the output, which is when we first see the
            // current node again ( unless its the first iteration )
    ...
    for (auto input_iter = consumer->inputs(); input_iter; ++input_iter) {
        int input_rank = 0;
        if (input_iter->node() &&
            // its possible that we are a dynamic engine and the input is wired from the parent
            // engine in this case we treat it as if its a 0-ranked input
...
int32_t Engine::computeRanks() {
    // We will start with output nodes and work our way back to avoid recomputing ranks repeatedly
    // if we want input -> out
    ...
    for (auto* consumer : roots) {
        // we dont include outputs in the path since its confusing to see in the cycle ( and clearly
        // not part of it )
...
void Engine::start() {
    ...
    // see registerOwnedObject( AdapterManager ) above, we register adapter managers with root.  At
    // this point we dont need the list of mgrs created in a dynamic engine anymore, so we clear out
    // the mem ( and effetively take them out of the stop() list for dynamic shutdown )
    if (!isRootEngine())
...
void Engine::stop() {
    // Ensure we only stop nodes/adapters that have started in the case an exception occurs during
    // startup
    for (auto& node : m_nodes) {
        if (node->started())
            node->stop();
