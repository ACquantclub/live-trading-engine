...
struct InOutId {
    InOutId(INOUT_ID_TYPE inputId, INOUT_ELEMID_TYPE elementId = ELEM_ID_NONE)
        : elemId(elementId), id(inputId) {
    }

    bool operator==(InOutId rhs) const {
        return rhs.elemId == elemId && rhs.id == id;
    }

    static constexpr size_t maxId() {
        return std::numeric_limits<INOUT_ID_TYPE>::max();
    }
    ...
    }  // max is taken by NONE
    static constexpr size_t maxBasketElements() {
        return maxElemId() + 1;
    }
