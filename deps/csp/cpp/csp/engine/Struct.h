...
class StructField {
  public:
    ...
    size_t offset() const {
        return m_offset;
    }  // offset to start of field's memory from start of struct mem
    size_t size() const {
        return m_size;
    ...
    uint8_t maskBit() const {
        return m_maskBit;
    }  // bit within mask byte associated with this field
    uint8_t maskBitMask() const {
        return m_maskBitMask;
    }  // same as maskBit but as a mask ( 1 << bit

    bool isNative() const {
        return m_type->type() <= CspType::Type::MAX_NATIVE_TYPE;
    }
  ...
  public:
    const T& value(const Struct* s) const {
        CSP_THROW(NotImplemented,
                  "Struct fields are not supported for type " << CspType::Type::fromCType<T>::type);
    }
...
class NonNativeStructField : public StructField {
  public:
    NonNativeStructField(CspTypePtr type, const std::string& fieldname, size_t size,
                         size_t alignment)
...
class ArrayStructField : public NonNativeStructField {
    ...
    template <typename T>
    static std::enable_if_t<CspType::isNative(CspType::Type::fromCType<T>::type), void> deepcopy(
        const std::vector<T>& src, std::vector<T>& dest) {
        dest = src;
    }
    ...
    static void deepcopy(const std::vector<DialectGenericType>& src,
                         std::vector<DialectGenericType>& dest) {
    ...
    static void deepcopy(const std::vector<std::vector<StorageT>>& src,
                         std::vector<std::vector<StorageT>>& dest) {
        dest.resize(src.size());
        for (size_t i = 0; i < src.size(); ++i)
...
class StructMeta : public std::enable_shared_from_this<StructMeta> {
  public:
    ...
    bool isNative() const {
        return m_isFullyNative;
    }

    const Fields& fields() const {
        return m_fields;
    }
    const FieldNames& fieldNames() const {
        return m_fieldnames;
    }

    size_t maskLoc() const {
        return m_maskLoc;
    }
    ...
    void setDefault(StructPtr default_) {
        m_default = default_;
    }

    Struct* createRaw() const;
    StructPtr create() const {
        return StructPtr(createRaw());
    }
    ...
    // Given two types x and y this will return one or the other based on which is a base class of
    // the other. if x derives from y, returns y.  if y derives from x, returns x.  Otherwise this
    // returns null.  NOTE this wont return a common ancestor
  ...
  private:
    ...
    size_t partialNativeSize() const {
        return m_size - m_nativeStart;
    }
    ...
    // m_fields includes all base fields as well. m_fieldnames maintains the proper iteration order
    // of fields
    Fields m_fields;
    FieldNames m_fieldnames;
    size_t m_size;  // full size of struct including base classes - NOTE this does not include
                    // refcount and meta ptr on Struct
...
template <typename T>
std::shared_ptr<typename StructField::upcast<T>::type> StructMeta::getMetaField(
    const char* fieldname, const char* expectedtype) {
    ...
    if (!typedfield)
        CSP_THROW(TypeError, expectedtype << " - provided struct type " << name()
                                          ...
                                          << " for field " << fieldname << " but got type "
...
class Struct {
  public:
    const StructMeta* meta() const {
        return hidden()->meta.get();
    }
    size_t refcount() const {
        return hidden()->refcount;
    }

    bool operator==(const Struct& rhs) const {
        return meta()->isEqual(this, &rhs);
    }
    bool operator!=(const Struct& rhs) const {
        return !((*this) == rhs);
    }
    ...
    void* dialectPtr() const {
        return hidden()->dialectPtr;
    }
    void setDialectPtr(void* p) {
        hidden()->dialectPtr = p;
    }
...
#pragma GCC diagnostic pop
    ...
    // Note these members are not included on size(), they're stored before "this" ptr ( see
    // operator new / delete )
    struct HiddenData {
        size_t refcount;
...
class StructStructField final : public NonNativeStructField {
  public:
    ...
    const StructMetaPtr& meta() const {
        return m_meta;
    }
...
void ArrayStructField<ElemT>::deepcopy(const std::vector<StructPtr>& src,
                                       std::vector<StructPtr>& dest) {
    dest.resize(src.size());
    for (size_t i = 0; i < src.size(); ++i)
        dest[i] = src[i]->deepcopy();
}
...
struct StructField::upcast<std::vector<StorageT>> {
    static_assert(!std::is_same<StorageT, bool>::value,
                  "vector<bool> should not be getting instantiated");
