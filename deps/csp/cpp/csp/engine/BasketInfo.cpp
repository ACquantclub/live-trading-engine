...
int64_t DynamicInputBasketInfo::addDynamicKey(const DialectGenericType& key,
    ...
    if (elemId >= m_capacity) {
        ...
        m_inputs = (TimeSeriesProvider const**)realloc(
            m_inputs - 1, sizeof(TimeSeriesProvider*) * (m_capacity + 1));
        ++m_inputs;
...
void DynamicInputBasketInfo::removeDynamicKey(uint64_t engineCycle, const DialectGenericType& key,
                                              int64_t removeId, int64_t replaceId) {
    CSP_ASSERT(m_size > 0);

    // we need to update ticked items if replaceId is set ( being moved ) AND it ticked.  Note that
    // we dont have to reset removeId since we dont allow removing a key same cycle as it is ticked.
    // We check if replaceId has a tick this cycle to avoid doing a potentially expensive
    // tickeditems scan if we dont need to
    if (replaceId != -1 && elem(replaceId)->lastCycleCount() == engineCycle) {
        // need to scan unfortunately, we had a long debate back and forth on this, we may revisit
        // in the future
...
OutputBasketInfo::OutputBasketInfo(CspTypePtr& type, Node* node, size_t size, bool isDynamic)
    : m_outputs(nullptr), m_size(size), m_isDynamic(isDynamic) {
    if (m_size > 0) {
        ...
        TimeSeriesProvider* p = reinterpret_cast<TimeSeriesProvider*>(
            reinterpret_cast<uint8_t*>(mem) + sizeof(TimeSeriesProvider*) * m_size);
...
DynamicOutputBasketInfo::DynamicOutputBasketInfo(CspTypePtr& type, Node* node)
    : OutputBasketInfo(type, node, 0, true),
      ...
      m_capacity(0),
      m_ownTs(true) {
    static CspTypePtr s_shapeType =
        std::make_shared<CspStructType>(csp::autogen::DynamicBasketEvent::meta());
    m_shapeTs.init(s_shapeType, node);
}

DynamicOutputBasketInfo::~DynamicOutputBasketInfo() {
    // we need to free indiviual timeseries here since we allocate TS's independently in dynamic
    // baskets static baskets do a single allocation for all ts we may be a dynamic graph output
    // basket, in which case we dont own the time series ( see comment in header )
    if (m_ownTs) {
...
bool DynamicOutputBasketInfo::addCapacity() {
    if (m_size == m_capacity) {
        m_capacity = std::max(1, m_capacity * 2);
        // in dynamic baskets we cant use the "single alloc" optimization we do for static baskets
        // since input baskets already refer to the TSPs by pointer ( which can be changed on
        // realloc ) so we do a regular alloc of array of ptrs, then allocate each item
        // independently
        m_outputs =
            (TimeSeriesProvider**)realloc(m_outputs, sizeof(TimeSeriesProvider*) * (m_capacity));
...
void DynamicOutputBasketInfo::propagateAddKey(const DialectGenericType& key,
                                              const TimeSeriesProvider* ts) {
...
int64_t DynamicOutputBasketInfo::addDynamicTs(const DialectGenericType& key,
                                              const TimeSeriesProvider* ts) {
...
int64_t DynamicOutputBasketInfo::removeDynamicKey(const DialectGenericType& key, int64_t elemId) {
    ...
    if (elemId != m_size - 1) {
        ...
        m_outputs[elemId]->propagator().apply(
            [elemId](Consumer*, const InputId& id) { const_cast<InputId&>(id).elemId = elemId; });
    }
