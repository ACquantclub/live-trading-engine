...
Node::Node(NodeDef def, Engine* engine) : Consumer(engine), m_outputs(nullptr), m_def(def) {
...
Node::~Node() {
    ...
    for (int i = 0; i < m_def.numOutputs; ++i) {
        if (m_outputs[i].isSet<TimeSeriesProvider>())
            ...
            // its possible to have null outputs if we had an exception on startup before creating
            // the output ( ie cppnode NotImplemented type ) check for null before checking is
            // Basket
...
void Node::validateInputIndex(size_t id) {
    if (id > InputId::maxId())
        CSP_THROW(ValueError, "Input " << id << " on node \"" << name()
                                       << "\" violates maximum allowable inputs");
}

void Node::validateOutputIndex(size_t id) {
    if (id > OutputId::maxId())
        CSP_THROW(ValueError, "Output " << id << " on node \"" << name()
                                        << "\" violates maximum allowable outputs");
}

void Node::validateInputBasketSize(size_t id, size_t size) {
    if (size > InputId::maxBasketElements())
        CSP_THROW(ValueError, "Input " << id << " on node \"" << name() << "\" has basket size "
                                       << size << " which violates maximum"
                                       << " allowable basket elements [ "
                                       << InputId::maxBasketElements() << " ]");
}

void Node::validateOutputBasketSize(size_t id, size_t size) {
    if (size > OutputId::maxBasketElements())
        CSP_THROW(ValueError, "Output " << id << " on node \"" << name() << "\" has basket size "
                                        << size << " which violates maximum"
                                        << " allowable basket elements [ "
                                        << OutputId::maxBasketElements() << " ]");
}
