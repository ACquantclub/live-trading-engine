  ...
  public:
    ...
    const InOutDef& tsinputDef(const char* inputName) const {
        ...
        if (it == m_nodedef->inputs.end())
            CSP_THROW(ValueError,
                      "CppNode failed to find input " << inputName << " on node " << name());
    ...
    const InOutDef& tsoutputDef(const char* outputName) const {
        ...
        if (it == m_nodedef->outputs.end())
            CSP_THROW(ValueError,
                      "CppNode failed to find output " << outputName << " on node " << name());
    ...
    const InOutDef& alarmDef(const char* alarmName) const {
        ...
        if (!def.isAlarm)
            CSP_THROW(TypeError, "CppNode expected alarm " << alarmName
                                                           << " but found it as an input on node "
                                                           << name());
        return def;
    }
    ...
    T scalarValue(const char* scalarName) const {
        ...
        if (!m_nodedef->scalars.exists(scalarName))
            CSP_THROW(ValueError,
                      "CppNode failed to find scalar " << scalarName << " on node " << name());
    ...
    DateTime now() const {
        return rootEngine()->now();
    }

    using Creator =
        std::function<CppNode*(csp::Engine* engine, const csp::CppNode::NodeDef& nodedef)>;

  protected:
    CppNode(Engine* engine, const NodeDef& nodedef)
        : csp::Node(asCspNodeDef(nodedef), engine), m_nodedef(&nodedef) {
    }

    csp::NodeDef asCspNodeDef(const NodeDef& nodedef) const {
        if (nodedef.inputs.size() > InputId::maxInputs())
            CSP_THROW(ValueError, "number of inputs exceeds limit of " << InputId::maxInputs()
                                                                       << " on node " << name());

        if (nodedef.outputs.size() > OutputId::maxOutputs())
            CSP_THROW(ValueError, "number of outputs exceeds limit of " << OutputId::maxOutputs()
                                                                        << " on node " << name());

        return csp::NodeDef{INOUT_ID_TYPE(nodedef.inputs.size()),
                            INOUT_ID_TYPE(nodedef.outputs.size())};
    }

    void validateNodeDef() const {
        if (!m_nodedef)
            CSP_THROW(RuntimeException,
                      "CppNode cpp nodedef information is only available during INIT");
    }
    ...
    class InputWrapper {
      public:
        ...
        InputWrapper(const char* name, const CppNode& node) : m_node(node), m_id(0) {
            auto& def = m_node.tsinputDef(name);
            if (def.isAlarm)
                CSP_THROW(TypeError, "CppNode expected input "
                                         << name << " but found it as an alarm on node "
                                         << m_node.name());
            m_id = InputId(def.index);
        }

        // for basket inputs
        InputWrapper(const CppNode& node, InputId id) : m_node(node), m_id(id) {
        }
        ...
        bool makeActive() const {
            return const_cast<CppNode&>(m_node).makeActive(m_id);
        }
        bool makePassive() const {
            return const_cast<CppNode&>(m_node).makePassive(m_id);
        }
      ...
      protected:
        const TimeSeriesProvider* ts() const {
            return m_node.input(m_id);
        }
    ...
    class TypedInputWrapper : public InputWrapper {
      public:
        ...
        const T& lastValue() const {
            return ts()->template lastValueTyped<T>();
        }

        const T& valueAtIndex(int32_t index) const {
            return ts()->template valueAtIndex<T>(index);
        }
    };
    ...
    class BasketInputWrapper {
      public:
        ...
        InputBasketInfo::ticked_iterator tickedinputs() const {
            return basketInfo()->begin_ticked();
        }
        InputBasketInfo::valid_iterator validinputs() const {
            return basketInfo()->begin_valid();
        }
        ...
        /**
         * @return The declared input types. NOTE: The declared input type. Note, we allow
         * connecting slightly different types than declared types (derived class ts where baseclass
         * ts is expected, native type to dialect generic ...) so care should be taking when using
         * this type
         */
        const CspTypePtr& type() const {
            return m_type;
        }

      protected:
        const InputBasketInfo* basketInfo() const {
    ...
    class DictInputBasketWrapper : public BasketInputWrapper<ElemWrapperT> {
      public:
        ...
        const std::vector<std::string>& shape() const {
            return m_shape;
        }
    ...
    struct GenericAlarmWrapper : public GenericInputWrapper {
        ...
        Scheduler::Handle scheduleAlarm(DateTime time, const GenericInputWrapper& input) {
            ...
            return switchCspType(input.type(), [this, time, &input](auto tag) {
                ...
                auto* alarm =
                    static_cast<AlarmInputAdapter<T>*>(const_cast<TimeSeriesProvider*>(ts()));
    ...
    struct TypedAlarmWrapper : public TypedInputWrapper<T> {
        ...
        Scheduler::Handle scheduleAlarm(TimeDelta delta, const T& value) {
            auto* alarm =
                static_cast<AlarmInputAdapter<T>*>(const_cast<TimeSeriesProvider*>(this->ts()));
    ...
    class OutputWrapper {
      public:
        ...
        OutputWrapper(const char* name, const CppNode& node)
            : m_node(node), m_id(m_node.tsoutputDef(name).index) {
        }

        // basket outputs
        OutputWrapper(const CppNode& node, OutputId id) : m_node(node), m_id(id) {
        }
    ...
    class CSP {
      public:
        ...
        template <typename T>
        static Scheduler::Handle schedule_alarm(TypedAlarmWrapper<T>& alarm, DateTime time,
                                                const T& value) {
            return alarm.scheduleAlarm(time, value);
        }

        template <typename T>
        static Scheduler::Handle schedule_alarm(TypedAlarmWrapper<T>& alarm, TimeDelta delta,
                                                const T& value) {
            return alarm.scheduleAlarm(delta, value);
        }
...
#define SCALAR_INPUT_RENAMED(Type, DeclName, VarName) \
    csp::CppNode::Scalar<Type> VarName{#DeclName, *this};
...
#define CPPNODE_CREATE_FWD_DECL(Namespace, Name)                                     \
    namespace Namespace {                                                            \
    csp::CppNode* CPPNODE_CREATE_METHOD(Name)(csp::Engine * engine,                  \
                                              const csp::CppNode::NodeDef& nodedef); \
    }
