...
StructField::StructField(CspTypePtr type, const std::string& fieldname, size_t size,
                         size_t alignment)
...
Derived structs will simply append to the layout of the base struct, properly padding between
classes to align its fields properly. This layout is imposed on Struct instances.  Since Struct
needs refcount and meta * fields, for convenience they are stored *before* a Struct's "this" pointer
as hidden data.  This way struct ptrs can be passed into StructMeta without and adjustments required
...
StructMeta::StructMeta(const std::string& name, const Fields& fields,
    ...
    std::sort(m_fields.begin(), m_fields.end(), [](auto&& a, auto&& b) {
        return a->isNative() < b->isNative() || a->size() > b->size();
    });
    ...
    // NOTE we can be more efficient by sticking masks into any potential alignment gaps, dont want
    // to spend time on it at this point
    ...
    if (m_base) {
        ...
        m_fieldnames.insert(m_fieldnames.begin(), m_base->m_fieldnames.begin(),
                            m_base->m_fieldnames.end());
    ...
    for (size_t idx = m_firstPartialField; idx < m_fields.size(); ++idx) {
        ...
        if (!rv.second)
            CSP_THROW(ValueError, "csp Struct " << name << " attempted to add existing field "
                                                << m_fields[idx]->fieldname());
    }
}
...
Struct* StructMeta::createRaw() const {
    Struct* s;
    // This weird looking new call actually is calling Struct::opereatore new( size_t,
    // std::shared_ptr<StructMeta> ) its not actually doing an in-place on StructMeta this
...
void StructMeta::copyFromImpl(const Struct* src, Struct* dest, bool deepcopy) const {
    ...
    if (isNative())
        ...
        // note that partialNative will include the mask bytes - this sets the native part and the
        // mask
...
void StructMeta::updateFromImpl(const Struct* src, Struct* dest) const {
    for (Fields::const_iterator it = m_fields.begin() + m_firstPartialField; it != m_fields.end();
         ++it) {
...
bool StructMeta::isEqual(const Struct* x, const Struct* y) const {
    ...
    // esp if expecting NaN == NaN to be false, and when comparing -0.0 to +0.0.. may want to
    // revisit We we do we may as well remove the basepadding copy
    if (isNative())
...
size_t StructMeta::hash(const Struct* x) const {
    ...
    if (unlikely(s_recursionDepth > MAX_RECURSION_DEPTH))
        CSP_THROW(RecursionError, "Exceeded max recursion depth of "
                                      << MAX_RECURSION_DEPTH << " in " << name()
...
void* Struct::operator new(std::size_t count, const std::shared_ptr<const StructMeta>& meta) {
    ...
    // so be careful changing this assert if for some reason in the future sizeof( HiddenData )
    // changes
