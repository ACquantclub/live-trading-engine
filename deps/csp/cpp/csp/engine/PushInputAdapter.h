...
PushInputAdapter can optionally be registered with a PushGroup object.  This will generally be
created and maintained on the AdapterMAnager servicing the PushInputAdapter ( it may have one or
many, ie a MktDataHub would likely have one group for the hub ).  When a push input adapter is part
a group, there are implications specifically when dealing with non-collapsing inputs that are part
of the group.  To maintain "synchronicity" of data across inputs in the group, an engine cycle
processing a non-collapsing event must ensure that other LAST_VALUE ( aka collapsing ) inputs do no
advance past that non-collapsing event.  Think of bid/ask ticking and collapsing, but trade coming
in non-collapsed.  Whenever trade ticks, you want the cycle to have the bid/ask as of the time of
the trade, not to collapse further into the future.

Inputs that tick in a group can / should be submitted to the engine as part of a PushBatch.  Events
will be added to the batch and atomically released to the engine thread.  The last event of the push
batch is flagged.

PushGroups can be in one of 3 states.  At the start of a given cycle they are all NONE.
If a non-collapsing tick comes in, state goes to LOCKING until the end of that group's events in the
given batch.  At that point it goes to LOCKED. while a group is in LOCKING, other inputs within the
same batch can still get processed ( note there is a TODO for a non-collapsing input that has two
events in the same batch ).  Once a group is in a LOCKED state events get put in ( or remain ) in
the PendingEvents queue for the next engine cycle

For PushInputAdapters that are non-collapsing but have NO group, they get put into the "ungrouped"
events in PendingEvents to apply on subsequent cycles as well
*/
...
class PushInputAdapter : public InputAdapter {
  public:
    PushInputAdapter(Engine* engine, CspTypePtr& type, PushMode pushMode,
                     PushGroup* group = nullptr)
        : InputAdapter(engine, type, pushMode), m_group(group) {
    }
...
inline bool PushInputAdapter::consumeEvent(PushEvent* event, std::vector<PushGroup*>& dirtyGroups) {
    ...
    bool consumed = switchCspType(dataType(), [event](auto tag) {
