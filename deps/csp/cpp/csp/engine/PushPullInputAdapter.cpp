...
void PushPullInputAdapter::start(DateTime start, DateTime end) {
    m_nextPullEvent = nextPullEvent();
    if (m_nextPullEvent) {
        m_timerHandle = rootEngine()->scheduleCallback(
            m_nextPullEvent->time, [this]() { return processNextPullEvent() ? nullptr : this; });
    }
}
...
bool PushPullInputAdapter::processNextPullEvent() {
    bool consumed = switchCspType(dataType(), [this](auto tag) {
...
PushPullInputAdapter::PullDataEvent* PushPullInputAdapter::nextPullEvent() {
    ...
    if (event) {
        ...
        //- User asked to replay from EARLIEST, so they should get all ticks replayed and we cant
        //replay before starttime
        //- User asked to replay from STARTTIME in which case, if the adapter is written correctly,
        //we shouldnt get ticks before starttime
