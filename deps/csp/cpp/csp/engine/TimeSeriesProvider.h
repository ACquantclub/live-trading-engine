...
class TimeSeriesProvider {
  public:
    ...
    // We want want to find a better way to get 0-rank, node is only kept here for ranking logic
    // seems wasteful
    Node* node() const {
        return m_node;
    }
    ...
    // time series iface
    bool valid() const {
        return m_lastCycleCount > 0;
    }
    int32_t numTicks() const {
        return m_timeseries->numTicks();
    }  // number of ticks available in buffer
    uint32_t count() const {
        return m_timeseries->count();
    }
    uint64_t lastCycleCount() const {
        return m_lastCycleCount;
    }
    DateTime lastTime() const {
        return m_timeseries->lastTime();
    }

    template <typename T>
    void outputTickTyped(uint64_t cycleCount, DateTime timestamp, const T& value,
                         bool propagate = true);
    template <typename T>
    ...
    const TickBuffer<DateTime>* timeline() const {
        return m_timeseries->timeline();
    }

    int32_t getValueIndex(DateTime time, DuplicatePolicyEnum duplicatePolicy =
                                             DuplicatePolicyEnum::LAST_VALUE) const {
        return m_timeseries->getValueIndex(time, duplicatePolicy);
    }

    int32_t tickCountPolicy() const {
        return m_timeseries->tickCountPolicy();
    }
    TimeDelta tickTimeWindowPolicy() const {
        return m_timeseries->tickTimeWindowPolicy();
    }
    ...
    const EventPropagator& propagator() const {
        return m_propagator;
    }
...
template <typename T>
inline void TimeSeriesProvider::outputTickTyped(uint64_t cycleCount, DateTime timestamp,
                                                const T& value, bool propagate_) {
    ...
    if (m_lastCycleCount == cycleCount)
        CSP_THROW(RuntimeException,
                  "Attempted to output twice on the same engine cycle at time " << timestamp);
