...
EventPropagator::ConsumerVector::ConsumerVector() {
    // if this is getting constructed that means we have > 1 consumer, so start with initialization
    // to size 2
    m_capacity = 2;
    m_size = 0;
...
EventPropagator::ConsumerInfo* EventPropagator::ConsumerVector::findConsumer(Consumer* consumer,
                                                                             InputId id) {
...
bool EventPropagator::ConsumerVector::addConsumer(Consumer* consumer, InputId id,
                                                  bool checkExists) {
