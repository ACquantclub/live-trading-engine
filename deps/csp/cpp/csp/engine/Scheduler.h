...
class Scheduler {
  ...
  public:
    ...
    // unrolling / not collapsing.  This will ensure pending events for a given input adapter will
    // get deferred to the next cycle until its completely unrolled.  This avoids a O(n^2) loop of
    // reprocessing multiple pending events on a given input adapter.  If the callback isnt for an
    // input adapter or if it is able to consume / process the tick, return nullptr
    ...
    // This is used by DynamicNode when starting a new dynamic engine to keep track of all events
    // scheduled for "now" which should execute after start
    ...
    bool hasEvents() {
        return !m_map.empty() || m_pendingEvents.hasEvents();
    }
    DateTime nextTime() const {
        return m_pendingEvents.hasEvents() ? m_pendingEvents.time() : m_map.begin()->first;
    }
  ...
  private:
    ...
    using EventMap = std::map<DateTime, EventList, std::less<DateTime>,
                              MapAllocator<std::pair<const DateTime, EventList>>>;
    ...
    struct Event {
        ...
        Event(Callback&& f, uint64_t id_)
            : next(nullptr), prev(nullptr), func(std::move(f)), id(id_) {
        }
        ...
        Callback func;
        // NOTE THE IMPORTANCE of this not being the first member, we rely on setting this after
        // dealloc but BasicAllocator overrides the first 8 bytes for next -> link in the freelist
        uint64_t id = -1;
    };
    ...
    class PendingEvents {
      ...
      private:
        // NOTE the reason that this isnt simply kept as an unordered_map<InputAdapter*,Event> list
        // is to ensure simulated runs have complete reproducibility and are deterministic.  If we
        // key by InputAdapter * pointer, ordering of invocations will vary across runs ( this
        // generally wouldnt matter if all code was side-effect free, but even so basket input
        // tickeditems() order depends on order of basket elemnt ticks in a given cycle )
        // Instead we maintain an unordered_map<InputAdapter*, list iterator> to ensure we keep an
        // adapter once in the m_pendingEvents list The list itself is the one used for iteration
        // and execution of events, which should alwys be deterministic
        ...
        std::unordered_map<const InputAdapter*, std::list<PendingEventList>::iterator>
            m_pendingAdapters;
        Scheduler& m_scheduler;
    };
...
inline Scheduler::Handle Scheduler::scheduleCallback(Handle reserved, DateTime time,
                                                     Callback&& cb) {
