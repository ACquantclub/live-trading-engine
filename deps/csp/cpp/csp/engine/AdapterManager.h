...
class AdapterManager : public EngineOwned {
  public:
    ...
    // for sim inputs, this should process all entries with time "time" and return the time of the
    // next available tick, or return DateTime::NONE() if there is no more data.  Initial call will
    // be with starttime subsequent calls will be with the previously returned DateTime
    ...
    RootEngine* rootEngine() {
        return m_engine->rootEngine();
    }
    const RootEngine* rootEngine() const {
        return m_engine->rootEngine();
    }
    ...
    void pushStatus(int64_t level, int64_t errCode, const std::string& errMsg,
                    PushBatch* batch = nullptr) const;

  protected:
    // for adapters that want status adapter synced to a PushGroup
    virtual PushGroup* statusPushGroup() {
        return nullptr;
    }
...
inline void AdapterManager::scheduleTimerCB(DateTime next) {
    try {
        rootEngine()->scheduleCallback(next, [this]() {
            processSimTimerCB();
            return nullptr;
        });
    } catch (const ValueError& err) {
        CSP_THROW(ValueError,
                  "AdapterManager " << name() << " scheduler error: " << err.description());
    }
}
