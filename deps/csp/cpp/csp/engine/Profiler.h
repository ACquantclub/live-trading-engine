...
class Profiler {
  ...
  public:
    ...
    double utilization(int64_t nodes) const {
        auto accum = [](int64_t a, const std::pair<std::string, ProfStat>& b) {
            return a + b.second.m_exec;
        };
        return static_cast<double>(
                   std::accumulate(m_nodeStats.begin(), m_nodeStats.end(), 0, accum)) /
               (m_cycleStat.m_exec *
                (nodes - 2));  // don't include profiler node and its associated nullts
    }
