  ...
  public:
    struct TypeTraits {
      public:
        ...
        // We store bool vectors as vector<uint8_t> to account for oddities with vector<bool> across
        // build environments toCArrayElemType means from the array's storage type to csp element
        // type ( ie uint8_t -> bool ), otherwise its just Storage. toCArrayStorageType is the
        // inverse
    ...
    static constexpr bool isNative(TypeTraits::_enum t) {
        return t <= TypeTraits::MAX_NATIVE_TYPE;
    }

    bool isNative() const {
        return isNative(m_type);
    }
...
class CspStringType : public CspType {
  public:
    CspStringType(bool isBytes) : CspType(CspType::Type::STRING), m_isBytes(isBytes) {
    }
...
class CspEnumType : public CspType {
  public:
    CspEnumType(std::shared_ptr<CspEnumMeta>& meta) : CspType(CspType::Type::ENUM), m_meta(meta) {
    }

    const std::shared_ptr<CspEnumMeta>& meta() const {
        return m_meta;
    }
...
class CspStructType : public CspType {
  public:
    CspStructType(const std::shared_ptr<StructMeta>& meta)
        : CspType(CspType::Type::STRUCT), m_meta(meta) {
    }

    const std::shared_ptr<StructMeta>& meta() const {
        return m_meta;
    }
...
class CspArrayType : public CspType {
  public:
    ...
    const CspTypePtr& elemType() const {
        return m_elemType;
    }
    bool isPyStructFastList() const {
        return m_isPyStructFastList;
    }
