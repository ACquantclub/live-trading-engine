...
class Node : public Consumer, public EngineOwned {
  public:
    ...
    // data access
    INOUT_ID_TYPE numInputs() const {
        return m_def.numInputs;
    }
    INOUT_ID_TYPE numOutputs() const {
        return m_def.numOutputs;
    }
    ...
    input_iterator inputs() const override {
        return input_iterator(m_inputs, m_def.numInputs);
    }
    ...
    void removeDynamicInputBasketKey(INOUT_ID_TYPE basketIdx, const DialectGenericType& key,
...
inline const TimeSeriesProvider* Node::input(InputId id) const {
    return m_inputs[id.id].isSet<TimeSeriesProvider>() ? tsinput(id.id)
                                                       : inputBasket(id.id)->elem(id.elemId);
}

inline TimeSeriesProvider* Node::output(OutputId id) const {
    return m_outputs[id.id].isSet<TimeSeriesProvider>() ? tsoutput(id.id)
                                                        : outputBasket(id.id)->elem(id.elemId);
}
...
inline bool Node::makeBasketActive(INOUT_ID_TYPE id) {
    ...
    for (auto it = basketInfo->begin_inputs(); it; ++it)
        rv |= const_cast<TimeSeriesProvider*>(it.get())->addConsumer(this, InputId(id, it.elemId()),
                                                                     true);
    return rv;
}
...
inline bool Node::makeBasketPassive(INOUT_ID_TYPE id) {
    ...
    auto* basketInfo = inputBasket(id);
    for (auto it = basketInfo->begin_inputs(); it; ++it)
        rv |= const_cast<TimeSeriesProvider*>(it.get())->removeConsumer(this,
                                                                        InputId(id, it.elemId()));
    return rv;
}
...
inline void Node::removeDynamicInputBasketKey(INOUT_ID_TYPE basketIdx,
                                              const DialectGenericType& key, int64_t elemId,
                                              int64_t replaceId) {
    CSP_ASSERT(isInputBasket(basketIdx));
    CSP_ASSERT(inputBasket(basketIdx)->isDynamicBasket());
    auto* dynBasket = static_cast<DynamicInputBasketInfo*>(inputBasket(basketIdx));
