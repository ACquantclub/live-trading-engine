...
struct EngineOwned {
    ...
    void* operator new(size_t sz, Engine*) {
        return ::operator new(sz);
    }

    void operator delete(void* ptr, Engine*) {
        ::operator delete(ptr);
    }
...
class Engine {
  ...
  public:
    ...
    bool isRootEngine() const {
        return static_cast<void*>(m_rootEngine) == this;
    }
    ...
    const InputAdapters& inputAdapters() const {
        return m_inputAdapters;
    }
    const OutputAdapters& outputAdapters() const {
    ...
    void registerGraphOutput(const DialectGenericType& key,
                             std::shared_ptr<GraphOutputAdapter> adapter);
    csp::GraphOutputAdapter* graphOutput(const DialectGenericType& key);
    const std::vector<DialectGenericType>& graphOutputKeys() const {
        return m_graphOutputKeys;
    }
