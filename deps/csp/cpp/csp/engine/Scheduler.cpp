...
void Scheduler::PendingEvents::addPendingEvent(const InputAdapter* adapter, Event* event,
                                               DateTime now) {
...
void Scheduler::PendingEvents::executeCycle() {
    ...
    for (auto elt = m_pendingEvents.begin(); elt != m_pendingEvents.end();) {
        ...
        if (unlikely(event->func() != nullptr)) {
            ...
            // This is how PullInputAdapter is already implemented, it keeps consuming if next
            // returns the same timestamp ( needed for bursts ) If that is ever reworked, we can add
            // an exception for returning false here CSP_THROW( RuntimeException, "Logical error
            // process pending schedule events" );
            event->id = eventId;
            ++elt;
            continue;
        }
...
void Scheduler::executeNextEvents(DateTime now, Event* start) {
    ...
    if (event) {
        // if we started from the middle of the list, link from where we started, otherwise we would
        // miss these events Note that start is only set when starting dynamic engines
        if (startPrev)
            event->prev = startPrev;
    } else if (startPrev) {
        // if we have startPrev that means this is a dynamic engine start flush of events scheduled
        // for "now" in engine construction.  in this case we want to ensure the dynamic events are
...
Scheduler::DynamicEngineStartMonitor::DynamicEngineStartMonitor(Scheduler& s, DateTime now)
    : m_scheduler(s), m_now(now) {
    // note we only keep track of main map time, not pending events, since all initial dynamic
    // events will schedule into main
