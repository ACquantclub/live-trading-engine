...
GraphOutputAdapter::GraphOutputAdapter(csp::Engine* engine, int32_t tickCount,
                                       TimeDelta tickHistory)
    : OutputAdapter(engine), m_tickHistory(tickHistory), m_tickCount(tickCount) {
}
...
void GraphOutputAdapter::stop() {
    // for dynamic engines the timeseries that this is referring to will most likely not exist after
    // the GraphOutputAdapter is stopped.  For this reason, we pre-convert the requested data up
    // front so its cached until the end of the root engine.
