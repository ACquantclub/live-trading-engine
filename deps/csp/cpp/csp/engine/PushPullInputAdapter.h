...
namespace csp {
// A variation of PushInputAdapter that lets you schedule historical data as well.  Used for
// adapters that can replay history and switch to realtime seamlessly ( ie kafka )
...
class PushPullInputAdapter : public PushInputAdapter {
  ...
  protected:
    ...
    bool flaggedLive() const {
        return m_notifiedEndOfPull;
    }

  private:
    template <typename T>
    struct TypedPullDataEvent : public PullDataEvent {
        TypedPullDataEvent(DateTime t, T&& d) : PullDataEvent{t}, data(std::forward<T>(d)) {
        }
    ...
    if (live) {
        ...
        if (unlikely(m_notifiedEndOfPull))
            CSP_THROW(RuntimeException,
                      "PushPullInputAdapter tried to push a sim tick after live tick");
