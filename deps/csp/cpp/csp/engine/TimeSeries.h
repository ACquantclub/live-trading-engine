...
class TickBufferAccess {
  public:
    ...
    const T& value() const {
        return const_cast<TickBufferAccess<T>*>(this)->value();
    }
    uint32_t numTicks() const {
        return (unlikely(m_buffer != nullptr) ? m_buffer->numTicks() : 1);
    }
    TickBuffer<T>* buffer() {
        return m_buffer;
    }
    const TickBuffer<T>* buffer() const {
        return const_cast<TickBufferAccess<T>*>(this)->buffer();
    }
...
class TimeSeries {
  ...
  public:
    ...
    uint32_t numTicks() const {
        return (m_count > 0 ? m_timeline.numTicks() : 0);
    }
    ...
    const TickBuffer<DateTime>* timeline() const {
        return m_timeline.buffer();
    }
     ...
     * @return The index that matches the given timestamp with the given policy. The returned index
     * is non negative, and less then count(). -1 is returned if no value is found. Example: Assume
     * the following values: (09:30, 0), (09:31, 1), (09:31, 2), (09:33, 3) The following calls will
     * return (roughly the syntax is incorrect, it's just to demonstate symantics):
     ...
     *      getValueIndex(09:31, DuplicatePolicyEnum::FIRST_VALUE) // Returns 1 since it's the first
     * value for timestamp getValueIndex(09:32, DuplicatePolicyEnum::FIRST_VALUE) // Returns 2, NOTE
     * this is slightly counter intuitive,
     *                                                             // we return the first value
     * 09:32 that is last at
    ...
    /**
     * Same as getValueIndexRange, the difference is that it supports ALL_VALUES as duplicate policy
     * in which case the whole range is returned (both start and end indices are inclusive)
     * @param time
    ...
    int32_t tickCountPolicy() const {
        return m_bufferTickCountPolicy;
    }
    TimeDelta tickTimeWindowPolicy() const {
        return m_bufferTimeWindowPolicy;
    }
...
class TimeSeriesTyped : public TimeSeries {
    ...
    T& reserveSpaceForTick(DateTime timestamp) {
        ...
        if (unlikely(!m_bufferTimeWindowPolicy.isNone() && timeBuffer->full())) {
            ...
            if (unlikely(diff <= m_bufferTimeWindowPolicy)) {
                ...
                // if we increase perfectly, it might get expensive quickly.  Will go for accuracy
                // in first attempt
  ...
  private:
    /*
        ...
        Since most of the time we only use the last/current value, don't bother allocating the tick
       buffer until we know we actually need it, and instead just store the value in
       TickBufferAccess.
    */
...
inline TimeSeries::TimeSeries() : m_bufferTickCountPolicy(1), m_count(0) {
}
...
inline std::pair<int32_t, int32_t> TimeSeries::getValueIndexRange(
    DateTime time, DuplicatePolicyEnum duplicatePolicy) const {
    ...
    auto dateTimeWithIndexGetter = [&](int32_t index) {
        return DateTimeWithIndex{index, timeAtIndex(index)};
    };
    ...
    // Generally it's not possible to find the first index using binary search. Consider the
    // following example: timestamps of timeseries: 09:30, 09:31, 09:31, ..., 09:31, 09:33 If we are
    // looking for 09:32 the binary search should first deduce that 09:31 is the matching timestamp,
    // it should then first deduce that the 09:31 is the matching timesamp to be returned.
    ...
    while (startI.index < endI.index) {
        ...
        if (midI.time <= time) {
            if (unlikely(midI.index == endI.index)) {
                // We know that there are only 2 elements in the range. We know that startI.time >
                // time and endI.time <= time. So we know that we need to select endI as the
                // candidate time
                startI = endI;
            } else {
                endI = midI;
    ...
    // Both startI and endI are pointing to the same element now, the element that is the last
    // element before timestamp we now need to do a binary search to find the first element if
    // necessary. Instead of doing binary search though, we will just search linearly here since we
    // expect the number of elements for each timestamp to be small.
