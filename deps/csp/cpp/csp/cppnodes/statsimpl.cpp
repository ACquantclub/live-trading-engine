...
class _cross_sectional_as_list
    : public _generic_cross_sectional<double, std::vector<double>, _cross_sectional_as_list> {
  public:
    using _generic_cross_sectional<double, std::vector<double>,
...
class _computeTwoArg : public _compute<C> {
  public:
    ...
    void initDataValidator(DataValidator<C>& validator) override {
        validator =
            DataValidator<C>(this->min_data_points, this->ignore_na, this->arg1, this->arg2);
    }
};
...
class _computeEMA : public _compute<C> {
  public:
    ...
    void initDataValidator(DataValidator<C>& validator) override {
        validator =
            DataValidator<C>(this->min_data_points, true, alpha, this->ignore_na, horizon, adjust);
    }
};
...
// They either have an additional input (i.e. ddof for covariance) or are implemented differently
// (i.e. weighted mean, which needs weight inputs)
...
EXPORT_TEMPLATE_CPPNODE(_weighted_sem,
                        SINGLE_ARG(_bivarComputeOneArg<int64_t, WeightedStandardError>));
...
DECLARE_CPPNODE(_quantile) {
    ...
    START() {
        s_qtl =
            DataValidator<Quantile>(min_data_points, ignore_na, quants.value(), interpolation_type);
    }
