...
class Variance {
  public:
    ...
    void add(double x) {
        ...
        m_consecutiveValueCount =
            (m_consecutiveValueCount && x == m_lastValue ? m_consecutiveValueCount + 1 : 1);
        m_lastValue = x;
  ...
  private:
    ...
    // Below variables are used to eliminate numerical errors when all values in the window are
    // identical
    double m_lastValue;
...
class Covariance {
  public:
    ...
    double compute() const {
        return (m_count > m_ddof ? m_unnormCov / (m_count - m_ddof)
...
class WeightedCovariance {
  public:
    ...
    double compute() const {
        return (m_wsum > m_ddof ? m_unnormWCov / (m_wsum - m_ddof)
                                : std::numeric_limits<double>::quiet_NaN());
    }
...
double kurtCompute(double count, double mx, double mx2, double mx3, double mx4, double vx,
                   bool bias, bool excess) {
...
class Kurtosis {
  public:
    ...
    double compute() const {
        return kurtCompute(m_count, m_mx.compute(), m_mx2.compute(), m_mx3.compute(),
                           m_mx4.compute(), m_vx.compute(), m_bias, m_excess);
    }
...
using ost = boost::multi_index::multi_index_container<
    double, boost::multi_index::indexed_by<boost::multi_index::ranked_non_unique<
                boost::multi_index::identity<double>, Comparator>>>;

class Quantile {
    enum Interpolate { LINEAR, LOWER, HIGHER, MIDPOINT, NEAREST };
...
class Rank {
    enum RankMethod { MIN, MAX, AVG };

    enum NanOption { KEEP, LAST };
  ...
  public:
    ...
    double compute() const {
        ...
        if (likely(!isnan(m_lastval) &&
                   ((m_method == MAX && m_maxtree.size() > 0) || m_mintree.size() > 0))) {
            switch (m_method) {
                case MIN: {
                    if (m_mintree.size() == 1)
                        return 0;
                ...
                case AVG: {
                    ...
                    for (; it != m_mintree.end() && *it == m_lastval; it++)
                        max_rank++;  // While this is in theory O(n), in reality this loop is only
                                     // interated once, since there are likely no duplicate values
                                     // or very few.
  ...
  public:
    ...
    void remove(double x) {
        if (likely(!isnan(x))) {
            ...
            // EMA may go to zero with a non-zero normalizer due to lots of 0 values in the time
            // series: they are separate conditions
...
struct NanCheck {
    template <typename FloatingT, typename... V,
...
class DataValidator {
  ...
  private:
    ...
    static constexpr bool m_process_na =
        (std::is_same<T, EMA>::value || std::is_same<T, AdjustedEMA>::value ||
         std::is_same<T, AlphaDebiasEMA>::value || std::is_same<T, HalflifeEMA>::value ||
         std::is_same<T, HalflifeDebiasEMA>::value || std::is_same<T, Rank>::value);
    static constexpr bool m_consider_na =
        (std::is_same<T, First>::value || std::is_same<T, Last>::value);
};
...
DECLARE_CPPNODE(_generic_time_window_updates) {
  protected:
    ...
    // Its to fix a rare bug when trigger is separate from the data, and the data has multiple ticks
    // at the same timestamp In this case the first data tick gets picked up on trigger and sent
    // with additions, but the second tick does NOT get picked up, because trigger already finished.
    // Then when its time to remove, the value that wasnt sent on additions gets added to removed
    // because its past the timestamp... example: import csp from datetime import datetime,
    // timedelta import csp.stats data = [
    ...
    //
    // We use a simple counter to work around this issue, we wont output a removal if there are no
    // pending remaining
