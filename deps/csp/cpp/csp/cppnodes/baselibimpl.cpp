...
DECLARE_CPPNODE(unroll) {
    ...
    INIT_CPPNODE(unroll) {
        ...
        // we cant easily support unrolling list of typed lists ( ts[ [[int]] ] ).  Since we dont
        // recurse type info more than one level the input elemType would be DIALECT_GENERIC, but
        // the output type would be the correct ARRAY:Type type.  Briding the two here is complex
        // and not (currently) wirht the effort, so fallback to python by throwing NotImplemented
        auto& out_def = tsoutputDef("");
        if (out_def.type->type() == CspType::Type::ARRAY)
            CSP_THROW(NotImplemented,
                      "unroll cppimpl doesnt currently support unrolloing lists of typed lists");
    }
...
DECLARE_CPPNODE(collect) {
    ...
    INIT_CPPNODE(collect) {
        // we cant process 'T' of type typed list ( it [int] ) because the input type would be
        // ARRAY:INT64 but the output type would become ARRAY:DIALECT_GENERIC, which we cant create.
        // just fallback to python
        auto& x_def = tsinputDef("x");
        if (x_def.type->type() == CspType::Type::ARRAY)
        ...
        auto& out_def = tsoutputDef("");
        if (out_def.type->type() != CspType::Type::ARRAY)
            CSP_THROW(TypeError, "cppimpl for collect expected output type to be list, got "
                                     << out_def.type->type());
        ...
        if (elemType->type() != x_def.type->type())
            CSP_THROW(TypeError, "cppimpl for collect has unexpected type mistmatch, input type is "
                                     << x_def.type->type() << " but output array type is "
                                     << elemType->type());
    }
...
DECLARE_CPPNODE(demultiplex) {
    ...
    INIT_CPPNODE(demultiplex) {
        ...
        if (key_def.type->type() != CspType::Type::STRING)
            CSP_THROW(NotImplemented,
                      "cppimpl for demultiplex not supported on non-string key types");
    }
...
/*
@csp.node
def multiplex(x: {'K': ts['T']}, key: ts['K'], tick_on_index: bool = False, raise_on_bad_key: bool =
False):
    __outputs__(ts['T'])
*/
DECLARE_CPPNODE(multiplex) {
    ...
    INIT_CPPNODE(multiplex) {
        ...
        if (key_def.type->type() != CspType::Type::STRING)
            CSP_THROW(NotImplemented,
                      "cppimpl for multiplex not supported on non-string key types");
    }
...
DECLARE_CPPNODE(struct_field) {
    ...
    START() {
        ...
        if (!m_fieldAccess)
            CSP_THROW(TypeError, "Struct " << structType->meta()->name() << " has no field named "
                                           << field.value());
    }
...
DECLARE_CPPNODE(struct_fromts) {
    ...
    START() {
        for (size_t elemId = 0; elemId < inputs.shape().size(); ++elemId) {
            ...
            if (!structField)
                CSP_THROW(ValueError, cls.value()->name()
                                          << ".fromts() received unknown struct field \"" << key
                                          << "\"");

            if (structField->type()->type() != inputs[elemId].type()->type())
                CSP_THROW(TypeError, cls.value()->name()
                                         << ".fromts() field \"" << key << "\" expected ts type "
                                         << structField->type()->type() << " but got "
                                         << inputs[elemId].type()->type());
...
DECLARE_CPPNODE(struct_collectts) {
    ...
    START() {
        for (size_t elemId = 0; elemId < inputs.shape().size(); ++elemId) {
            ...
            if (!structField)
                CSP_THROW(ValueError, cls.value()->name()
                                          << ".collectts() received unknown struct field \"" << key
                                          << "\"");
