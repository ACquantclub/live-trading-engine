...
class Exception : public std::exception {
  public:
    Exception(const char* exType, const std::string& description, const char* file,
    ...
    ~Exception() {
        free(m_backtracemessages);
    }
    Exception(const Exception&);
    Exception(Exception&&);

    const char* what() const noexcept override {
        return full(false).c_str();
    }
    ...
    char** btmessages() const {
        return m_backtracemessages;
    }
    int btsize() const {
        return m_backtracesize;
    }

    std::string backtraceString() const;
    void writeBacktrace(std::ostream&)
        const;  // streams are generally not-copyable (e.g. std::cerr) so we need this overload
    void writeBacktrace(std::ostream&& dest) const {
        writeBacktrace(dest);
    }
...
#define CSP_DECLARE_EXCEPTION(DerivedException, BaseException)                       \
    class DerivedException : public BaseException {                                  \
      public:                                                                        \
        DerivedException(const char* exType, const std::string& r, const char* file, \
                         const char* func, int line)                                 \
...
#define CSP_THROW_EX(EX_TYPE, MSG, ...)                                                        \
    do {                                                                                       \
        ...
        desc << MSG;                                                                           \
        csp::throw_exc(                                                                        \
            EX_TYPE(#EX_TYPE, desc.str(), __FILENAME__, __FUNCTION__, __LINE__, __VA_ARGS__)); \
    } while (0)
...
#define CSP_TRUE_OR_THROW_RUNTIME(EXPR, MESSAGE) \
    CSP_TRUE_OR_THROW(EXPR, csp::RuntimeException, MESSAGE)
#define CSP_NOT_IMPLEMENTED CSP_THROW(csp::NotImplemented, "");

// This is kind of the same as CSP_ASSERT, the difference is that whatever will be validated with
// this check, will be checked in both debug and release builds. Generally CSP_TRUE_OR_THROW_RUNTIME
// is better since usually the error message should be customized.
