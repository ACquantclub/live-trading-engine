...
// From
// https://stackoverflow.com/questions/2443135/how-do-i-find-where-an-exception-was-thrown-in-c/2443366
...
void csp_terminate() {
    ...
    try {
        ...
        std::cerr << __FUNCTION__ << " caught unhandled csp::Exception. what(): " << ex.what()
                  << std::endl;
        if (ex.btsize() > 0)
            printBacktrace(ex.btmessages(), ex.btsize(), std::cerr);
    } catch (const std::exception& e) {
        std::cerr << __FUNCTION__ << " caught unhandled std::exception. what(): " << e.what()
                  << std::endl;
    } catch (...) {
