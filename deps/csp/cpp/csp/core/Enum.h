...
/*
Example usage:
Define your enum traits struct which must define a basic enum type named _enum with UNKNOWN = 0 and
NUM_TYPES members ( where NUM_TYPES is number of enum values ) Along with an instance of said enum
as a protected member m_value.  You can also add any methods you like on your traits:
...
struct Enum : public EnumTraits {
    ...
    constexpr Enum(const Enum& rhs) {
        this->m_value = rhs.m_value;
    }

    Enum(const char* s) {
        this->m_value = reverseMap().fromString(s);
    }
    Enum(const std::string& s) : Enum(s.c_str()) {
    }
    Enum(UType v);

    static const std::string& asString(EnumV v) {
        return mapping()[v];
    }

    const std::string& asString() const {
        return asString(this->m_value);
    }
    const char* asCString() const {
        return asString(this->m_value).c_str();
    }

    // this pulls in all comparison operators for free since w will auto-convert to the raw enum
    constexpr operator EnumV() const {
        return this->m_value;
    }

    constexpr UType value() const {
        return this->m_value;
    }

    // common convenience methods
    bool isKnown() const {
        return this->m_value != EnumTraits::UNKNOWN;
    }
    ...
    static constexpr size_t numTypes() {
        return (size_t)EnumTraits::NUM_TYPES;
    }
    ...
    static iterator end() {
        return iterator(numTypes());
    }

  protected:
    using Aliases = std::unordered_multimap<std::string, std::string>;

    struct ReverseMap : public std::unordered_map<const char*, typename EnumTraits::_enum,
                                                  hash::CStrHash, hash::CStrEq> {
        ...
        EnumV fromString(const char* s) const {
            ...
            if (it == this->end()) {
                ...
                CSP_THROW(ValueError, "Unrecognized enum value: " << s << " for enum "
...
Enum<EnumTraits>::Enum(UType v) {
    if (v < 0 || v >= numTypes()) {
        if (UnknownOnInvalidValue<EnumTraits>(0))
            ...
            CSP_THROW(ValueError, "enum value: " << v << " out of range for enum "
                                                 << typeid(EnumTraits).name());
    } else
