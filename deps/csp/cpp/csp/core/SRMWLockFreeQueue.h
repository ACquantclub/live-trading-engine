...
/*
  The SRMWLockFreeQueue is safe for multipe writers / single reader access.  Likely not the most
  efficient impl, but certainly one of the simplest!  template type is required to have an intrinsic
  next pointer
*/
template <typename T>
  ...
  public:
    class Batch {
      public:
        ...
        void append(T* event) {
            // note we link the batch events in reverse order so that we can do a quick CAS
            // switcheroo
            if (!m_head)
                m_head = m_tail = event;
            else {
    ...
    SRMWLockFreeQueue(bool blocking = false)
        : m_head(nullptr), m_wait(blocking ? new QueueWaiter : nullptr), m_curItems(nullptr) {
    }
    ~SRMWLockFreeQueue() {
        delete (m_wait);
    }
    ...
    bool empty() const {
        return m_head == nullptr && m_curItems == nullptr;
    }
    void push(T*);
...
inline void SRMWLockFreeQueue<T>::push(Batch& batch) {
    ...
    while (!m_head.compare_exchange_weak(batch.m_tail->next, batch.m_head,
