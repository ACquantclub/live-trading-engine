...
class TimeDelta {
  public:
    ...
    int64_t asMicroseconds() const {
        return m_ticks / NANOS_PER_MICROSECOND;
    }
    int64_t asMilliseconds() const {
        return m_ticks / NANOS_PER_MILLISECOND;
    }
    int64_t asSeconds() const {
        return m_ticks / NANOS_PER_SECOND;
    }

    int32_t days() const {
        return asSeconds() / SECONDS_PER_DAY;
    }
    int32_t hours() const {
        return (asSeconds() % SECONDS_PER_DAY) / 3600;
    }
    int32_t minutes() const {
        return (asSeconds() % 3600) / 60;
    }
    ...
    int32_t nanoseconds() const {
        return asNanoseconds() % NANOS_PER_SECOND;
    }

    TimeDelta abs() const {
        return TimeDelta(std::abs(m_ticks));
    }
    int sign() const {
        if (m_ticks < 0) {
            return -1;
    ...
    static constexpr TimeDelta fromSeconds(int64_t seconds) {
        return fromNanoseconds(seconds * NANOS_PER_SECOND);
    }
    ...
    static constexpr TimeDelta fromDays(int64_t days) {
        return fromSeconds(days * SECONDS_PER_DAY);
    }
    ...
    TimeDelta operator+(const TimeDelta& rhs) const {
        return TimeDelta(m_ticks + rhs.m_ticks);
    }
    TimeDelta operator-(const TimeDelta& rhs) const {
        return TimeDelta(m_ticks - rhs.m_ticks);
    }
    ...
    TimeDelta operator*(int mult) const {
        return TimeDelta(m_ticks * mult);
    }
    ...
    TimeDelta operator/(int div) const {
        return TimeDelta(m_ticks / div);
    }
    ...
    int64_t operator/(TimeDelta div) const {
        return m_ticks / div.m_ticks;
    }

    TimeDelta operator-() const {
        return TimeDelta(-m_ticks);
    }

    static constexpr TimeDelta ZERO() {
        return TimeDelta(0);
    }
    static constexpr TimeDelta NONE() {
        return TimeDelta(std::numeric_limits<int64_t>::min());
    }
    static constexpr TimeDelta MIN_VALUE() {
        return TimeDelta(std::numeric_limits<int64_t>::min() + 1);
    }  // min reserved for NONE
    static constexpr TimeDelta MAX_VALUE() {
...
class Date {
  public:
    ...
    size_t hash() const {
        return std::hash<int32_t>()(m_data.value);
    }
...
class Time {
  public:
    ...
    bool isNone() const {
        return (*this) == Time::NONE();
    }

    // from XX since midnight
    static Time fromNanoseconds(int64_t nanos) {
        return Time(nanos);
    }
    static Time fromMicroseconds(int64_t micros) {
        return fromNanoseconds(micros * NANOS_PER_MICROSECOND);
    }
    static Time fromMilliseconds(int64_t millis) {
        return fromNanoseconds(millis * NANOS_PER_MILLISECOND);
    }
    ...
    Time operator+(const TimeDelta& delta) const {
        return Time(m_ticks + delta.asNanoseconds());
    }
    Time operator-(const TimeDelta& delta) const {
        return Time(m_ticks - delta.asNanoseconds());
    }
    TimeDelta operator-(const Time& rhs) const {
        return TimeDelta::fromNanoseconds(m_ticks - rhs.m_ticks);
    }
...
inline Time::Time(int hour, int minute, int second, int32_t nanosecond) {
    ...
    m_ticks = (int64_t(hour) * 3600 + int64_t(minute) * 60 + int64_t(second)) * NANOS_PER_SECOND +
              nanosecond;
}
...
class DateTime {
  public:
    ...
    static DateTime fromNanoseconds(int64_t nanos) {
        return DateTime(nanos);
    }
    static DateTime fromMicroseconds(int64_t micros) {
        return fromNanoseconds(micros * NANOS_PER_MICROSECOND);
    }
    static DateTime fromMilliseconds(int64_t millis) {
        return fromNanoseconds(millis * NANOS_PER_MILLISECOND);
    }
    static DateTime fromSeconds(int64_t seconds) {
        return fromNanoseconds(seconds * NANOS_PER_SECOND);
    }
    ...
    bool isMin() const {
        return (*this) == DateTime::MIN_VALUE();
    }
    bool isMax() const {
        return (*this) == DateTime::MAX_VALUE();
    }

    // returns time / date component of the datetime
    Time time() const {
        return isNone() ? Time::NONE()
                        : Time::fromNanoseconds(m_ticks % (SECONDS_PER_DAY * NANOS_PER_SECOND));
    }
    Date date() const;

    // return a datetime with same date but with given time
    DateTime withTime(Time t) const {
        return DateTime((m_ticks / (SECONDS_PER_DAY * NANOS_PER_SECOND)) * SECONDS_PER_DAY *
                            NANOS_PER_SECOND +
                        t.asNanoseconds());
    }
    ...
    DateTime operator+(const TimeDelta& delta) const {
        return DateTime(m_ticks + delta.asNanoseconds());
    }
    DateTime operator-(const TimeDelta& delta) const {
        return DateTime(m_ticks - delta.asNanoseconds());
    }
    ...
    static constexpr DateTime NONE() {
    ...
    static constexpr DateTime MAX_VALUE() {
...
class DateTimeEx : public DateTime {
  public:
    ...
    // the fractional second access are non-cumulative, meaning microseconds() includes
    // milliseconds(). ie if we have micros 222333, milliseconds() wil return 222 and microseconds
    // will return 222333
    int milliseconds() const {
        return nanoseconds() / NANOS_PER_MILLISECOND;
    }
    int microseconds() const {
        return nanoseconds() / NANOS_PER_MICROSECOND;
    }
