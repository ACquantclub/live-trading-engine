...
inline BasicAllocator::BasicAllocator(size_t elemSize, size_t blockSize, bool useHugePage,
                                      bool grow) {
...
inline void BasicAllocator::allocBlock() {
    ...
    size_t allocSize;
    size_t rawsize = allocSize =
        m_arenas.size() ? m_arenas.back().size * 2 : m_blockSize * m_elemSize;
    void* buffer;
    ...
    if (m_useHugePage) {
        ...
        buffer = mmap(NULL, hugesz, PROT_READ | PROT_WRITE,
...
class TypedBasicAllocator : public BasicAllocator {
  public:
    ...
    TypedBasicAllocator(size_t blockSize, bool useHugePage, bool grow) {
        init(blockSize, useHugePage, grow);
    }
    ...
    T* allocate() {
        return static_cast<T*>(BasicAllocator::allocate());
    }
};
