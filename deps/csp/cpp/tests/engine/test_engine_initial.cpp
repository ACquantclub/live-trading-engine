...
class WriteNode : public Node {
  public:
    WriteNode(Engine* engine, std::string tag) : Node(NodeDef(1, 0), engine), m_tag(tag) {
    }

    void execute() {
        // std::cerr << engine() -> time() << ": " << m_tag << " " << input( InputId( 0 ) ) ->
        // lastValueTyped<double>() << std::endl;
        printf("%s: %s %f\n", engine()->time().asString().c_str(), m_tag.c_str(),
               input(InputId(0))->lastValueTyped<double>());
    }
...
class DummyPullAdapter : public PullInputAdapter<double> {
  public:
    DummyPullAdapter(Engine* engine, DateTime startTime, TimeDelta interval, int tickCount)
        : PullInputAdapter<double>(engine),
...
TEST(EngineTest, test_engine_initial) {
    ...
    auto a2 =
        new DummyPullAdapter(&engine, DateTime(2017, 1, 1, 0, 0, 1), TimeDelta::fromSeconds(2), 20);
