...
class OrderValidatorTest : public ::testing::Test {
  protected:
    void SetUp() override {
        ...
        valid_limit_order = std::make_shared<Order>("ord-001", "user-001", "AAPL", OrderType::LIMIT,
                                                    OrderSide::BUY, 100.0, 150.0);
    }

    OrderValidator validator;
    std::shared_ptr<Order> valid_limit_order;
};
...
TEST_F(OrderValidatorTest, InvalidSymbol) {
    auto order = std::make_shared<Order>("ord-002", "user-001", "MSFT", OrderType::LIMIT,
                                         OrderSide::BUY, 100.0, 300.0);
...
TEST_F(OrderValidatorTest, QuantityTooLow) {
    auto order = std::make_shared<Order>("ord-003", "user-001", "AAPL", OrderType::LIMIT,
                                         OrderSide::BUY, 0.5, 150.0);
    auto result = validator.validate(order);
    ASSERT_FALSE(result.is_valid);
...
TEST_F(OrderValidatorTest, QuantityTooHigh) {
    auto order = std::make_shared<Order>("ord-004", "user-001", "AAPL", core::OrderType::LIMIT,
                                         core::OrderSide::BUY, 1500.0, 150.0);
    auto result = validator.validate(order);
    ASSERT_FALSE(result.is_valid);
    EXPECT_EQ(result.error, ValidationError::INVALID_QUANTITY);
}
...
TEST_F(OrderValidatorTest, PriceTooLow) {
    auto order = std::make_shared<Order>("ord-005", "user-001", "AAPL", OrderType::LIMIT,
                                         OrderSide::BUY, 100.0, 5.0);
    auto result = validator.validate(order);
    ASSERT_FALSE(result.is_valid);
...
TEST_F(OrderValidatorTest, PriceTooHigh) {
    auto order = std::make_shared<Order>("ord-006", "user-001", "AAPL", OrderType::LIMIT,
                                         OrderSide::BUY, 100.0, 6000.0);
    auto result = validator.validate(order);
    ASSERT_FALSE(result.is_valid);
    EXPECT_EQ(result.error, ValidationError::INVALID_PRICE);
}
...
TEST_F(OrderValidatorTest, ValidMarketOrder) {
    auto order = std::make_shared<Order>("ord-007", "user-001", "GOOGL", OrderType::MARKET,
                                         OrderSide::SELL, 50.0, 0.0);
    auto result = validator.validate(order);
    ASSERT_TRUE(result.is_valid);
    EXPECT_EQ(result.error, ValidationError::NONE);
}
